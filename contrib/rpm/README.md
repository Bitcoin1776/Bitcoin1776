...
The RPM spec file provided here is for JFKBitcoin1776-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://jfkbitcoin1776.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[JFKBitcoin1776 git repository](https://github.com/jfkbitcoin1776/jfkbitcoin1776) but are not part of
...
Currently only `Source10` is used, for the example `jfkbitcoin1776.conf` file.
...
in the jfkbitcoin1776 git repository. At present only an SVG version of the JFKBitcoin1776
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the jfkbitcoin1776
...
needed when 0.12.1 is released, a proper fix is already in the JFKBitcoin1776 git
master branch.
...
idea to build JFKBitcoin1776 Core against a static version of that library compiled
...
The `jfkbitcoin1776.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `jfkbitcoin1776-server` package. At this point, most RPM based
...
The `jfkbitcoin1776-server` package should have SELinux support. How to properly do
...
JFKBitcoin1776-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/jfkbitcoin1776/). He has been
packaging JFKBitcoin1776 for Fedora at least since 2012.
...
4. I renamed the `jfkbitcoin1776` package that contains the Qt GUI to `jfkbitcoin1776-core` as
that appears to be how the general population refers to it, in contrast to
`jfkbitcoin1776-xt` or `jfkbitcoin1776-classic`. I wanted to make sure the general population
