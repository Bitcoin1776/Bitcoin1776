#!/bin/bash
# Copyright (c) 2013 The JFKBitcoin1776 Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete JFKBitcoin1776 database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no JFKBitcoin1776 datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old JFKBitcoin1776 datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected JFKBitcoin1776 0.7 datadir."
    ;;
  3)
    echo "Detected JFKBitcoin1776 pre-0.8 datadir."
    ;;
  4)
    echo "Detected JFKBitcoin1776 0.8 datadir."
    ;;
esac
