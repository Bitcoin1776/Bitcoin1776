...
###   jfkbitcoin1776core.org and jfkbitcoin1776.org and compares them.
...
WORKINGDIR="/tmp/jfkbitcoin1776_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://jfkbitcoin1776core.org"
HOST2="https://jfkbitcoin1776.org"
BASEDIR="/bin/"
VERSIONPREFIX="jfkbitcoin1776-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'jfkbitcoin1776-',
...
if [ $? -ne 0 ]; then
   echo "jfkbitcoin1776.org failed to provide signature file, but jfkbitcoin1776core.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "jfkbitcoin1776.org and jfkbitcoin1776core.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the JFKBitcoin1776 Core binary release signing key installed?"
   fi
