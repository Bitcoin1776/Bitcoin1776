 ...
 *  JFKBitcoin1776 because since the activation of BIP66, signatures are
 * verified to be
...
bool CPubKey::Verify(const uint256 &hash,
  ...
  secp256k1_ecdsa_signature sig;
  if (!secp256k1_ec_pubkey_parse(secp256k1_context_verify, &pubkey, &(*this)[0],
                                 size())) {
    return false;
  }
   ...
   * not historically been enforced in JFKBitcoin1776, so normalize them first.
   */
