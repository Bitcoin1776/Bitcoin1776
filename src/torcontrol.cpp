...
ParseTorReplyMapping(const std::string &s) {
  ...
  while (ptr < s.size()) {
    ...
    if (ptr < s.size() && s[ptr] == '"') { // Quoted string
      ...
      ++ptr; // skip closing '"'
      /**
       * Unescape value. Per https://spec.torproject.org/control-spec section
       * 2.1.1:
       *
      ...
      value = escaped_value;
    } else { // Unquoted value. Note that values can contain '=' at will, just
             // no spaces
      while (ptr < s.size() && s[ptr] != ' ') {
        value.push_back(s[ptr]);
        ++ptr;
...
/****** JFKBitcoin1776 specific TorController implementation ********/
...
void TorController::auth_cb(TorControlConnection &_conn,
  ...
  if (reply.code == 250) {
    ...
    // Note that the 'virtual' port doesn't have to be the same as our internal
    // port, but this is just a convenient
...
void TorController::protocolinfo_cb(TorControlConnection &_conn,
  ...
  if (reply.code == 250) {
    ...
    if (!torpassword.empty()) {
      ...
      LogPrint(BCLog::TOR, "tor: Using SAFECOOKIE authentication, reading "
