...
static uint256 BlockBuildMerkleTree(const CBlock &block, bool *fMutated,
  ...
  vMerkleTree.clear();
  vMerkleTree.reserve(block.vtx.size() * 2 +
...
BOOST_AUTO_TEST_CASE(merkle_test) {
  for (int i = 0; i < 32; i++) {
    // Try 32 block sizes: all sizes from 0 to 16 inclusive, and then 15 random
    // sizes.
    ...
    for (int mutate = 0; mutate <= 3; mutate++) {
      ...
      if (duplicate1 >= ntx)
        break; // Duplication of the entire tree results in a different root (it
               // adds a level).
      int ntx1 = ntx + duplicate1; // The resulting number of transactions after
                                   // the first duplication.
      int duplicate2 =
          mutate >= 2 ? 1 << ctz(ntx1) : 0; // Likewise for the second mutation.
      if (duplicate2 >= ntx1)
        break;
      int ntx2 = ntx1 + duplicate2;
      int duplicate3 =
          mutate >= 3 ? 1 << ctz(ntx2) : 0; // And for the third mutation.
      if (duplicate3 >= ntx2)
        break;
      ...
      // Optionally mutate by duplicating the last transactions, resulting in
      ...
      if (mutate == 0) {
        for (int loop = 0; loop < std::min(ntx, 16); loop++) {
          ...
          BOOST_CHECK(ComputeMerkleRootFromBranch(block.vtx[mtx]->GetHash(),
