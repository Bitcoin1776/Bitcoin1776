...
BOOST_AUTO_TEST_CASE(coins_cache_simulation_test) {
  ...
  std::vector<CCoinsViewCacheTest *>
      stack; // A stack of CCoinsViewCaches on top.
  ...
  for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
    ...
    {
      uint256 txid = txids[InsecureRandRange(
          txids.size())]; // txid we're going to modify in this iteration.
      Coin &coin = result[COutPoint(txid, 0)];

      // Determine whether to test HaveCoin before or after Access* (or both).
      // As these functions
      // can influence each other's behaviour by pulling things into the cache,
      // all combinations
      // are tested.
      bool test_havecoin_before = InsecureRandBits(2) == 0;
      bool test_havecoin_after = InsecureRandBits(2) == 0;

      bool result_havecoin = test_havecoin_before
      ...
      if (InsecureRandRange(5) == 0 || coin.IsSpent()) {
        ...
        if (InsecureRandRange(16) == 0 && coin.IsSpent()) {
          ...
          newcoin.out.scriptPubKey.assign(
              InsecureRandBits(6),
              0); // Random sizes so we can test memory usage accounting
          (coin.IsSpent() ? added_an_entry : updated_an_entry) = true;
          coin = newcoin;
        }
        stack.back()->AddCoin(COutPoint(txid, 0), std::move(newcoin),
...
// has the expected effect (the other duplicate is overwitten at all cache
// levels)
    ...
    if (randiter % 20 < 19) {
      ...
      // 17/20 times reconnect previous or add a regular tx
        ...
        if (randiter % 20 == 2 && disconnected_coins.size()) {
          ...
          // If this tx is already IN the UTXO, then it must be a coinbase, and
        ...
        // In this simple test coins only have two states, spent or unspent,
        // save the unspent state to restore
        old_coin = result[prevout];
        // Update the expected result of prevouthash to know these coins are
        // spent
        result[prevout].Clear();

        utxoset.erase(prevout);

        // The test is designed to ensure spending a duplicate coinbase will
        // work properly
        // if that ever happens and not resurrect the previously overwritten
        // coinbase
...
BOOST_AUTO_TEST_CASE(ccoins_serialization) {
  ...
  BOOST_CHECK_EQUAL(HexStr(cc1.out.scriptPubKey),
  ...
  BOOST_CHECK_EQUAL(HexStr(cc2.out.scriptPubKey),
                    HexStr(GetScriptForDestination(CKeyID(uint160(ParseHex(
