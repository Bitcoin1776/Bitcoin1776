...
BOOST_AUTO_TEST_CASE(SimpleRoundTripTest) {
  ...
  BOOST_CHECK_EQUAL(
      pool.mapTx.find(block.vtx[2]->GetHash())->GetSharedTx().use_count(),
      SHARED_TX_OFFSET + 0);
  ...
  // Do a simple ShortTxIDs RT
  {
    ...
    CBlock block2;
    {
      ...
      BOOST_CHECK(partialBlock.FillBlock(block2, {}) ==
                  READ_STATUS_INVALID); // No transactions
      partialBlock = tmp;
    }
    ...
    {
      PartiallyDownloadedBlock tmp = partialBlock;
      partialBlock.FillBlock(block2, {block.vtx[2]}); // Current implementation
                                                      // doesn't check txn here,
...
BOOST_AUTO_TEST_CASE(NonCoinbasePreforwardRTTest) {
  ...
  // Test with pre-forwarding tx 1, but not coinbase
  {
    ...
    {
      PartiallyDownloadedBlock tmp = partialBlock;
      partialBlock.FillBlock(block2, {block.vtx[1]}); // Current implementation
                                                      // doesn't check txn here,
                                                      // but don't require that
      partialBlock = tmp;
    }
    ...
    BOOST_CHECK_EQUAL(pool.mapTx.find(txhash)->GetSharedTx().use_count(),
                      SHARED_TX_OFFSET + 1); // + 1 because of partialBlockCopy.
  }
  BOOST_CHECK_EQUAL(pool.mapTx.find(txhash)->GetSharedTx().use_count(),
                    SHARED_TX_OFFSET + 0);
}
...
BOOST_AUTO_TEST_CASE(SufficientPreforwardRTTest) {
  ...
  BOOST_CHECK_EQUAL(
      pool.mapTx.find(block.vtx[1]->GetHash())->GetSharedTx().use_count(),
      SHARED_TX_OFFSET + 0);
  ...
  // Test with pre-forwarding coinbase + tx 2 with tx 1 in mempool
  {
    ...
    shortIDs.prefilledtxn[1] = {
        1, block.vtx[2]}; // id == 1 as it is 1 after index 1
