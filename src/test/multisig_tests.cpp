...
BOOST_AUTO_TEST_CASE(multisig_verify) {
  ...
  escrow << OP_2 << ToByteVector(key[0].GetPubKey())
         << ToByteVector(key[1].GetPubKey()) << ToByteVector(key[2].GetPubKey())
  ...
  for (int i = 0; i < 4; i++) {
    ...
    s = sign_multisig(a_and_b, keys, txTo[0], 0);
    BOOST_CHECK_MESSAGE(
        !VerifyScript(s, a_and_b, nullptr, flags,
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (i == 0 || i == 1) {
      BOOST_CHECK_MESSAGE(
          VerifyScript(s, a_or_b, nullptr, flags,
...
BOOST_AUTO_TEST_CASE(multisig_IsStandard) {
  ...
  malformed[1] << OP_2 << ToByteVector(key[0].GetPubKey())
               << ToByteVector(key[1].GetPubKey()) << OP_3 << OP_CHECKMULTISIG;
  malformed[2] << OP_0 << ToByteVector(key[0].GetPubKey())
               << ToByteVector(key[1].GetPubKey()) << OP_2 << OP_CHECKMULTISIG;
  malformed[3] << OP_1 << ToByteVector(key[0].GetPubKey())
               << ToByteVector(key[1].GetPubKey()) << OP_0 << OP_CHECKMULTISIG;
