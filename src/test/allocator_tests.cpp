...
BOOST_AUTO_TEST_CASE(arena_tests) {
  ...
  BOOST_CHECK(b.alloc(1024) ==
              nullptr); // memory is full, this must return nullptr
    ...
    *lockingSuccess = false;
    if (count > 0) {
      ...
      return reinterpret_cast<void *>(
          0x08000000 +
          (count << 24)); // Fake address, do not actually use this memory
    }
    return 0;
  }
...
BOOST_AUTO_TEST_CASE(lockedpool_tests_live) {
  ...
  *((uint32_t *)a0) = 0x1234;
  ...
  // If more than one new arena was allocated for the above tests, something is
  // wrong
