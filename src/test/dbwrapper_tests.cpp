...
BOOST_AUTO_TEST_CASE(existing_data_no_obfuscate) {
  ...
  BOOST_CHECK(is_null_key(dbwrapper_private::GetObfuscateKey(
      odbw))); // The key should be an empty string
...
BOOST_AUTO_TEST_CASE(iterator_ordering) {
  ...
  for (int seek_start : {0x00, 0x80}) {
    ...
    for (int x = seek_start; x < 255; ++x) {
      ...
      if (!it->Valid()) // Avoid spurious errors about invalid iterator's key
                        // and value in case of failure
        break;
