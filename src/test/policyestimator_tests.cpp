...
BOOST_AUTO_TEST_CASE(BlockPolicyEstimates) {
  ...
  while (blocknum < 200) {
    for (int j = 0; j < 10; j++) {  // For each fee
      for (int k = 0; k < 4; k++) { // add 4 fee txs
        tx.vin[0].prevout.n =
            10000 * blocknum + 100 * j + k; // make transaction unique
    ...
    if (blocknum == 3) {
      // At this point we should need to combine 3 buckets to get enough data
      // points
      // So estimateFee(1) should fail and estimateFee(2) should return
      // somewhere around
  ...
  // so estimateFee(1) would return 10*baseRate but is hardcoded to return
  // failure
  ...
  // change
  // We haven't decayed the moving average enough so we still have enough data
  // points in every bucket
  while (blocknum < 250)
    mpool.removeForBlock(block, ++blocknum);
  ...
  BOOST_CHECK(feeEst.estimateFee(1) == CFeeRate(0));
  for (int i = 2; i < 9; i++) { // At 9, the original estimate was already at
                                // the bottom (b/c scale = 2)
    BOOST_CHECK(feeEst.estimateFee(i).GetFeePerK() <
                origFeeEst[i - 1] - deltaFee);
  }
}
