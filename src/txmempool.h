...
#ifndef JFKBITCOIN1776_TXMEMPOOL_H
#define JFKBITCOIN1776_TXMEMPOOL_H
...
private:
  ...
  size_t nTxWeight;  //!< ... and avoid recomputing tx weight (also used for
                     //!GetTxSize())
...
public:
  bool operator()(const CTxMemPoolEntry &a, const CTxMemPoolEntry &b) const {
    ...
    double aModFee =
        fUseADescendants ? a.GetModFeesWithDescendants() : a.GetModifiedFee();
    double aSize =
        fUseADescendants ? a.GetSizeWithDescendants() : a.GetTxSize();

    double bModFee =
        fUseBDescendants ? b.GetModFeesWithDescendants() : b.GetModifiedFee();
    double bSize =
...
private:
  ...
  unsigned int nTransactionsUpdated; //!< Used by getblocktemplate to trigger
                                     //!CreateNewBlock() invocation
  CBlockPolicyEstimator *minerPolicyEstimator;

  uint64_t totalTxSize; //!< sum of all mempool tx's virtual sizes. Differs from
                        //!serialized tx size since witness data is discounted.
                        //!Defined in BIP 141.
  uint64_t cachedInnerUsage; //!< sum of dynamic memory usage of all the map
                             //!elements (NOT the maps themselves)

  mutable int64_t lastRollingFeeUpdate;
  mutable bool blockSinceLastRollingFeeBump;
  mutable double rollingMinimumFeeRate; //!< minimum fee to get into the pool,
...
public:
  ...
  std::vector<std::pair<uint256, txiter>>
      vTxHashes; //!< All tx witness hashes/entries in mapTx, in random order
...
public:
   ...
   * or
   *    look up parents from mapLinks. Must be true for entries not in the
   * mempool
   */
...
#endif // JFKBITCOIN1776_TXMEMPOOL_H
