...
void WalletModel::checkBalanceChanged() {
  ...
  if (cachedBalance != newBalance ||
    ...
    Q_EMIT balanceChanged(newBalance, newUnconfirmedBalance, newImmatureBalance,
                          newWatchOnlyBalance, newWatchUnconfBalance,
...
bool WalletModel::validateAddress(const QString &address) {
  CJFKBitcoin1776Address addressParsed(address.toStdString());
...
WalletModel::prepareTransaction(WalletModelTransaction &transaction,
  ...
  for (const SendCoinsRecipient &rcp : recipients) {
    ...
    if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
      ...
      total += subtotal;
    } else { // User-entered jfkbitcoin1776 address / amount:
      ...
      CScript scriptPubKey = GetScriptForDestination(
          CJFKBitcoin1776Address(rcp.address.toStdString()).Get());
...

  {
    ...
    bool fCreated =
        wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired,
...
WalletModel::sendCoins(WalletModelTransaction &transaction) {
    ...
    for (const SendCoinsRecipient &rcp : transaction.getRecipients()) {
      if (rcp.paymentRequest.IsInitialized()) {
        ...
        newTx->vOrderForm.push_back(make_pair(key, value));
                      ...
                      .isEmpty()) // Message from normal jfkbitcoin1776:URI
                                  // (jfkbitcoin1776:123...?message=example)
  ...
  checkBalanceChanged(); // update balance immediately, otherwise there could be
                         // a short noticeable delay until pollBalanceChanged
                         // hits
...
static void NotifyAddressBookChanged(WalletModel *walletmodel, CWallet *wallet,
                                     ...
                                     ChangeType status) {
  QString strAddress =
      QString::fromStdString(CJFKBitcoin1776Address(address).ToString());
...
static void NotifyWatchonlyChanged(WalletModel *walletmodel,
  ...
  QMetaObject::invokeMethod(walletmodel, "updateWatchOnlyFlag",
                            Qt::QueuedConnection, Q_ARG(bool, fHaveWatchonly));
}
...
WalletModel::UnlockContext WalletModel::requestUnlock() {
  ...
  // If wallet is still locked, unlock was failed or cancelled, mark context as
  // invalid
...
void WalletModel::listCoins(
  ...
  for (auto &group : wallet->ListCoins()) {
    auto &resultGroup = mapCoins[QString::fromStdString(
        CJFKBitcoin1776Address(group.first).ToString())];
...
bool WalletModel::saveReceiveRequest(const std::string &sAddress,
  ...
  CTxDestination dest = CJFKBitcoin1776Address(sAddress).Get();

  std::stringstream ss;
  ss << nId;
  std::string key =
      "rr" + ss.str(); // "rr" prefix = "receive request" in destdata
...
bool WalletModel::bumpFee(uint256 hash) {
  ...
  if (feeBump->getResult() != BumpFeeResult::OK) {
    QMessageBox::critical(
        0, tr("Fee bump error"),
        tr("Increasing transaction fee failed") + "<br />(" +
            (feeBump->getErrors().size()
  ...
  questionString.append(JFKBitcoin1776Units::formatHtmlWithUnit(
  ...
  if (!res) {
    QMessageBox::critical(0, tr("Fee bump error"),
                          tr("Can't sign transaction."));
    return false;
  }
  ...
  if (!res) {
    QMessageBox::critical(0, tr("Fee bump error"),
                          tr("Could not commit transaction") + "<br />(" +
                              QString::fromStdString(feeBump->getErrors()[0]) +
                              ")");
    return false;
  }
...
int WalletModel::getDefaultConfirmTarget() const { return nTxConfirmTarget; }

bool WalletModel::getDefaultWalletRbf() const { return fWalletRbf; }
