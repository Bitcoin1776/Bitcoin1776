...
/* Window elements init */
  ...
  ui->jfkbitcoin1776AtStartup->setToolTip(
      ui->jfkbitcoin1776AtStartup->toolTip().arg(tr(PACKAGE_NAME)));
  ui->jfkbitcoin1776AtStartup->setText(
      ui->jfkbitcoin1776AtStartup->text().arg(tr(PACKAGE_NAME)));

  ui->openJFKBitcoin1776ConfButton->setToolTip(
      ui->openJFKBitcoin1776ConfButton->toolTip().arg(tr(PACKAGE_NAME)));
  ...
  for (const QString &langStr : translations.entryList()) {
    ...
    if (langStr.contains("_")) {
      ...
      /** display language strings as "native language - native country (locale
       * name)", e.g. "Deutsch - Deutschland (de)" */
      ui->lang->addItem(locale.nativeLanguageName() + QString(" - ") +
                            locale.nativeCountryName() + QString(" (") +
                            langStr + QString(")"),
                        QVariant(langStr));
#else
      /** display language strings as "language - country (locale name)", e.g.
       * "German - Germany (de)" */
...
#if QT_VERSION >= 0x040800
      /** display language strings as "native language (locale name)", e.g.
       * "Deutsch (de)" */
      ui->lang->addItem(locale.nativeLanguageName() + QString(" (") + langStr +
                            QString(")"),
                        QVariant(langStr));
#else
      /** display language strings as "language (locale name)", e.g. "German
       * (de)" */
      ui->lang->addItem(QLocale::languageToString(locale.language()) +
                            QString(" (") + langStr + QString(")"),
                        QVariant(langStr));
  ...
  ui->unit->setModel(new JFKBitcoin1776Units(this));
...
void OptionsDialog::setModel(OptionsModel *_model) {
  ...
  /* warn when one of the following settings changes by user action (placed here
   * so init via mapper doesn't trigger them) */
...
void OptionsDialog::setMapper() {
  /* Main */
  mapper->addMapping(ui->jfkbitcoin1776AtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openJFKBitcoin1776ConfButton_clicked() {
  ...
  if (!GUIUtil::openJFKBitcoin1776Conf())
