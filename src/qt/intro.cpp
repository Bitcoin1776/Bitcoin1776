...
void FreespaceChecker::check() {
  ...
  while (parentDir.has_parent_path() && !fs::exists(parentDir)) {
    ...
    /* Check if we make any progress, break if not to prevent an infinite loop
     * here */
...
Intro::Intro(QWidget *parent)
  ...
  ui->sizeWarningLabel->setText(
      tr("%1 will download and store a copy of the JFKBitcoin1776 block chain.")
          .arg(tr(PACKAGE_NAME)) +
...
bool Intro::pickDataDirectory() {
  ...
  if (!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) ||
    ...
    intro.setWindowIcon(QIcon(":icons/jfkbitcoin1776"));
   ...
   * to
   * override -datadir in the jfkbitcoin1776.conf file in the default data
   * directory
   * (to be consistent with jfkbitcoin1776d behavior)
   */
  if (dataDir != getDefaultDataDirectory())
