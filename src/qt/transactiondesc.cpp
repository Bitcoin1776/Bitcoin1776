...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx,
  ...
  if (wtx.IsCoinBase() && nCredit == 0) {
    ...
    if (wtx.IsInMainChain())
      strHTML +=
          JFKBitcoin1776Units::formatHtmlWithUnit(unit, nUnmatured) + " (" +
    ...
    //
    strHTML += "<b>" + tr("Credit") + ":</b> " +
               JFKBitcoin1776Units::formatHtmlWithUnit(unit, nNet) + "<br>";
  } else {
    ...
    if (fAllFromMe) {
      ...
      for (const CTxOut &txout : wtx.tx->vout) {
          ...
          if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (wallet->mapAddressBook.count(address) &&
              ...
              strHTML +=
                  ...
                  " ";
            strHTML +=
                GUIUtil::HtmlEscape(CJFKBitcoin1776Address(address).ToString());
        ...
        strHTML +=
            "<b>" + tr("Debit") + ":</b> " +
            JFKBitcoin1776Units::formatHtmlWithUnit(unit, -txout.nValue) +
            "<br>";
        if (toSelf)
          strHTML +=
              "<b>" + tr("Credit") + ":</b> " +
      ...
      if (fAllToMe) {
        ...
        strHTML += "<b>" + tr("Total debit") + ":</b> " +
                   JFKBitcoin1776Units::formatHtmlWithUnit(unit, -nValue) +
                   "<br>";
      ...
      if (nTxFee > 0)
        strHTML += "<b>" + tr("Transaction fee") + ":</b> " +
                   JFKBitcoin1776Units::formatHtmlWithUnit(unit, -nTxFee) +
                   "<br>";
    } else {
      //
      ...
      for (const CTxIn &txin : wtx.tx->vin)
        if (wallet->IsMine(txin))
          strHTML += "<b>" + tr("Debit") + ":</b> " +
                     JFKBitcoin1776Units::formatHtmlWithUnit(
  ...
  strHTML += "<b>" + tr("Net amount") + ":</b> " +
             JFKBitcoin1776Units::formatHtmlWithUnit(unit, nNet, true) + "<br>";
  ...
  // Message from normal jfkbitcoin1776:URI
  ...
  if (wtx.IsCoinBase()) {
    ...
    strHTML +=
        "<br>" +
        tr("Generated coins must mature %1 blocks before they can be spent. "
           "When you generated this block, it was broadcast to the network to "
           "be added to the block chain. If it fails to get into the chain, "
           "its state will change to \"not accepted\" and it won't be "
           "spendable. This may occasionally happen if another node generates "
           "a block within a few seconds of yours.")
      ...
      if (pcoinsTip->GetCoin(prevout, prev)) {
        {
          ...
          if (ExtractDestination(vout.scriptPubKey, address)) {
            ...
            strHTML += QString::fromStdString(
                CJFKBitcoin1776Address(address).ToString());
          }
          strHTML = strHTML + " " + tr("Amount") + "=" +
                    JFKBitcoin1776Units::formatHtmlWithUnit(unit, vout.nValue);
