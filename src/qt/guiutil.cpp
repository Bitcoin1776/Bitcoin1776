...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent) {
  ...
  widget->setPlaceholderText(
      QObject::tr("Enter a JFKBitcoin1776 address (e.g. %1)")
...
bool parseJFKBitcoin1776URI(const QUrl &uri, SendCoinsRecipient *out) {
  ...
  for (QList<QPair<QString, QString>>::iterator i = items.begin();
    ...
    if (i->first == "message") {
      ...
      if (!i->second.isEmpty()) {
        if (!JFKBitcoin1776Units::parse(JFKBitcoin1776Units::JFK, i->second,
                                        &rv.amount)) {
          return false;
        }
      }
...
bool parseJFKBitcoin1776URI(QString uri, SendCoinsRecipient *out) {
  ...
  //
  //    Cannot handle this later, because jfkbitcoin1776:// will cause Qt to see
  //    the part after // as host,
...
QString formatJFKBitcoin1776URI(const SendCoinsRecipient &info) {
  ...
  if (info.amount) {
    ret += QString("?amount=%1")
               .arg(JFKBitcoin1776Units::format(
                   JFKBitcoin1776Units::JFK, info.amount, false,
                   JFKBitcoin1776Units::separatorNever));
    paramCount++;
  }
...
bool openJFKBitcoin1776Conf() {
  boost::filesystem::path pathConfig =
      GetConfigFile(JFKBITCOIN1776_CONF_FILENAME);
...
fs::path static StartupShortcutPath() {
  ...
  return GetSpecialFolderPath(CSIDL_STARTUP) /
         strprintf("JFKBitcoin1776 (%s).lnk", chain);
}
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list,
                                              CFURLRef findUrl) {
  // loop through the list of startup items and try to find the jfkbitcoin1776
  // app
...
bool GetStartOnSystemStartup() {
  ...
  LSSharedFileListItemRef foundItem =
      findStartupItemInList(loginItems, jfkbitcoin1776AppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart) {
  ...
  if (fAutoStart && !foundItem) {
    ...
    LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst,
                                  nullptr, nullptr, jfkbitcoin1776AppUrl,
                                  nullptr, nullptr);
