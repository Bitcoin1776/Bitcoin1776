...
static std::string Translate(const char *psz) {
  return QCoreApplication::translate("jfkbitcoin1776-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase,
  ...
  // Load e.g. jfkbitcoin1776_de.qm (shortcut "de" needs to be defined in
  // jfkbitcoin1776.qrc)
  if (translatorBase.load(lang, ":/translations/"))
    QApplication::installTranslator(&translatorBase);

  // Load e.g. jfkbitcoin1776_de_DE.qm (shortcut "de_DE" needs to be defined in
  // jfkbitcoin1776.qrc)
...
/** Class encapsulating JFKBitcoin1776 Core startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class JFKBitcoin1776Core : public QObject {
  Q_OBJECT
public:
  explicit JFKBitcoin1776Core();
...
/** Main JFKBitcoin1776 application object */
class JFKBitcoin1776Application : public QApplication {
  Q_OBJECT
public:
  explicit JFKBitcoin1776Application(int &argc, char **argv);
  ~JFKBitcoin1776Application();
  ...
  /// Get window identifier of QMainWindow (JFKBitcoin1776GUI)
  WId getMainWinId() const;

public Q_SLOTS:
  void initializeResult(bool success);
  void shutdownResult();
  /// Handle runaway exceptions. Shows a message box with the problem and quits
  /// the program.
...
#include "jfkbitcoin1776.moc"

JFKBitcoin1776Core::JFKBitcoin1776Core() : QObject() {}

void JFKBitcoin1776Core::handleRunawayException(const std::exception *e) {
  PrintExceptionContinue(e, "Runaway exception");
  Q_EMIT runawayException(QString::fromStdString(GetWarnings("gui")));
}

bool JFKBitcoin1776Core::baseInitialize() {
...
void JFKBitcoin1776Core::initialize() {
  try {
...
void JFKBitcoin1776Core::shutdown() {
  try {
...
JFKBitcoin1776Application::JFKBitcoin1776Application(int &argc, char **argv)
  ...
  // This must be done inside the JFKBitcoin1776Application constructor, or
  // after it, because
  // PlatformStyle::instantiate requires a QApplication
  std::string platformName;
  platformName =
      gArgs.GetArg("-uiplatform", JFKBitcoin1776GUI::DEFAULT_UIPLATFORM);
  platformStyle =
...
JFKBitcoin1776Application::~JFKBitcoin1776Application() {
  if (coreThread) {
...
#ifdef ENABLE_WALLET
void JFKBitcoin1776Application::createPaymentServer() {
  paymentServer = new PaymentServer(this);
}
#endif

void JFKBitcoin1776Application::createOptionsModel(bool resetSettings) {
  optionsModel = new OptionsModel(nullptr, resetSettings);
}

void JFKBitcoin1776Application::createWindow(const NetworkStyle *networkStyle) {
  window = new JFKBitcoin1776GUI(platformStyle, networkStyle, 0);
...
void JFKBitcoin1776Application::createSplashScreen(
...
void JFKBitcoin1776Application::startThread() {
  if (coreThread)
    return;
  coreThread = new QThread(this);
  JFKBitcoin1776Core *executor = new JFKBitcoin1776Core();
...
void JFKBitcoin1776Application::parameterSetup() {
  InitLogging();
  InitParameterInteraction();
}

void JFKBitcoin1776Application::requestInitialize() {
  qDebug() << __func__ << ": Requesting initialize";
  startThread();
  Q_EMIT requestedInitialize();
}

void JFKBitcoin1776Application::requestShutdown() {
...
void JFKBitcoin1776Application::initializeResult(bool success) {
  ...
  if (success) {
    ...
    if (!vpwallets.empty()) {
      ...
      window->addWallet(JFKBitcoin1776GUI::DEFAULT_WALLET, walletModel);
      window->setCurrentWallet(JFKBitcoin1776GUI::DEFAULT_WALLET);
    ...
    // jfkbitcoin1776: URIs or payment requests:
...
void JFKBitcoin1776Application::shutdownResult() {
  quit(); // Exit main loop after shutdown finished
}

void JFKBitcoin1776Application::handleRunawayException(const QString &message) {
  QMessageBox::critical(
      0, "Runaway exception",
      JFKBitcoin1776GUI::tr("A fatal error occurred. JFKBitcoin1776 can no "
                            "longer continue safely and will quit.") +
          QString("\n\n") + message);
  ::exit(EXIT_FAILURE);
}

WId JFKBitcoin1776Application::getMainWinId() const {
  if (!window)
    return 0;
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
  ...
  Q_INIT_RESOURCE(jfkbitcoin1776);
  Q_INIT_RESOURCE(jfkbitcoin1776_locale);

  JFKBitcoin1776Application app(argc, argv);
  ...
  /// 6. Determine availability of data directory and parse jfkbitcoin1776.conf
  ...
  try {
    gArgs.ReadConfigFile(gArgs.GetArg("-conf", JFKBITCOIN1776_CONF_FILENAME));
  } catch (const std::exception &e) {
  ...
  // calling IPC
  // - Do this *after* setting up the data directory, as the data directory hash
  // is used in the name
  // of the server.
  ...
  // jfkbitcoin1776: links repeatedly have their payment requests routed to this
  // process:
  ...
  // messages (WM_QUERYENDSESSION and WM_ENDSESSION)
  ...
  try {
    ...
    // Perform base initialization before spinning up initialization/shutdown
    // thread
    // This is acceptable because this function only contains steps that are
    // quick to execute,
    // so the GUI thread won't be held up.
    if (JFKBitcoin1776Core::baseInitialize()) {
