  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    ...
    case Amount:
      if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
        ...
        return JFKBitcoin1776Units::format(
            walletModel->getOptionsModel()->getDisplayUnit(),
            rec->recipient.amount, false, JFKBitcoin1776Units::separatorNever);
      else
        return JFKBitcoin1776Units::format(
            walletModel->getOptionsModel()->getDisplayUnit(),
...
bool RecentRequestsTableModel::removeRows(int row, int count,
  ...
  if (count > 0 && row >= 0 && (row + count) <= list.size()) {
    ...
    for (int i = 0; i < count; ++i) {
      rec = &list[row + i];
      if (!walletModel->saveReceiveRequest(rec->recipient.address.toStdString(),
                                           rec->id, ""))
        return false;
    }
