...
// Parse "jfkbitcoin1776:" URI into recipient object, return true on successful
// parsing
...
/** Qt event filter that intercepts ToolTipChange events, and replaces the
  tooltip with a rich text
  representation if needed. This assures that Qt can word-wrap long tooltip
  messages.
...
Q_SIGNALS:
  /** Emitted when the progressbar is clicked. The relative mouse coordinates of
   * the click are
   * passed to the signal.
   */
  void clicked(const QPoint &point);
