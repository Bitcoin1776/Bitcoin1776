...
public:
  ...
  void updateWallet(const uint256 &hash, int status, bool showTransaction) {
    ...
    case CT_NEW:
      if (inModel) {
        qWarning() << "TransactionTablePriv::updateWallet: Warning: Got "
                      "CT_NEW, but transaction is already in model";
        break;
      }
      if (showTransaction) {
        LOCK2(cs_main, wallet->cs_wallet);
        ...
        if (mi == wallet->mapWallet.end()) {
          qWarning() << "TransactionTablePriv::updateWallet: Warning: Got "
                        "CT_NEW, but transaction is not in wallet";
          break;
        }
    ...
    case CT_DELETED:
      if (!inModel) {
        qWarning() << "TransactionTablePriv::updateWallet: Warning: Got "
                      "CT_DELETED, but transaction is not in model";
        break;
      }
      ...
      break;
    case CT_UPDATED:
      // Miscellaneous updates -- nothing to do, status update will take care of
      // this, and is only computed for
...
TransactionTableModel::TransactionTableModel(
  ...
  columns << QString() << QString() << tr("Date") << tr("Type") << tr("Label")
          << JFKBitcoin1776Units::getAmountColumnTitle(
...
void TransactionTableModel::updateAmountColumnTitle() {
  columns[Amount] = JFKBitcoin1776Units::getAmountColumnTitle(
      walletModel->getOptionsModel()->getDisplayUnit());
...
TransactionTableModel::formatTxStatus(const TransactionRecord *wtx) const {
  ...
  case TransactionStatus::OpenUntilDate:
    status =
        tr("Open until %1").arg(GUIUtil::dateTimeStr(wtx->status.open_for));
    break;
  ...
  case TransactionStatus::MaturesWarning:
    status = tr("This block was not received by any other nodes and will "
...
QString TransactionTableModel::formatTxAmount(
    ...
    JFKBitcoin1776Units::SeparatorStyle separators) const {
  QString str = JFKBitcoin1776Units::format(
      walletModel->getOptionsModel()->getDisplayUnit(),
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const {
  ...
  case Qt::DisplayRole:
    ...
    case Amount:
      return formatTxAmount(rec, true, JFKBitcoin1776Units::separatorAlways);
    }
    break;
  case Qt::EditRole:
  ...
  case TxPlainTextRole: {
    ...
    QString txLabel = walletModel->getAddressTableModel()->labelForAddress(
        QString::fromStdString(rec->address));
    ...
    details.append(
        formatTxAmount(rec, false, JFKBitcoin1776Units::separatorNever));
    return details;
  }
  case ConfirmedRole:
  ...
  case FormattedAmountRole:
    ...
    return formatTxAmount(rec, false, JFKBitcoin1776Units::separatorNever);
  case StatusRole:
...
QVariant TransactionTableModel::headerData(int section,
  ...
  if (orientation == Qt::Horizontal) {
    if (role == Qt::DisplayRole) {
      ...
      switch (section) {
      case Status:
        return tr("Transaction status. Hover over this field to show number of "
                  "confirmations.");
      case Date:
      ...
      case Watchonly:
        return tr("Whether or not a watch-only address is involved in this "
                  "transaction.");
      case ToAddress:
