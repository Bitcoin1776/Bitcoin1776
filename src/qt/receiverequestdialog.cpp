...
#include "config/jfkbitcoin1776-config.h" /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::update() {
  ...
  QString uri = GUIUtil::formatJFKBitcoin1776URI(info);
  ...
  html += "<b>" + tr("Address") + "</b>: " + GUIUtil::HtmlEscape(info.address) +
          "<br>";
  if (info.amount)
    html += "<b>" + tr("Amount") +
  ...
  if (!info.label.isEmpty())
    html += "<b>" + tr("Label") + "</b>: " + GUIUtil::HtmlEscape(info.label) +
            "<br>";
  ...
  if (!uri.isEmpty()) {
    ...
    if (uri.length() > MAX_URI_LENGTH) {
      ui->lblQRCode->setText(tr("Resulting URI too long, try to reduce the "
                                "text for label / message."));
    } else {
...
void ReceiveRequestDialog::on_btnCopyURI_clicked() {
  GUIUtil::setClipboard(GUIUtil::formatJFKBitcoin1776URI(info));
}
