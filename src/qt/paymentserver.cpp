...
const int JFKBITCOIN1776_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString JFKBITCOIN1776_IPC_PREFIX("jfkbitcoin1776:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/jfkbitcoin1776-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/jfkbitcoin1776-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/jfkbitcoin1776-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the jfkbitcoin1776: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(JFKBITCOIN1776_IPC_PREFIX, Qt::CaseInsensitive)) // jfkbitcoin1776: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseJFKBitcoin1776URI(arg, &r) && !r.address.isEmpty())
            {
                CJFKBitcoin1776Address address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(JFKBITCOIN1776_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(JFKBITCOIN1776_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click jfkbitcoin1776: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start jfkbitcoin1776: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling jfkbitcoin1776: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in jfkbitcoin1776: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(JFKBITCOIN1776_IPC_PREFIX, Qt::CaseInsensitive)) // jfkbitcoin1776: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseJFKBitcoin1776URI(s, &recipient))
            {
                CJFKBitcoin1776Address address(recipient.address.toStdString());
                if (!address.IsValid()) {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid JFKBitcoin1776 address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CJFKBitcoin1776Address(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom jfkbitcoin1776 addresses are not supported
        ...
        // JFKBitcoin1776 amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(JFKBitcoin1776Units::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
