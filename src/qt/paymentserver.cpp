...
const char *BIP71_MIMETYPE_PAYMENTREQUEST =
    "application/jfkbitcoin1776-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char *argv[]) {
  for (int i = 1; i < argc; i++) {
    ...
    // If the jfkbitcoin1776: URI contains a payment request, we are not able to
    // detect the
    // network as that would require fetching and parsing the payment request.
    // That means clicking such an URI which contains a testnet payment request
    // will start a mainnet instance and throw a "wrong network" error.
    if (arg.startsWith(JFKBITCOIN1776_IPC_PREFIX,
                       Qt::CaseInsensitive)) // jfkbitcoin1776: URI
    {
...
PaymentServer::PaymentServer(QObject *parent, bool startLocalServer)
  ...
  if (startLocalServer) {
    ...
    if (!uriServer->listen(name)) {
      // constructor is called early in init, so don't use "Q_EMIT message()"
      // here
...
//
// OSX-specific way of handling jfkbitcoin1776: URIs and PaymentRequest mime
// types.
...
void PaymentServer::handleURIOrFile(const QString &s) {
  ...
  if (s.startsWith(JFKBITCOIN1776_IPC_PREFIX,
                   Qt::CaseInsensitive)) // jfkbitcoin1776: URI
  {
#if QT_VERSION < 0x050000
    QUrl uri(s);
      ...
      if (GUIUtil::parseJFKBitcoin1776URI(s, &recipient)) {
        ...
        Q_EMIT message(
            ...
            tr("URI cannot be parsed! This can be caused by an invalid "
               "JFKBitcoin1776 address or malformed URI parameters."),
            CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request,
  ...
  for (const std::pair<CScript, CAmount> &sendingTo : sendingTos) {
    ...
    if (ExtractDestination(sendingTo.first, dest)) {
      ...
      addresses.append(
          QString::fromStdString(CJFKBitcoin1776Address(dest).ToString()));
    } else if (!recipient.authenticatedMerchant.isEmpty()) {
      // Unauthenticated payment requests to custom jfkbitcoin1776 addresses are
      // not supported
    ...
    // JFKBitcoin1776 amounts are stored as (optional) uint64 in the protobuf
    ...
    if (IsDust(txOut, ::dustRelayFee)) {
      Q_EMIT message(
          ...
          tr("Requested payment amount of %1 is too small (considered dust).")
              .arg(JFKBitcoin1776Units::formatWithUnit(
    ...
    // Also verify that the final amount is still in a valid range after adding
    // additional amounts.
