...
CoinControlDialog::CoinControlDialog(const PlatformStyle *_platformStyle,
  ...
  copyTransactionHashAction = new QAction(
      tr("Copy transaction ID"), this); // we need to enable/disable this
  lockAction =
...
// click on header
  ...
  // see https://github.com/jfkbitcoin1776/jfkbitcoin1776/issues/5716
...
void CoinControlDialog::showMenu(const QPoint &point) {
  ...
  if (item) {
    ...
    if (item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64
                                                  // characters (this means its
                                                  // a child node, so its not a
                                                  // parent node in tree mode)
    {
...
void CoinControlDialog::copyAmount() {
  GUIUtil::setClipboard(
      JFKBitcoin1776Units::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::headerSectionClicked(int logicalIndex) {
  if (logicalIndex ==
      COLUMN_CHECKBOX) // click on most left column -> do nothing
  {
    ui->treeWidget->header()->setSortIndicator(sortColumn, sortOrder);
  } else {
    if (sortColumn == logicalIndex)
      ...
      sortOrder = ((sortColumn == COLUMN_LABEL || sortColumn == COLUMN_ADDRESS)
                       ? Qt::AscendingOrder
                       : Qt::DescendingOrder); // if label or address then
                                               // default => asc, else default
                                               // => desc
    }
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem *item, int column) {
  if (column == COLUMN_CHECKBOX &&
      item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64
                                                // characters (this means its a
                                                // child node, so its not a
                                                // parent node in tree mode)
  {
    ...
    else if (item->isDisabled()) // locked (this happens if "check all" through
                                 // parent node)
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog) {
  ...
  if (nQuantity > 0) {
    ...
    if (fWitness) {
      ...
      // transaction size calculation that will keep this estimate from being
      // exact.
      // usually, the result will be an overestimate within a couple of satoshis
      // so that the confirmation dialog ends up displaying a slightly smaller
      // fee.
      // also, the witness stack size value is a variable sized integer.
      // usually, the number of stack items will be well under the single byte
      // var int limit.
      nBytes += 2;         // account for the serialized marker and flag bytes
      nBytes += nQuantity; // account for the witness byte that holds the number
                           // of stack items for each input.
    }
    ...
    // Fee
    nPayFee =
        CWallet::GetMinimumFee(nBytes, *coinControl, ::mempool, ::feeEstimator,
  ...
  int nDisplayUnit = JFKBitcoin1776Units::JFK;
  ...
  l2->setText(
      JFKBitcoin1776Units::formatWithUnit(nDisplayUnit, nAmount)); // Amount
  l3->setText(
      JFKBitcoin1776Units::formatWithUnit(nDisplayUnit, nPayFee)); // Fee
  l4->setText(JFKBitcoin1776Units::formatWithUnit(nDisplayUnit,
                                                  nAfterFee)); // After Fee
  l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") +
              QString::number(nBytes));      // Bytes
  l7->setText(fDust ? tr("yes") : tr("no")); // Dust
  l8->setText(
      JFKBitcoin1776Units::formatWithUnit(nDisplayUnit, nChange)); // Change
  if (nPayFee > 0) {
...
void CoinControlDialog::updateView() {
  ...
  ui->treeWidget->clear();
  ui->treeWidget->setEnabled(false); // performance, otherwise updateLabels
                                     // would be called for every checked
                                     // checkbox
  ...
  for (const std::pair<QString, std::vector<COutput>> &coins : mapCoins) {
    ...
    for (const COutput &out : coins.second) {
      ...
      if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey,
        ...
        sAddress = QString::fromStdString(
            CJFKBitcoin1776Address(outputAddress).ToString());

        // if listMode or change => show jfkbitcoin1776 address. In tree mode,
    ...
    if (treeMode) {
      ...
      itemWalletAddress->setText(
          COLUMN_AMOUNT, JFKBitcoin1776Units::format(nDisplayUnit, nSum));
