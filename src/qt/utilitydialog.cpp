 ...
 * between
 * 32 and 64 bit builds. On other architectures, 32/64 bit may be more
 * ambiguous.
 */
  ...
  if (about) {
    ...
    std::string strUsage = HelpMessage(HMM_JFKBITCOIN1776_QT);
    ...
    strUsage += HelpMessageOpt(
        "-choosedatadir",
        strprintf(
            tr("Choose data directory on startup (default: %u)").toStdString(),
    ...
    strUsage += HelpMessageOpt(
        ...
        tr("Set SSL root certificates for payment request (default: -system-)")
            .toStdString());
    strUsage += HelpMessageOpt(
        "-splash",
        strprintf(
    ...
    if (showDebug) {
      strUsage += HelpMessageOpt(
          "-uiplatform", strprintf("Select platform to customize UI for (one "
                                   ...
                                   JFKBitcoin1776GUI::DEFAULT_UIPLATFORM));
    }
...
void HelpMessageDialog::printToConsole() {
  // On other operating systems, the expected action is to print the message to
  // the console.
  fprintf(stdout, "%s\n", qPrintable(text));
}
...
QWidget *ShutdownWindow::showShutdownWindow(JFKBitcoin1776GUI *window) {
  if (!window)
    return nullptr;
