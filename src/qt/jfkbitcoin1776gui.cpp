...
const std::string JFKBitcoin1776GUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
    "macosx"
...
const QString JFKBitcoin1776GUI::DEFAULT_WALLET = "~Default";

JFKBitcoin1776GUI::JFKBitcoin1776GUI(const PlatformStyle *_platformStyle,
...
JFKBitcoin1776GUI::~JFKBitcoin1776GUI() {
  // Unsubscribe from notifications from core
  unsubscribeFromCoreSignals();

  QSettings settings;
  settings.setValue("MainWindowGeometry", saveGeometry());
  if (trayIcon) // Hide tray icon, as deleting will let it linger until quit (on
                // Ubuntu)
    trayIcon->hide();
#ifdef Q_OS_MAC
  delete appMenuBar;
...
void JFKBitcoin1776GUI::createActions() {
  ...
  sendCoinsAction = new QAction(platformStyle->SingleColorIcon(":/icons/send"),
                                tr("&Send"), this);
  sendCoinsAction->setStatusTip(tr("Send coins to a JFKBitcoin1776 address"));
  ...
  receiveCoinsAction =
      new QAction(platformStyle->SingleColorIcon(":/icons/receiving_addresses"),
                  tr("&Receive"), this);
  receiveCoinsAction->setStatusTip(
      tr("Request payments (generates QR codes and jfkbitcoin1776: URIs)"));
  ...
  historyAction = new QAction(platformStyle->SingleColorIcon(":/icons/history"),
  ...
  aboutQtAction = new QAction(platformStyle->TextColorIcon(":/icons/about_qt"),
                              tr("About &Qt"), this);
  ...
  toggleHideAction = new QAction(platformStyle->TextColorIcon(":/icons/about"),
                                 tr("&Show / Hide"), this);
  ...
  signMessageAction->setStatusTip(tr("Sign messages with your JFKBitcoin1776 "
                                     "addresses to prove you own them"));
  verifyMessageAction =
      new QAction(platformStyle->TextColorIcon(":/icons/verify"),
                  tr("&Verify message..."), this);
  verifyMessageAction->setStatusTip(
      tr("Verify messages to ensure they were signed with specified "
  ...
  openAction = new QAction(platformStyle->TextColorIcon(":/icons/open"),
                           tr("Open &URI..."), this);
  openAction->setStatusTip(tr("Open a jfkbitcoin1776: URI or payment request"));
  ...
  showHelpMessageAction->setStatusTip(
      tr("Show the %1 help message to get a list with possible JFKBitcoin1776 "
...
void JFKBitcoin1776GUI::createMenuBar() {
#ifdef Q_OS_MAC
...
void JFKBitcoin1776GUI::createToolBars() {
  if (walletFrame) {
...
void JFKBitcoin1776GUI::setClientModel(ClientModel *_clientModel) {
  this->clientModel = _clientModel;
  if (_clientModel) {
    // Create system tray menu (or setup the dock menu) that late to prevent
    // users from calling actions,
    ...
    if (optionsModel) {
      ...
      // initialize the disable state of the tray icon with the current value in
      // the model.
...
#ifdef ENABLE_WALLET
bool JFKBitcoin1776GUI::addWallet(const QString &name,
...
bool JFKBitcoin1776GUI::setCurrentWallet(const QString &name) {
  if (!walletFrame)
    return false;
  return walletFrame->setCurrentWallet(name);
}

void JFKBitcoin1776GUI::removeAllWallets() {
  if (!walletFrame)
    return;
  setWalletActionsEnabled(false);
...
void JFKBitcoin1776GUI::setWalletActionsEnabled(bool enabled) {
...
void JFKBitcoin1776GUI::createTrayIcon(const NetworkStyle *networkStyle) {
#ifndef Q_OS_MAC
...
void JFKBitcoin1776GUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void JFKBitcoin1776GUI::trayIconActivated(
...
void JFKBitcoin1776GUI::optionsClicked() {
...
void JFKBitcoin1776GUI::aboutClicked() {
  if (!clientModel)
    return;
...
void JFKBitcoin1776GUI::showDebugWindow() {
...
void JFKBitcoin1776GUI::showDebugWindowActivateConsole() {
  rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
  showDebugWindow();
}

void JFKBitcoin1776GUI::showHelpMessageClicked() { helpMessageDialog->show(); }

#ifdef ENABLE_WALLET
void JFKBitcoin1776GUI::openClicked() {
...
void JFKBitcoin1776GUI::gotoOverviewPage() {
  overviewAction->setChecked(true);
  if (walletFrame)
    walletFrame->gotoOverviewPage();
}

void JFKBitcoin1776GUI::gotoHistoryPage() {
  historyAction->setChecked(true);
  if (walletFrame)
    walletFrame->gotoHistoryPage();
}

void JFKBitcoin1776GUI::gotoReceiveCoinsPage() {
  receiveCoinsAction->setChecked(true);
  if (walletFrame)
    walletFrame->gotoReceiveCoinsPage();
}

void JFKBitcoin1776GUI::gotoSendCoinsPage(QString addr) {
  sendCoinsAction->setChecked(true);
  if (walletFrame)
    walletFrame->gotoSendCoinsPage(addr);
}

void JFKBitcoin1776GUI::gotoSignMessageTab(QString addr) {
  if (walletFrame)
    walletFrame->gotoSignMessageTab(addr);
}

void JFKBitcoin1776GUI::gotoVerifyMessageTab(QString addr) {
  if (walletFrame)
    walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void JFKBitcoin1776GUI::updateNetworkState() {
  ...
  if (clientModel->getNetworkActive()) {
    tooltip =
        tr("%n active connection(s) to JFKBitcoin1776 network", "", count) +
...
void JFKBitcoin1776GUI::setNumConnections(int count) { updateNetworkState(); }

void JFKBitcoin1776GUI::setNetworkActive(bool networkActive) {
  updateNetworkState();
}

void JFKBitcoin1776GUI::updateHeadersSyncProgressLabel() {
  int64_t headersTipTime = clientModel->getHeaderTipTime();
  int headersTipHeight = clientModel->getHeaderTipHeight();
  int estHeadersLeft =
      (GetTime() - headersTipTime) / Params().GetConsensus().nPowTargetSpacing;
...
void JFKBitcoin1776GUI::setNumBlocks(int count, const QDateTime &blockDate,
  ...
  // Prevent orphan statusbar messages (e.g. hover Quit in main menu, wait until
  // chain-sync starts -> garbled text)
...
void JFKBitcoin1776GUI::message(const QString &title, const QString &message,
                                unsigned int style, bool *ret) {
  QString strTitle = tr("JFKBitcoin1776"); // default title
  ...
  // Append title to "JFKBitcoin1776 - "
  if (!msgType.isEmpty())
    strTitle += " - " + msgType;
...
void JFKBitcoin1776GUI::changeEvent(QEvent *e) {
...
void JFKBitcoin1776GUI::closeEvent(QCloseEvent *event) {
#ifndef Q_OS_MAC // Ignored on Mac
  if (clientModel && clientModel->getOptionsModel()) {
...
void JFKBitcoin1776GUI::showEvent(QShowEvent *event) {
...
#ifdef ENABLE_WALLET
void JFKBitcoin1776GUI::incomingTransaction(const QString &date, int unit,
  ...
  QString msg =
      tr("Date: %1\n").arg(date) +
      tr("Amount: %1\n")
          .arg(JFKBitcoin1776Units::formatWithUnit(unit, amount, true)) +
      tr("Type: %1\n").arg(type);
  if (!label.isEmpty())
    msg += tr("Label: %1\n").arg(label);
...
void JFKBitcoin1776GUI::dragEnterEvent(QDragEnterEvent *event) {
  // Accept only URIs
  if (event->mimeData()->hasUrls())
    event->acceptProposedAction();
}

void JFKBitcoin1776GUI::dropEvent(QDropEvent *event) {
  if (event->mimeData()->hasUrls()) {
...
bool JFKBitcoin1776GUI::eventFilter(QObject *object, QEvent *event) {
  // Catch status tip events
  if (event->type() == QEvent::StatusTip) {
    // Prevent adding text from setStatusTip(), if we currently use the status
    // bar for displaying other stuff
...
#ifdef ENABLE_WALLET
bool JFKBitcoin1776GUI::handlePaymentRequest(
...
void JFKBitcoin1776GUI::setHDStatus(int hdEnabled) {
...
void JFKBitcoin1776GUI::setEncryptionStatus(int status) {
  switch (status) {
...
void JFKBitcoin1776GUI::showNormalIfMinimized(bool fToggleHidden) {
  if (!clientModel)
    return;
...
void JFKBitcoin1776GUI::toggleHidden() { showNormalIfMinimized(true); }

void JFKBitcoin1776GUI::detectShutdown() {
...
void JFKBitcoin1776GUI::showProgress(const QString &title, int nProgress) {
  if (nProgress == 0) {
...
void JFKBitcoin1776GUI::setTrayIconVisible(bool fHideTrayIcon) {
  if (trayIcon) {
    trayIcon->setVisible(!fHideTrayIcon);
  }
}

void JFKBitcoin1776GUI::showModalOverlay() {
  if (modalOverlay &&
      (progressBar->isVisible() || modalOverlay->isLayerVisible()))
    modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(JFKBitcoin1776GUI *gui,
  ...
  bool ret = false;
  // In case of modal message, use blocking connection to wait for user to click
  // a button
...
void JFKBitcoin1776GUI::subscribeToCoreSignals() {
...
void JFKBitcoin1776GUI::unsubscribeFromCoreSignals() {
...
void JFKBitcoin1776GUI::toggleNetworkActive() {
  if (clientModel) {
...
UnitDisplayStatusBarControl::UnitDisplayStatusBarControl(
  ...
  QList<JFKBitcoin1776Units::Unit> units =
      JFKBitcoin1776Units::availableUnits();
  int max_width = 0;
  const QFontMetrics fm(font());
  for (const JFKBitcoin1776Units::Unit unit : units) {
    max_width = qMax(max_width, fm.width(JFKBitcoin1776Units::name(unit)));
  }
...
void UnitDisplayStatusBarControl::createContextMenu() {
  menu = new QMenu(this);
  for (JFKBitcoin1776Units::Unit u : JFKBitcoin1776Units::availableUnits()) {
    QAction *menuAction =
        new QAction(QString(JFKBitcoin1776Units::name(u)), this);
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits) {
  setText(JFKBitcoin1776Units::name(newUnits));
}
