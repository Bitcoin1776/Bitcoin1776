  ...
  inline void paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    if (index.data(TransactionTableModel::WatchonlyRole).toBool()) {
      ...
      QRect watchonlyRect(boundingRect.right() + 5,
                          mainRect.top() + ypad + halfheight, 16, halfheight);
    ...
    painter->setPen(foreground);
    QString amountText = JFKBitcoin1776Units::formatWithUnit(
        unit, amount, true, JFKBitcoin1776Units::separatorAlways);
    if (!confirmed) {
...
void OverviewPage::setBalance(const CAmount &balance,
  ...
  ui->labelBalance->setText(JFKBitcoin1776Units::formatWithUnit(
      unit, balance, false, JFKBitcoin1776Units::separatorAlways));
  ui->labelUnconfirmed->setText(JFKBitcoin1776Units::formatWithUnit(
      unit, unconfirmedBalance, false, JFKBitcoin1776Units::separatorAlways));
  ui->labelImmature->setText(JFKBitcoin1776Units::formatWithUnit(
      unit, immatureBalance, false, JFKBitcoin1776Units::separatorAlways));
  ui->labelTotal->setText(JFKBitcoin1776Units::formatWithUnit(
      unit, balance + unconfirmedBalance + immatureBalance, false,
      JFKBitcoin1776Units::separatorAlways));
  ui->labelWatchAvailable->setText(JFKBitcoin1776Units::formatWithUnit(
      unit, watchOnlyBalance, false, JFKBitcoin1776Units::separatorAlways));
  ui->labelWatchPending->setText(JFKBitcoin1776Units::formatWithUnit(
      unit, watchUnconfBalance, false, JFKBitcoin1776Units::separatorAlways));
  ui->labelWatchImmature->setText(JFKBitcoin1776Units::formatWithUnit(
      unit, watchImmatureBalance, false, JFKBitcoin1776Units::separatorAlways));
  ui->labelWatchTotal->setText(JFKBitcoin1776Units::formatWithUnit(
...
void OverviewPage::setWalletModel(WalletModel *model) {
  ...
  if (model && model->getOptionsModel()) {
    ...
    connect(model, SIGNAL(balanceChanged(CAmount, CAmount, CAmount, CAmount,
                                         CAmount, CAmount)),
            this, SLOT(setBalance(CAmount, CAmount, CAmount, CAmount, CAmount,
                                  CAmount)));
