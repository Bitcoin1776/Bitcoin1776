...
TransactionView::TransactionView(const PlatformStyle *platformStyle,
    ...
    : QWidget(parent), model(0), transactionProxyModel(0), transactionView(0),
      abandonAction(0), bumpFeeAction(0), columnResizingFixer(0) {
  // Build filter row
  ...
  watchOnlyWidget->addItem(platformStyle->SingleColorIcon(":/icons/eye_minus"),
  ...
  typeWidget->addItem(
      tr("Mined"), TransactionFilterProxy::TYPE(TransactionRecord::Generated));
  ...
  connect(amountWidget, SIGNAL(textChanged(QString)), amount_typing_delay,
          SLOT(start()));
...
void TransactionView::chooseDate(int idx) {
  ...
  case All:
    ...
    break;
  case Today:
    transactionProxyModel->setDateRange(QDateTime(current),
                                        TransactionFilterProxy::MAX_DATE);
    break;
  case ThisWeek: {
    // Find last Monday
    QDate startOfWeek = current.addDays(-(current.dayOfWeek() - 1));
    transactionProxyModel->setDateRange(QDateTime(startOfWeek),
                                        TransactionFilterProxy::MAX_DATE);
...
void TransactionView::changedAmount() {
  ...
  if (JFKBitcoin1776Units::parse(model->getOptionsModel()->getDisplayUnit(),
...
void TransactionView::exportClicked() {
  ...
  writer.addColumn(JFKBitcoin1776Units::getAmountColumnTitle(
  ...
  if (!writer.write()) {
    Q_EMIT message(
        tr("Exporting Failed"),
        tr("There was an error trying to save the transaction history to %1.")
            .arg(filename),
...
void TransactionView::contextualMenu(const QPoint &point) {
  ...
  // check if transaction can be abandoned, disable context menu action in case
  // it doesn't
  uint256 hash;
...
void TransactionView::openThirdPartyTxUrl(QString url) {
  ...
  QModelIndexList selection =
      transactionView->selectionModel()->selectedRows(0);
  if (!selection.isEmpty())
    QDesktopServices::openUrl(QUrl::fromUserInput(url.replace(
        "%s",
        selection.at(0).data(TransactionTableModel::TxHashRole).toString())));
}
