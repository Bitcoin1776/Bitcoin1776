...
struct AddressTableEntryLessThan {
  bool operator()(const AddressTableEntry &a,
                  const AddressTableEntry &b) const {
...
translateTransactionType(const QString &strPurpose, bool isMine) {
  ...
  if (strPurpose == "send")
    ...
    addressType = AddressTableEntry::Receiving;
           ...
           strPurpose == "") // if purpose not set, guess
    addressType =
        (isMine ? AddressTableEntry::Receiving : AddressTableEntry::Sending);
  return addressType;
}
...
public:
    ...
    cachedAddressTable.clear();
    {
      ...
      for (const std::pair<CTxDestination, CAddressBookData> &item :
        ...
        cachedAddressTable.append(
            AddressTableEntry(addressType, QString::fromStdString(strName),
                              QString::fromStdString(address.ToString())));
      }
    }
    // qLowerBound() and qUpperBound() require our cachedAddressTable list to be
    // sorted in asc order
    // Even though the map is already sorted this re-sorting step is needed
    // because the originating map
  ...
  void updateEntry(const QString &address, const QString &label, bool isMine,
    ...
    AddressTableEntry::Type newEntryType =
        translateTransactionType(purpose, isMine);

    switch (status) {
    case CT_NEW:
      if (inModel) {
        qWarning() << "AddressTablePriv::updateEntry: Warning: Got CT_NEW, but "
                      "entry is already in model";
        break;
      }
      parent->beginInsertRows(QModelIndex(), lowerIndex, lowerIndex);
      cachedAddressTable.insert(
          lowerIndex, AddressTableEntry(newEntryType, label, address));
      parent->endInsertRows();
      break;
    case CT_UPDATED:
      if (!inModel) {
        qWarning() << "AddressTablePriv::updateEntry: Warning: Got CT_UPDATED, "
                      "but entry is not in model";
        break;
      }
    ...
    case CT_DELETED:
      if (!inModel) {
        qWarning() << "AddressTablePriv::updateEntry: Warning: Got CT_DELETED, "
                      "but entry is not in model";
        break;
      }
...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value,
  ...
  std::string strPurpose =
      (rec->type == AddressTableEntry::Sending ? "send" : "receive");
  editStatus = OK;

  if (role == Qt::EditRole) {
    LOCK(wallet->cs_wallet); /* For SetAddressBook / DelAddressBook */
    CTxDestination curAddress =
        CJFKBitcoin1776Address(rec->address.toStdString()).Get();
    if (index.column() == Label) {
      ...
      CTxDestination newAddress =
          CJFKBitcoin1776Address(value.toString().toStdString()).Get();
      ...
      // addresses, if you try
      // to paste an existing address over another address (with a different
      // label)
...
QVariant AddressTableModel::headerData(int section, Qt::Orientation orientation,
                                       int role) const {
...
QModelIndex AddressTableModel::index(int row, int column,
...
void AddressTableModel::updateEntry(const QString &address,
                                    ...
                                    const QString &purpose, int status) {
  // Update address book model from JFKBitcoin1776 core
  priv->updateEntry(address, label, isMine, purpose, status);
}

QString AddressTableModel::addRow(const QString &type, const QString &label,
  ...
  if (type == Send) {
    ...
    // Check for duplicate addresses
    {
      ...
      if (wallet->mapAddressBook.count(
              CJFKBitcoin1776Address(strAddress).Get())) {
        editStatus = DUPLICATE_ADDRESS;
        return QString();
      }
    }
    ...
    strAddress = CJFKBitcoin1776Address(newKey.GetID()).ToString();
  } else {
    return QString();
  }
  ...
  {
    LOCK(wallet->cs_wallet);
    wallet->SetAddressBook(CJFKBitcoin1776Address(strAddress).Get(), strLabel,
                           (type == Send ? "send" : "receive"));
  }
  return QString::fromStdString(strAddress);
}

bool AddressTableModel::removeRows(int row, int count,
...
QString AddressTableModel::labelForAddress(const QString &address) const {
  {
    LOCK(wallet->cs_wallet);
    CJFKBitcoin1776Address address_parsed(address.toStdString());
...
void AddressTableModel::emitDataChanged(int idx) {
  Q_EMIT dataChanged(index(idx, 0, QModelIndex()),
                     index(idx, columns.length() - 1, QModelIndex()));
}
