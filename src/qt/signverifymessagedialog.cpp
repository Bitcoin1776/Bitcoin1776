...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked() {
  ...
  /* Clear old signature to ensure users don't get confused on error with an old
   * signature displayed */
  ui->signatureOut_SM->clear();

  CJFKBitcoin1776Address addr(ui->addressIn_SM->text().toStdString());
  if (!addr.IsValid()) {
    ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
    ui->statusLabel_SM->setText(tr("The entered address is invalid.") +
                                QString(" ") +
  ...
  if (!key.SignCompact(ss.GetHash(), vchSig)) {
    ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
    ui->statusLabel_SM->setText(
        QString("<nobr>") + tr("Message signing failed.") + QString("</nobr>"));
    return;
  }
...
void SignVerifyMessageDialog::on_verifyMessageButton_VM_clicked() {
  CJFKBitcoin1776Address addr(ui->addressIn_VM->text().toStdString());
  if (!addr.IsValid()) {
    ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
    ui->statusLabel_VM->setText(tr("The entered address is invalid.") +
                                QString(" ") +
                                tr("Please check the address and try again."));
    return;
  }
  ...
  bool fInvalid = false;
  std::vector<unsigned char> vchSig =
      DecodeBase64(ui->signatureIn_VM->text().toStdString().c_str(), &fInvalid);
  ...
  if (!(CJFKBitcoin1776Address(pubkey.GetID()) == addr)) {
    ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
