...
void OptionsModel::Init(bool resetSettings) {
  ...
  if (!settings.contains("nDisplayUnit"))
    settings.setValue("nDisplayUnit", JFKBitcoin1776Units::JFK);
...
QVariant OptionsModel::data(const QModelIndex &index, int role) const {
  if (role == Qt::EditRole) {
    ...
    case ProxyIP: {
      ...
      QStringList strlIpPort = settings.value("addrProxy")
                                   .toString()
    ...
    case ProxyIPTor: {
      // contains IP at index 0 and port at index 1
      QStringList strlIpPort = settings.value("addrSeparateProxyTor")
                                   .toString()
                                   .split(":", QString::SkipEmptyParts);
      return strlIpPort.at(0);
    }
...
void OptionsModel::checkAndMigrate() {
  ...
  if (settingsVersion < CLIENT_VERSION) {
    ...
    // see https://github.com/jfkbitcoin1776/jfkbitcoin1776/pull/8273
