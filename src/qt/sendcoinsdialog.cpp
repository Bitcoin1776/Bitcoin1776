...
void SendCoinsDialog::setModel(WalletModel *_model) {
  ...
  if (_model && _model->getOptionsModel()) {
    ...
    connect(_model, SIGNAL(balanceChanged(CAmount, CAmount, CAmount, CAmount,
                                          CAmount, CAmount)),
    ...
    connect(ui->checkBoxMinimumFee, SIGNAL(stateChanged(int)), this,
            SLOT(coinControlUpdateLabels()));
    connect(ui->optInRBF, SIGNAL(stateChanged(int)), this,
            SLOT(updateSmartFeeLabel()));
    connect(ui->optInRBF, SIGNAL(stateChanged(int)), this,
            SLOT(coinControlUpdateLabels()));
    ...
    ui->optInRBF->setCheckState(model->getDefaultWalletRbf() ? Qt::Checked
                                                             : Qt::Unchecked);

    // set the smartfee-sliders default value (wallets default conf.target or
    // last stored value)
    QSettings settings;
    if (settings.value("nSmartFeeSliderPosition").toInt() != 0) {
      // migrate nSmartFeeSliderPosition to nConfTarget
      // nConfTarget is available since 0.15 (replaced nSmartFeeSliderPosition)
      int nConfirmTarget = 25 -
                           settings.value("nSmartFeeSliderPosition")
                               .toInt(); // 25 == old slider range
...
void SendCoinsDialog::on_sendButton_clicked() {
  ...
  processSendCoinsReturn(prepareStatus,
                         JFKBitcoin1776Units::formatWithUnit(
  ...
  for (const SendCoinsRecipient &rcp : currentTransaction.getRecipients()) {
    ...
    QString amount =
        "<b>" + JFKBitcoin1776Units::formatHtmlWithUnit(
    ...
    } else if (!rcp.authenticatedMerchant
  ...
  for (JFKBitcoin1776Units::Unit u : JFKBitcoin1776Units::availableUnits()) {
    if (u != model->getOptionsModel()->getDisplayUnit())
      alternativeUnits.append(
          JFKBitcoin1776Units::formatHtmlWithUnit(u, totalAmount));
  }
  questionString.append(
      tr("Total Amount %1")
          .arg(JFKBitcoin1776Units::formatHtmlWithUnit(
  ...
  if (ui->optInRBF->isChecked()) {
    ...
    questionString.append(
        tr("This transaction signals replaceability (optin-RBF)."));
    questionString.append("</span>");
  }
...
void SendCoinsDialog::updateCoinControlState(CCoinControl &ctrl) {
  ...
  // Either custom fee will be used or if not selected, the confirmation target
  // from dropdown box
...
void SendCoinsDialog::updateSmartFeeLabel() {
  ...
  CFeeRate feeRate = CFeeRate(CWallet::GetMinimumFee(
  ...
  if (feeCalc.reason == FeeReason::FALLBACK) {
    ui->labelSmartFee2->show(); // (Smart fee not initialized yet. This usually
                                // takes a few blocks...)
...
void SendCoinsDialog::coinControlChangeEdited(const QString &text) {
  if (model && model->getAddressTableModel()) {
    ...
    {
      ui->labelCoinControlChangeLabel->setText(
          tr("Warning: Invalid JFKBitcoin1776 address"));
    } else // Valid address
    {
