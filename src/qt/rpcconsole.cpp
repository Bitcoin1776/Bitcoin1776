    ...
    case STATE_COMMAND_EXECUTED: {
      ...
      default:
        if (state == STATE_COMMAND_EXECUTED_INNER) {
          if (ch != ']') {
            // append char to the current argument (which is also used for the
            // query command)
            curarg += ch;
            break;
          }
          if (curarg.size() && fExecute) {
            // if we have a value query, query arrays with index and objects
            // with a string key
            UniValue subelement;
            if (lastResult.isArray()) {
              ...
              throw std::runtime_error(
                  "Invalid result query"); // no array or object: abort
        ...
        // if we have a non empty result, use it as stack argument otherwise as
        // general result
        if (curarg.size()) {
          if (stack.size())
        ...
        if ((ch == ')' || ch == '\n') && stack.size() > 0) {
          if (fExecute) {
            ...
            req.params = RPCConvertValues(
                stack.back()[0],
                std::vector<std::string>(stack.back().begin() + 1,
                                         stack.back().end()));
            req.strMethod = stack.back()[0];
#ifdef ENABLE_WALLET
            // TODO: Move this logic to WalletModel
            if (!vpwallets.empty()) {
              // in Qt, use always the wallet with index 0 when running with
              // multiple wallets
              QByteArray encodedName = QUrl::toPercentEncoding(
                  QString::fromStdString(vpwallets[0]->GetName()));
              req.URI = "/wallet/" + std::string(encodedName.constData(),
                                                 encodedName.length());
            }
#endif
    ...
    try // Nice formatting for standard-format error
    {
      ...
      Q_EMIT reply(RPCConsole::CMD_ERROR,
                   QString::fromStdString(message) + " (code " +
                       QString::number(code) + ")");
    } catch (const std::runtime_error &) // raised when converting to invalid
                                         // type, i.e. missing code or message
...
bool RPCConsole::eventFilter(QObject *obj, QEvent *event) {
  if (event->type() == QEvent::KeyPress) // Special key handling
  {
    ...
    case Qt::Key_Enter:
      ...
      break;
    default:
      // Typing in messages widget brings focus to line edit, and redirects key
      // there
      // Exclude most combinations and keys that emit no text, except paste
      // shortcuts
...
void RPCConsole::setClientModel(ClientModel *model) {
  ...
  if (model && clientModel->getPeerTableModel() &&
    ...
    // We need to use int (instead of int64_t), because signal mapper only
    // supports
    ...
    // peer table signal handling - update peer details when new nodes are added
    // to the model
    connect(model->getPeerTableModel(), SIGNAL(layoutChanged()), this,
            SLOT(peerLayoutChanged()));
    // peer table signal handling - cache selected node ids
    connect(model->getPeerTableModel(), SIGNAL(layoutAboutToBeChanged()), this,
    ...
    // ban table signal handling - clear peer details when clicking a peer in
    // the ban table
      ...
      (tr("Welcome to the %1 RPC console.").arg(tr(PACKAGE_NAME)) + "<br>" +
       ...
       "<br>" + tr("Type <b>help</b> for an overview of available commands.")) +
          ...
          tr("WARNING: Scammers have been active, telling users to type "
             "commands here, stealing their wallet contents. Do not use this "
             "console without fully understanding the ramifications of a "
             "command.") +
          "</span>",
      true);
}
...
void RPCConsole::setNetworkActive(bool networkActive) { updateNetworkState(); }
  ...
  if (!cmd.isEmpty()) {
    ...
    try {
      ...
      QMessageBox::critical(
          this, "Error", QString("Error: ") + QString::fromStdString(e.what()));
      return;
    }
...
void RPCConsole::peerLayoutAboutToChange() {
  ...
  for (int i = 0; i < selected.size(); i++) {
    const CNodeCombinedStats *stats =
        clientModel->getPeerTableModel()->getNodeStats(selected.at(i).row());
