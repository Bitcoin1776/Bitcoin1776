  ...
  if (nNet > 0 || wtx.IsCoinBase()) {
    ...
    for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
      ...
      if (mine) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address) &&
          ...
          // Received by JFKBitcoin1776 Address
          sub.type = TransactionRecord::RecvWithAddress;
          sub.address = CJFKBitcoin1776Address(address).ToString();
        } else {
    ...
    if (fAllFromMe && fAllToMe) {
      ...
      parts.last().involvesWatchAddress =
          involvesWatchAddress; // maybe pass to TransactionRecord as
      ...
      for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        CTxDestination address;
        if (ExtractDestination(txout.scriptPubKey, address)) {
          // Sent to JFKBitcoin1776 Address
