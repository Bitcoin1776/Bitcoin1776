...
#if defined(NDEBUG)
#error "JFKBitcoin1776 cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "JFKBitcoin1776 Signed Message:\n";
...
void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool,
  ...
  while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
    ...
    if (!fAddToMempool || (*it)->IsCoinBase() ||
        !AcceptToMemoryPool(mempool, stateDummy, *it, false, nullptr, nullptr,
                            true)) {
  ...
  LimitMempoolSize(
      mempool, gArgs.GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000,
...
static bool CheckInputsFromMempoolAndCache(const CTransaction &tx,
                                           ...
                                           CTxMemPool &pool, unsigned int flags,
                                           bool cacheSigStore,
...
static bool AcceptToMemoryPoolWorker(
    const CChainParams &chainparams, CTxMemPool &pool, CValidationState &state,
  ...
  // (override with -prematurewitness)
  bool witnessEnabled =
      IsWitnessEnabled(chainActive.Tip(), chainparams.GetConsensus());
  if (!gArgs.GetBoolArg("-prematurewitness", false) && tx.HasWitness() &&
      !witnessEnabled) {
...

  {
    ...
    LockPoints lp;
    {
      ...
      for (const CTxIn txin : tx.vin) {
        ...
        if (!view.HaveCoin(txin.prevout)) {
          ...
          return false; // fMissingInputs and !state.IsInvalid() is used to
                        // detect this condition, don't set state.Invalid()
        }
      }
      ...
      // Only accept BIP68 sequence locked transactions that can be mined in the
      // next
      // block; we don't want our mempool filled up with transactions that can't
      // be mined yet.
      // Must keep pool.cs for this unless we change CheckSequenceLocks to take
      // a
    ...
    if (!pool.CalculateMemPoolAncestors(entry, setAncestors, nLimitAncestors,
                                        nLimitAncestorSize, nLimitDescendants,
                                        nLimitDescendantSize, errString)) {
      return state.DoS(0, false, REJECT_NONSTANDARD, "too-long-mempool-chain",
                       false, errString);
    }

    // A transaction that spends outputs that would be replaced by it is
    // invalid. Now
    ...
    if (fReplacementTransaction) {
      ...
      for (const uint256 &hashConflicting : setConflicts) {
        ...
        if (newFeeRate <= oldFeeRate) {
          return state.DoS(
              0, false, REJECT_INSUFFICIENTFEE, "insufficient fee", false,
              strprintf(
                  "rejecting replacement %s; new feerate %s <= old feerate %s",
                  hash.ToString(), newFeeRate.ToString(),
                  oldFeeRate.ToString()));
        }
      ...
      if (nConflictingCount <= maxDescendantsToVisit) {
        ...
        return state.DoS(0, false, REJECT_NONSTANDARD,
                         ...
                         strprintf("rejecting replacement %s; too many "
                                   "potential replacements (%d > %d)\n",
                                   hash.ToString(), nConflictingCount,
      ...
      for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        if (!setConflictsParents.count(tx.vin[j].prevout.hash)) {
          ...
          if (pool.mapTx.find(tx.vin[j].prevout.hash) != pool.mapTx.end())
            return state.DoS(
                0, false, REJECT_NONSTANDARD, "replacement-adds-unconfirmed",
                false,
      ...
      if (nModifiedFees < nConflictingFees) {
        return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "insufficient fee",
                         false,
                         strprintf("rejecting replacement %s, less fees than "
                                   "conflicting txs; %s < %s",
                                   hash.ToString(), FormatMoney(nModifiedFees),
      ...
      if (nDeltaFees < ::incrementalRelayFee.GetFee(nSize)) {
        return state.DoS(
            0, false, REJECT_INSUFFICIENTFEE, "insufficient fee", false,
            strprintf("rejecting replacement %s, not enough additional fees to "
                      "relay; %s < %s",
    ...
    // This is done last to help prevent CPU exhaustion denial-of-service
    // attacks.
    ...
    if (!CheckInputs(tx, state, view, true, scriptVerifyFlags, true, false,
      ...
      CValidationState
          stateDummy; // Want reported failures to be from first CheckInputs
      if (!tx.HasWitness() &&
          CheckInputs(tx, stateDummy, view, true,
                      scriptVerifyFlags &
                          ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK),
                      true, false, txdata) &&
          !CheckInputs(tx, stateDummy, view, true,
                       scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true,
                       false, txdata)) {
    ...
    if (!CheckInputsFromMempoolAndCache(tx, state, view, pool,
      ...
      if (!(~scriptVerifyFlags & currentBlockScriptVerifyFlags)) {
        ...
        if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS,
                         true, false, txdata)) {
          return error("%s: ConnectInputs failed against MANDATORY but not "
    ...
    for (const CTxMemPool::txiter it : allConflicting) {
      LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s JFK additional "
                               "fees, %d delta bytes\n",
    ...
    bool validForFeeEstimation = !fReplacementTransaction &&
    ...
    if (!fOverrideMempoolLimit) {
      LimitMempoolSize(
          pool, gArgs.GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000,
          gArgs.GetArg("-mempoolexpiry", DEFAULT_MEMPOOL_EXPIRY) * 60 * 60);
      if (!pool.exists(hash))
...
static bool AcceptToMemoryPoolWithTime(
  ...
  bool res = AcceptToMemoryPoolWorker(
      chainparams, pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime,
...
bool AcceptToMemoryPool(CTxMemPool &pool, CValidationState &state,
  ...
  return AcceptToMemoryPoolWithTime(chainparams, pool, state, tx, fLimitFree,
 ...
 * placed in hashBlock */
bool GetTransaction(const uint256 &hash, CTransactionRef &txOut,
...
static bool
WriteBlockToDisk(const CBlock &block, CDiskBlockPos &pos,
...
bool ReadBlockFromDisk(CBlock &block, const CBlockIndex *pindex,
  ...
  if (block.GetHash() != pindex->GetBlockHash())
    return error("ReadBlockFromDisk(CBlock&, CBlockIndex*): GetHash() doesn't "
                 "match index for %s at %s",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params &consensusParams) {
  ...
  CAmount nSubsidy = CAmount(nHeight > FORK_BLOCK ? 1 : 50) * COIN;
...
static void CheckForkWarningConditions() {
  ...
  if (pindexBestForkTip || (pindexBestInvalid &&
    ...
    if (pindexBestForkTip && pindexBestForkBase) {
      ...
      LogPrintf("%s: Warning: Found invalid chain at least ~6 blocks longer "
...
void InitScriptExecutionCache() {
  ...
  size_t nMaxCacheSize =
      std::min(
          std::max(
              (int64_t)0,
              gArgs.GetArg("-maxsigcachesize", DEFAULT_MAX_SIG_CACHE_SIZE) / 2),
          MAX_MAX_SIG_CACHE_SIZE) *
      ((size_t)1 << 20);
  size_t nElems = scriptExecutionCache.setup_bytes(nMaxCacheSize);
  LogPrintf("Using %zu MiB out of %zu/2 requested for script execution cache, "
            "able to store %zu elements\n",
            (nElems * sizeof(uint256)) >> 20, (nMaxCacheSize * 2) >> 20,
            nElems);
}

/**
 * Check whether all inputs of this transaction are valid (no double spends,
 * scripts & sigs, amounts)
 * This does not modify the UTXO set.
 *
 * If pvChecks is not nullptr, script checks are pushed onto it instead of being
 * performed inline. Any
 * script checks which are not necessary (eg due to script execution cache hits)
 * are, obviously,
 * not pushed onto pvChecks/run.
 *
 * Setting cacheSigStore/cacheFullScriptStore to false will remove elements from
 * the corresponding cache
 * which are matched. This is useful for checking blocks where we will likely
 * never need the cache
 * entry again.
 *
...
bool CheckInputs(const CTransaction &tx, CValidationState &state,
  ...
  if (!tx.IsCoinBase()) {
    ...
    if (fScriptChecks) {
      ...
      AssertLockHeld(cs_main); // TODO: Remove this requirement by making
                               // CuckooCache not require external locks
      ...
      for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        // Verify signature
        CScriptCheck check(scriptPubKey, amount, tx, i, flags, cacheSigStore,
                           &txdata);
        if (pvChecks) {
...
bool UndoReadFromDisk(CBlockUndo &blockundo, const CDiskBlockPos &pos,
  ...
  CHashVerifier<CAutoFile> verifier(
      &filein); // We need a CHashVerifier as reserializing may lose data
  try {
    verifier << hashBlock;
    verifier >> blockundo;
    filein >> hashChecksum;
...
int ApplyTxInUndo(Coin &&undo, CCoinsViewCache &view, const COutPoint &out) {
  ...
  if (undo.nHeight == 0) {
    ...
    // outputs. This implies that it must be present for some other output of
    // the same tx.
    ...
    if (!alternate.IsSpent()) {
      ...
      return DISCONNECT_FAILED; // adding output for transaction without known
                                // metadata
    }
  }
  // The potential_overwrite parameter to AddCoin is only allowed to be false if
  // we know for
  // sure that the coin did not already exist in the cache. As we have queried
  // for that above
  // using HaveCoin, we don't need to guess. When fClean is false, a coin
  // already existed and
  // it is an overwrite.
...
static DisconnectResult DisconnectBlock(const CBlock &block,
  ...
  if (pindex->nHeight == FORK_BLOCK) {
    // undo premine tx
    CAmount premine_amount = (CAmount)49875000000000;
    CJFKBitcoin1776Address addr;
    addr.Set(CPubKey(ParseHex("020e2d5ad3b8900feda0b7801a503475b09c27bcd41266c9"
                              "dce7ef68b7adfa52ca"))
                 .GetID());
    CScript dest = GetScriptForDestination(addr.Get());

    // add premine tx, which is also a coinbase tx (in other words, the fork
    // block has TWO coinbase transactions)
    CMutableTransaction premine_mtx;
    premine_mtx.nVersion = 2;
    premine_mtx.vin.emplace_back(uint256(), 0);
    premine_mtx.vout.emplace_back(premine_amount, dest);
    CTransaction premine_tx(premine_mtx);
    COutPoint out(premine_tx.GetHash(), 0);
    Coin coin;
    bool is_spent = view.SpendCoin(out, &coin);
    if (!is_spent || premine_tx.vout[0] != coin.out ||
        pindex->nHeight != coin.nHeight || !coin.fCoinBase) {
      // fClean = false; // transaction output mismatch
      fprintf(
          stderr,
          "* premine undo failure! UTXO set is tainted and may need resync\n");
    }
  }
    ...
    for (size_t o = 0; o < tx.vout.size(); o++) {
      if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
        ...
        if (!is_spent || tx.vout[o] != coin.out ||
            pindex->nHeight != coin.nHeight || is_coinbase != coin.fCoinBase) {
          fClean = false; // transaction output mismatch
        }
      }
    }
...
void ThreadScriptCheck() {
  RenameThread("jfkbitcoin1776-scriptch");
...
static unsigned int
GetBlockScriptFlags(const CBlockIndex *pindex,
                    const Consensus::Params &consensusparams) {
  AssertLockHeld(cs_main);
...
static bool ConnectBlock(const CBlock &block, CValidationState &state,
  ...
  bool premine_block = pindex->nHeight == FORK_BLOCK;
  if (premine_block)
    printf("** premine block; will add to UTXO set **\n");
  ...
  if (!hashAssumeValid.IsNull()) {
    // We've been configured with the hash of a block which has been externally
    // verified to have a valid history.
    // A suitable default value is included with the software and updated from
    // time to time.  Because validity
    //  relative to a piece of software is an objective fact these defaults can
    //  be easily reviewed.
    // This setting doesn't force the selection of any particular chain but
    // makes validating some faster by
    ...
    if (it != mapBlockIndex.end()) {
      if (it->second->GetAncestor(pindex->nHeight) == pindex &&
        ...
        // This block is a member of the assumed verified chain and an ancestor
        // of the best header.
        // The equivalent time check discourages hash power from extorting the
        // network via DOS attack
        //  into accepting an invalid block through telling users they must
        //  manually set assumevalid.
        //  Requiring a software change or burying the invalid block, regardless
        //  of the setting, makes
        //  it hard to hide the implication of the demand.  This also avoids
        //  having release candidates
        //  that are hardly doing any signature verification at all in testing
        //  without having to
        //  artificially set the default assumed verified block further back.
        // The test against nMinimumChainWork prevents the skipping when denied
        // access to any chain at
  ...
  // If such overwrites are allowed, coinbases and transactions depending upon
  // those
  // can be duplicated to remove the ability to spend the first instance -- even
  // after
  ...
  // This rule was originally applied to all blocks with a timestamp after March
  // 15, 2012, 0:00 UTC.
  // Now that the whole chain is irreversibly beyond that time it is applied to
  // all blocks except the
  // two in the chain that violate it. This prevents exploiting the issue
  // against nodes during their
  // initial block download.
  bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock
                                                // invocations which don't have
                                                // a hash.
                       !((pindex->nHeight == 91842 &&
                          pindex->GetBlockHash() ==
                              uint256S("0x00000000000a4d0a398161ffc163c503763b1"
                                       "f4360639393e0e4c8e300e0caec")) ||
                         (pindex->nHeight == 91880 &&
                          pindex->GetBlockHash() ==
                              uint256S("0x00000000000743f190a18c5577a3c2d2a1f61"
                                       "0ae9601ac046a38084ccb7cd721")));

  // Once BIP34 activated it was not possible to create new duplicate coinbases
  // and thus other than starting
  // with the 2 existing duplicate coinbase pairs, not possible to create
  // overwriting txs.  But by the
  // time BIP34 activated, in each of the existing pairs the duplicate coinbase
  // had overwritten the first
  // before the first had been spent.  Since those coinbases are sufficiently
  // buried its no longer possible to create further
  // duplicate transactions descending from the known pairs either.
  // If we're on the known chain at height greater than where BIP34 activated,
  // we can save the db accesses needed for the BIP30 check.
  ...
  txdata.reserve(block.vtx.size()); // Required so that pointers to individual
                                    // PrecomputedTransactionData don't get
                                    // invalidated
  for (unsigned int i = 0; i < block.vtx.size(); i++) {
    ...
    if (!tx.IsCoinBase()) {
      ...
      if (!SequenceLocks(tx, nLockTimeFlags, &prevheights, *pindex)) {
        return state.DoS(
            100, error("%s: contains a non-BIP68-final transaction", __func__),
    ...
    if (!tx.IsCoinBase()) {
      ...
      bool
          fCacheResults = fJustCheck; /* Don't cache results if we're actually
                                         connecting blocks (still consult the
                                         cache, though) */
  ...
  if (premine_block) {
    ...
    CMutableTransaction premine_mtx;
    premine_mtx.nVersion = 2;
    premine_mtx.vin.emplace_back(uint256(), (uint32_t)-1);
    premine_mtx.vout.emplace_back(premine_amount, dest);
    CTransaction premine_tx(premine_mtx);
    assert(premine_tx.IsCoinBase());
    CTxUndo undoDummy;
    UpdateCoins(premine_tx, view, undoDummy, pindex->nHeight);
    LogPrintf("Premine transaction ID %s\n",
              premine_tx.GetHash().ToString().c_str());
  }

  int64_t nTime3 = GetTimeMicros();
  nTimeConnect += nTime3 - nTime2;
  LogPrint(BCLog::BENCH, "      - Connect %u transactions: %.2fms (%.3fms/tx, "
                         "%.3fms/txin) [%.2fs]\n",
  ...
  if (!control.Wait())
    return state.DoS(100, error("%s: CheckQueue failed", __func__),
  ...
  try {
    {
      ...
      // The cache is large and we're within 10% and 10 MiB of the limit, but we
      // have time now (not in the middle of a block processing).
      bool fCacheLarge =
      ...
      if (fDoFullFlush || fPeriodicWrite) {
        ...
        FlushBlockFile();
        // Then update all block file information (which may refer to block and
        // undo files).
        {
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams &chainParams) {
  ...
  if (!IsInitialBlockDownload()) {
    ...
    for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
      ...
      if (state == THRESHOLD_ACTIVE || state == THRESHOLD_LOCKED_IN) {
        const std::string strWarning = strprintf(
            _("Warning: unknown new rules activated (versionbit %i)"), bit);
    ...
    if (nUpgraded > 100 / 2) {
      std::string strWarning = _("Warning: Unknown block versions being mined! "
                                 "It's possible unknown rules are in effect");
      // notify GetWarnings(), called by Qt and the JSON-RPC code to warn the
      // user:
      DoWarning(strWarning);
    }
  }
...
static CBlockIndex *FindMostWorkChain() {
  do {
    ...
    // Check whether all blocks on the path between the currently active chain
    // and the candidate are valid.
    // Just going until the active chain is an optimization, as we know all
    // blocks in it are valid already.
...
static bool ActivateBestChainStep(CValidationState &state,
  ...
  while (fContinue && nHeight != pindexMostWork->nHeight) {
    // Don't iterate the entire list of potential improvements toward the best
    // tip, as we likely only need
    ...
    for (CBlockIndex *pindexConnect : reverse_iterate(vpindexToConnect)) {
      if (!ConnectTip(state, chainparams, pindexConnect,
        ...
        if (!pindexOldTip ||
          ...
          // We're in a better position than we were. Return temporarily to
          // release the lock.
          fContinue = false;
          break;
        }
...
bool ActivateBestChain(CValidationState &state, const CChainParams &chainparams,
  ...
  do {
    ...
    bool fInitialDownload;
    {
      LOCK(cs_main);
      ConnectTrace connectTrace(
          mempool); // Destructed before cs_main is unlocked
...
static CBlockIndex *AddToBlockIndex(const CBlockHeader &block) {
  ...
  pindexNew->nTimeMax =
      (pindexNew->pprev ? std::max(pindexNew->pprev->nTimeMax, pindexNew->nTime)
                        : pindexNew->nTime);
...
static bool
ReceivedBlockTransactions(const CBlock &block, CValidationState &state,
  ...
  if (pindexNew->pprev == nullptr || pindexNew->pprev->nChainTx) {
    ...
    // Recursively process any descendant blocks that now may be eligible to be
    // connected.
    while (!queue.empty()) {
...
bool CheckBlock(const CBlock &block, CValidationState &state,
  ...
  if (block.vtx.empty() ||
      ...
      ::GetSerializeSize(block, SER_NETWORK,
  ...
  for (unsigned int i = 1; i < block.vtx.size(); i++)
    if (block.vtx[i]->IsCoinBase())
      return state.DoS(100, false, REJECT_INVALID, "bad-cb-multiple", false,
...
void UpdateUncommittedBlockStructures(
    CBlock &block, const CBlockIndex *pindexPrev,
    const Consensus::Params &consensusParams) {
  int commitpos = GetWitnessCommitmentIndex(block);
  static const std::vector<unsigned char> nonce(32, 0x00);
  if (commitpos != -1 && IsWitnessEnabled(pindexPrev, consensusParams) &&
...
GenerateCoinbaseCommitment(CBlock &block, const CBlockIndex *pindexPrev,
  ...
  if (consensusParams.vDeployments[Consensus::DEPLOYMENT_SEGWIT].nTimeout !=
      0) {
    if (commitpos == -1) {
      uint256 witnessroot = BlockWitnessMerkleRoot(block, nullptr);
      CHash256()
          .Write(witnessroot.begin(), 32)
          .Write(ret.data(), 32)
          .Finalize(witnessroot.begin());
      CTxOut out;
      out.nValue = 0;
      ...
      commitment = std::vector<unsigned char>(out.scriptPubKey.begin(),
...
static bool ContextualCheckBlockHeader(const CBlockHeader &block,
  ...
  if (!fork_conforksus.active && nHeight > FORK_BLOCK)
    return true;
  ...
  if (fCheckpointsEnabled) {
    ...
    if (pcheckpoint && nHeight < pcheckpoint->nHeight)
      return state.DoS(
          100, error("%s: forked chain older than last checkpoint (height %d)",
                     __func__, nHeight),
...
static bool ContextualCheckBlock(const CBlock &block, CValidationState &state,
  ...
  // * We compute the witness hash (which is the hash including witnesses) of
  // all the block's transactions, except the
  //   coinbase (where 0x0000....0000 is used instead).
  // * The coinbase scriptWitness is a stack of a single 32-byte vector,
  // containing a witness nonce (unconstrained).
  // * We build a merkle tree with all those witness hashes as leaves (similar
  // to the hashMerkleRoot in the block header).
  // * There must be at least one output whose scriptPubKey is a single 36-byte
  // push, the first 4 bytes of which are
  //   {0xaa, 0x21, 0xa9, 0xed}, and the following 32 bytes are SHA256^2(witness
  //   root, witness nonce). In case there are
  ...
  if (VersionBitsState(pindexPrev, consensusParams,
    ...
    if (commitpos != -1) {
      ...
      if (block.vtx[0]->vin[0].scriptWitness.stack.size() != 1 ||
        ...
        return state.DoS(
            100, false, REJECT_INVALID, "bad-witness-nonce-size", true,
      ...
      if (memcmp(hashWitness.begin(),
                 &block.vtx[0]->vout[commitpos].scriptPubKey[6], 32)) {
        return state.DoS(
            100, false, REJECT_INVALID, "bad-witness-merkle-match", true,
  ...
  // No witness data is allowed in blocks that don't commit to witness data, as
  // this would otherwise leave room for spam
  if (!fHaveWitness) {
    for (const auto &tx : block.vtx) {
      if (tx->HasWitness()) {
        return state.DoS(
            100, false, REJECT_INVALID, "unexpected-witness", true,
...
static bool AcceptBlock(const std::shared_ptr<const CBlock> &pblock,
  ...
  // good...RELAY NOW
  // (but if it does not build on our best tip, let the SendMessages loop relay
  // it)
  ...
  if (fCheckForPruning)
    FlushStateToDisk(
        chainparams, state,
        FLUSH_STATE_NONE); // we just allocated more disk space for block files
...
bool ProcessNewBlock(const CChainParams &chainparams,
                     ...
                     bool fForceProcessing, bool *fNewBlock) {
  {
    ...
    if (ret) {
      // Store to disk
      ret = AcceptBlock(pblock, state, chainparams, &pindex, fForceProcessing,
                        nullptr, fNewBlock);
    }
    CheckBlockIndex(chainparams.GetConsensus());
    if (!ret) {
 ...
 * The user sets the target (in MB) on the command line or in config file.  This
 * will be run on startup and whenever new
 * space is allocated in a block or undo file, staying below the target.
 * Changing back to unpruned requires a reindex
 * (which in this case means the blockchain must be re-downloaded.)
 *
 * Pruning functions are called from FlushStateToDisk when the global
 * fCheckForPruning flag has been set.
 * Block and undo files are deleted in lock-step (when blk00003.dat is deleted,
 * so is rev00003.dat.)
 * Pruning cannot take place until the longest chain is at least a certain
 * length (100000 on mainnet, 1000 on testnet, 1000 on regtest).
 * Pruning will never delete a block within a defined distance (currently 288)
 * from the active chain's tip.
 * The block index is updated by unsetting HAVE_DATA and HAVE_UNDO for any
 * blocks that were stored in the deleted files.
...
static void FindFilesToPrune(std::set<int> &setFilesToPrune,
  ...
  if (nCurrentUsage + nBuffer >= nPruneTarget) {
    for (int fileNumber = 0; fileNumber < nLastBlockFile; fileNumber++) {
      ...
      // don't prune files that could have a block within MIN_BLOCKS_TO_KEEP of
      // the main chain's tip but keep scanning
  ...
  for (const std::pair<int, CBlockIndex *> &item : vSortedByHeight) {
    ...
    pindex->nTimeMax =
        (pindex->pprev ? std::max(pindex->pprev->nTimeMax, pindex->nTime)
                       : pindex->nTime);
    // We can link the chain of blocks for which we've received transactions at
    // some point.
...
bool CVerifyDB::VerifyDB(const CChainParams &chainparams, CCoinsView *coinsview,
  ...
  for (CBlockIndex *pindex = chainActive.Tip(); pindex && pindex->pprev;
    ...
    int percentageDone = std::max(
        1, std::min(
               99, (int)(((double)(chainActive.Height() - pindex->nHeight)) /
    ...
    // check level 3: check for inconsistencies during memory-only disconnect of
    // tip blocks
    if (nCheckLevel >= 3 && pindex == pindexState &&
      ...
      if (res == DISCONNECT_FAILED) {
        return error("VerifyDB(): *** irrecoverable inconsistency in block "
                     "data at %d, hash=%s",
                     pindex->nHeight, pindex->GetBlockHash().ToString());
      }
  ...
  if (pindexFailure)
    return error("VerifyDB(): *** coin database inconsistencies found (last %i "
                 "blocks, %i good transactions before that)\n",
...
bool ReplayBlocks(const CChainParams &params, CCoinsView *view) {
  ...
  if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating
                                // it's the first flush.
  ...
  while (pindexOld != pindexFork) {
    if (pindexOld->nHeight > 0) { // Never disconnect the genesis block.
      ...
      // If DISCONNECT_UNCLEAN is returned, it means a non-existing UTXO was
      // deleted, or an existing UTXO was
      // overwritten. It corresponds to cases where the block-to-be-disconnect
      // never had all its operations
      // applied to the UTXO set. However, as both writing a UTXO and deleting a
      // UTXO are idempotent operations,
      // the result is still a version of the UTXO set with the effects of that
      // block undone.
    }
...
bool RewindBlockIndex(const CChainParams &params) {
  ...
  // Note that during -reindex-chainstate we are called with an empty
  // chainActive!
  ...
  // lack of data
  // to disk before writing the chainstate, resulting in a failure to continue
  // if interrupted.
  for (BlockMap::iterator it = mapBlockIndex.begin(); it != mapBlockIndex.end();
       it++) {
    ...
    if (IsWitnessEnabled(pindexIter->pprev, params.GetConsensus()) &&
      ...
      // Reduce validity
      pindexIter->nStatus =
          std::min<unsigned int>(pindexIter->nStatus & BLOCK_VALID_MASK,
                                 BLOCK_VALID_TREE) |
      ...
      std::pair<std::multimap<CBlockIndex *, CBlockIndex *>::iterator,
                std::multimap<CBlockIndex *, CBlockIndex *>::iterator>
          ret = mapBlocksUnlinked.equal_range(pindexIter->pprev);
...
bool LoadExternalBlockFile(const CChainParams &chainparams, FILE *fileIn,
  ...
  try {
    ...
    // destructor
    CBufferedFile blkdat(fileIn, 2 * MAX_BLOCK_SERIALIZED_SIZE,
                         MAX_BLOCK_SERIALIZED_SIZE + 8, SER_DISK,
                         CLIENT_VERSION);
    ...
    while (!blkdat.eof()) {
      ...
      try {
        ...
        if (mapBlockIndex.count(hash) == 0 ||
          ...
          LOCK(cs_main);
          CValidationState state;
          if (AcceptBlock(pblock, state, chainparams, nullptr, true, dbp,
                          nullptr))
            nLoaded++;
          if (state.IsError())
            break;
          ...
          while (range.first != range.second) {
            ...
            if (ReadBlockFromDisk(*pblockrecursive, it->second,
              ...
              LogPrint(BCLog::REINDEX,
                       ...
                       pblockrecursive->GetHash().ToString(), head.ToString());
              LOCK(cs_main);
              CValidationState dummy;
              if (AcceptBlock(pblockrecursive, dummy, chainparams, nullptr,
...
void static CheckBlockIndex(const Consensus::Params &consensusParams) {
  ...
  // During a reindex, we read the genesis block and call CheckBlockIndex before
  // ActivateBestChain,
  // so we have the genesis block in mapBlockIndex but no active chain.  (A few
  // of the tests when
  ...
  rangeGenesis.first++;
  assert(rangeGenesis.first ==
         rangeGenesis
             .second); // There is only one index entry with parent nullptr.
  ...
  CBlockIndex *pindexFirstNotTreeValid =
      nullptr; // Oldest ancestor of pindex which does not have BLOCK_VALID_TREE
               // (regardless of being valid or not).
  CBlockIndex *pindexFirstNotTransactionsValid =
      nullptr; // Oldest ancestor of pindex which does not have
               // BLOCK_VALID_TRANSACTIONS (regardless of being valid or not).
  CBlockIndex *pindexFirstNotChainValid =
      nullptr; // Oldest ancestor of pindex which does not have
               // BLOCK_VALID_CHAIN (regardless of being valid or not).
  ...
  while (pindex != nullptr) {
    ...
    if (pindex->pprev == nullptr) {
      ...
      assert(
          pindex->GetBlockHash() ==
          consensusParams.hashGenesisBlock); // Genesis block's hash must match.
      assert(pindex == chainActive.Genesis()); // The current active chain's
                                               // genesis block must be this
                                               // block.
    }
    if (pindex->nChainTx == 0)
      assert(pindex->nSequenceId <= 0); // nSequenceId can't be set positive for
                                        // blocks that aren't linked (negative
                                        // is used for preciousblock)
    // VALID_TRANSACTIONS is equivalent to nTx > 0 for all nodes (whether or not
    // pruning has occurred).
    // HAVE_DATA is only equivalent to nTx > 0 (or VALID_TRANSACTIONS) if no
    // pruning has occurred.
    if (!fHavePruned) {
    ...
    // All parents having had data (at some point) is equivalent to all parents
    // being VALID_TRANSACTIONS, which is equivalent to nChainTx being set.
    assert((pindexFirstNeverProcessed != nullptr) ==
           (pindex->nChainTx == 0)); // nChainTx != 0 is used to signal that all
                                     // parent blocks have been processed (but
    ...
    assert(pindex->pprev == nullptr ||
           pindex->nChainWork >=
               pindex->pprev->nChainWork); // For every block except the genesis
                                           // block, the chainwork must be
                                           // larger than the parent's.
    assert(nHeight < 2 || (pindex->pskip && (pindex->pskip->nHeight <
                                             nHeight))); // The pskip pointer
                                                         // must point back for
                                                         // all but the first 2
                                                         // blocks.
    ...
    if (pindexFirstInvalid == nullptr) {
      ...
      assert((pindex->nStatus & BLOCK_FAILED_MASK) == 0); // The failed mask
                                                          // cannot be set for
                                                          // blocks without
                                                          // invalid parents.
    }
    if (!CBlockIndexWorkComparator()(pindex, chainActive.Tip()) &&
      ...
      if (pindexFirstInvalid == nullptr) {
        ...
        // setBlockIndexCandidates but had to be removed because of the missing
        // data.
        // In this case it must be in mapBlocksUnlinked -- see test below.
      }
    } else { // If this block sorts worse than the current tip or some
    ...
    if (!(pindex->nStatus & BLOCK_HAVE_DATA))
      assert(!foundInUnlinked); // Can't be in mapBlocksUnlinked if we don't
                                // HAVE_DATA
    ...
    if (pindex->pprev && (pindex->nStatus & BLOCK_HAVE_DATA) &&
      ...
      // We HAVE_DATA for this block, have received data for all parents at some
      // point, but we're currently missing data for some parent.
    ...
    // This is a leaf node.
    // Move upwards until we reach a node of which we have not yet visited the
    // last child.
    while (pindex) {
      ...
      while (rangePar.first->second != pindex) {
        assert(rangePar.first != rangePar.second); // Our parent must have at
                                                   // least the node we're
                                                   // coming from as child.
        rangePar.first++;
      }
...
bool LoadMempool(void) {
  ...
  try {
    ...
    while (num--) {
      ...
      if (nTime + nExpiryTimeout > nNow) {
        LOCK(cs_main);
        AcceptToMemoryPoolWithTime(chainparams, mempool, state, tx, true,
                                   nullptr, nTime, nullptr, false, 0);
        if (state.IsValid()) {
          ++count;
        } else {
          ++failed;
  ...
  LogPrintf("Imported mempool transactions from disk: %i successes, %i failed, "
            "%i expired\n",
