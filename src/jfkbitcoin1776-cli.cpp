...
std::string HelpMessageCli() {
  ...
  strUsage +=
      HelpMessageOpt("-rpcclienttimeout=<n>",
                     strprintf(_("Timeout in seconds during HTTP requests, or "
                                 "0 for no timeout. (default: %d)"),
                               DEFAULT_HTTP_CLIENT_TIMEOUT));
  strUsage += HelpMessageOpt("-stdin",
                             _("Read extra arguments from standard input, one "
                               "per line until EOF/Ctrl-D (recommended for "
                               "sensitive information such as passphrases)"));
  strUsage += HelpMessageOpt(
      "-rpcwallet=<walletname>",
      _("Send RPC for non-default wallet on RPC server (argument is wallet "
        "filename in jfkbitcoin1776d directory, required if "
        "jfkbitcoin1776d/-Qt runs with multiple wallets)"));
...
static int AppInitRPC(int argc, char *argv[]) {
  ...
  if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
    ...
    if (!gArgs.IsArgSet("-version")) {
      strUsage +=
          ...
          "  jfkbitcoin1776-cli [options] -named <command> [name=value] ... " +
          strprintf(_("Send command to %s (with named arguments)"),
                    _(PACKAGE_NAME)) +
          "\n" + "  jfkbitcoin1776-cli [options] help                " +
          _("List commands") + "\n" +
          "  jfkbitcoin1776-cli [options] help <command>      " +
  ...
  // Check for -testnet or -regtest parameter (BaseParams() calls are only valid
  // after this clause)
  try {
  ...
  if (gArgs.GetArg("-rpcpassword", "") == "") {
    ...
    if (!GetAuthCookie(&strRPCUserColonPass)) {
      throw std::runtime_error(strprintf(
          _("Could not locate RPC credentials. No authentication cookie could "
            "be found, and no rpcpassword is set in the configuration file "
            "(%s)"),
...
int CommandLineRPC(int argc, char *argv[]) {
  ...
  try {
    ...
    args.erase(
        args.begin()); // Remove trailing method name from arguments vector
          ...
          if (error.isObject()) {
            ...
            if (errCode.isNum() &&
              ...
              strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to "
                          "jfkbitcoin1776-cli command line.";
            }
          }
        } else {
          // Result
