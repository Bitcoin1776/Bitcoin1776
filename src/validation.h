...
#ifndef JFKBITCOIN1776_VALIDATION_H
#define JFKBITCOIN1776_VALIDATION_H
 ...
 * we fetch?
 *  Larger windows tolerate larger download speed differences between peer, but
 * increase the potential
 *  degree of disordering of blocks on disk (which make reindexing and in the
 * future perhaps pruning
 *  harder). We'll probably want to make this a per-peer adaptive value at some
 * point. */
...
// prune in
// full block file chunks, we need the high water mark which triggers the prune
// to be
// one 128MB block file + added 15% undo data = 147MB greater for a total of
// 545MB
// Setting the target to > than 550MB will make it likely we can respect the
// target.
...
/** Get the numerical statistics for the BIP9 state for a given deployment at
 * the current tip. */
 ...
 * active chain.
 * Optionally stores in LockPoints the resulting height and time calculated and
 * the hash
 * of the block needed for calculation or skips the calculation and uses the
 * LockPoints
 * passed in for evaluation.
 * The LockPoints should not be considered valid if CheckSequenceLocks returns
 * false.
 *
 * See consensus/consensus.h for flag definitions.
 */
...
/** Produce the necessary coinbase commitment for a block (modifies the hash,
 * don't call for mined blocks). */
...
#endif // JFKBITCOIN1776_VALIDATION_H
