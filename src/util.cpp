...
const char *const JFKBITCOIN1776_CONF_FILENAME = "jfkbitcoin1776.conf";
const char *const JFKBITCOIN1776_PID_FILENAME = "jfkbitcoin1776d.pid";
...
public:
  CInit() {
    ...
    // OpenSSL can optionally load a config file which lists optional loadable
    // modules and engines.
    // We don't use them so we don't require the config. However some of our
    // libs may call functions
    // which attempt to load the config file, possibly resulting in an exit() or
    // crash if it is missing
    // or corrupt. Explicitly tell OpenSSL not to try to load the file. The
    // result for our libs will be
    // that the config appears to have been loaded and there are no
    // modules/engines available.
    OPENSSL_no_config();
...
#else
  const char *pszModule = "jfkbitcoin1776";
#endif
  if (pex)
...
// Data\JFKBitcoin1776
// Windows >= Vista: C:\Users\Username\AppData\Roaming\JFKBitcoin1776
// Mac: ~/Library/Application Support/JFKBitcoin1776
// Unix: ~/.jfkbitcoin1776
#ifdef WIN32
  // Windows
  return GetSpecialFolderPath(CSIDL_APPDATA) / "JFKBitcoin1776";
#else
  fs::path pathRet;
...
#ifdef MAC_OSX
  // Mac
  return pathRet / "Library/Application Support/JFKBitcoin1776";
#else
  // Unix
  return pathRet / ".jfkbitcoin1776";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string &confPath) {
  ...
  if (!streamConfig.good())
    return; // No jfkbitcoin1776.conf file is OK
...
#ifndef WIN32
fs::path GetPidFile() {
  fs::path pathPidFile(gArgs.GetArg("-pid", JFKBITCOIN1776_PID_FILENAME));
 ...
 * directory exists.
 * Specifically handles case where path p exists, but it wasn't possible for the
 * user to
...
bool TryCreateDirectories(const fs::path &p) {
  ...
  // create_directories didn't create the directory, it had to have existed
  // already
  return false;
}
 ...
 * number.
 * It returns the actual file descriptor limit (which may be more or less than
 * nMinFD)
 */
 ...
 * (corresponding to disk space)
 * it is advisory, and the range specified in the arguments will never contain
 * live data
 */
void AllocateFileRange(FILE *file, unsigned int offset, unsigned int length) {
  ...
  while (length > 0) {
    ...
    if (length < now)
      now = length;
    fwrite(buf, 1, now,
           file); // allowed to fail; this function is advisory anyway
    length -= now;
  }
#endif
}
...
#else // Must fall back to hardware_concurrency, which unfortunately counts
      // virtual cores
...
std::string CopyrightHolders(const std::string &strPrefix) {
  ...
  // Check for untranslated substitution to make sure JFKBitcoin1776 Core
  // copyright is not removed by accident
  if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION)
          .find("JFKBitcoin1776 Core") == std::string::npos) {
