...
static BlockAssembler::Options DefaultOptions(const CChainParams &params) {
  ...
  // If neither -blockmaxsize or -blockmaxweight is given, limit to
  // DEFAULT_BLOCK_MAX_*
...
BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn,
  ...
  if (fork_conforksus.active) {
    coinbaseTx.vin[0].scriptSig =
        CScript() << std::vector<unsigned char>(FORK_HASH_UINT256.begin(),
  ...
  // LogPrintf("CreateNewBlock(): block weight: %u txs: %u fees: %ld sigops
...
void IncrementExtraNonce(CBlock *pblock, const CBlockIndex *pindexPrev,
  ...
  if (fork_conforksus.active) {
    txCoinbase.vin[0].scriptSig =
        (CScript() << std::vector<unsigned char>(FORK_HASH_UINT256.begin(),
                                                 FORK_HASH_UINT256.end())
                   << nHeight << CScriptNum(nExtraNonce)) +
        COINBASE_FLAGS;
  } else {
    txCoinbase.vin[0].scriptSig =
...
// zero.
//
bool static ScanHash(const CBlockHeader *pblock, uint32_t &nNonce,
                     uint256 *phash) {
...
static bool ProcessBlockFound(const std::shared_ptr<const CBlock> pblock,
  ...
  // Found a solution
  {
    ...
    if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
      return error("JFKBitcoin1776Miner: generated block is stale");
  }
  ...
  if (!ProcessNewBlock(chainparams, pblock, true, NULL))
    return error("JFKBitcoin1776Miner: ProcessNewBlock, block not accepted");

  return true;
}

void static JFKBitcoin1776Miner(const CChainParams &chainparams) {
  LogPrintf("JFKBitcoin1776Miner started\n");
  // SetThreadPriority(THREAD_PRIORITY_LOWEST);
  RenameThread("jfkbitcoin1776-miner");
  ...
  try {
    ...
    while (true) {
      ...
      if (chainparams.MiningRequiresPeers()) {
        do {
          if (vNodes.size() > 0 && !IsInitialBlockDownload())
            break;
          MilliSleep(1000);
        } while (true);
      }
      ...
      if (!pblocktemplate.get()) {
        LogPrintf("Error in JFKBitcoin1776Miner: Keypool ran out, please call "
      ...
      // LogPrintf("Running JFKBitcoin1776Miner with %u transactions in block
      // (%u bytes)\n", pblock->vtx.size(),
      ...
      while (true) {
        ...
        if (UpdateTime(pblock.get(), chainparams.GetConsensus(), pindexPrev) <
            0)
    ...
    LogPrintf("JFKBitcoin1776Miner terminated\n");
    throw;
  } catch (const std::runtime_error &e) {
    LogPrintf("JFKBitcoin1776Miner runtime error: %s\n", e.what());
    return;
  }
}

void GenerateJFKBitcoin1776s(bool fGenerate, int nThreads,
                             const CChainParams &chainparams) {
  ...
  for (int i = 0; i < nThreads; i++)
    minerThreads->create_thread(
        boost::bind(&JFKBitcoin1776Miner, boost::cref(chainparams)));
}
