...
static IntrRecvError InterruptibleRecv(uint8_t *data, size_t len, int timeout,
  ...
  while (len > 0 && curTime < endTime) {
    ssize_t ret = recv(hSocket, (char *)data, len,
...
static bool Socks5(const std::string &strDest, int port,
  ...
  if (pchRet1[1] == SOCKS5Method::USER_PASS && auth) {
    ...
    uint8_t pchRetA[2];
    if ((recvr = InterruptibleRecv(pchRetA, 2, SOCKS5_RECV_TIMEOUT, hSocket)) !=
        IntrRecvError::OK) {
      CloseSocket(hSocket);
  ...
  ret =
      send(hSocket, (const char *)vSocks5.data(), vSocks5.size(), MSG_NOSIGNAL);
...
bool ConnectSocket(const CService &addrDest, SOCKET &hSocketRet, int nTimeout,
  ...
  if (GetProxy(addrDest.GetNetwork(), proxy))
    return ConnectThroughProxy(proxy, addrDest.ToStringIP(), addrDest.GetPort(),
...
bool LookupSubNet(const char *pszName, CSubNet &ret) {
  ...
  if (LookupHost(strAddress.c_str(), vIP, 1, false)) {
    ...
    if (slash != strSubnet.npos) {
      ...
      int32_t n;
      // IPv4 addresses start at offset 12, and first 12 bytes must match, so
      // just offset n
...
void InterruptSocks5(bool interrupt) { interruptSocks5Recv = interrupt; }
