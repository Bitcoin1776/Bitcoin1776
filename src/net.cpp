...
static const uint64_t RANDOMIZER_ID_NETGROUP =
    0x6c0edd8036ef4036ULL; // SHA256("netgroup")[0:8]
static const uint64_t RANDOMIZER_ID_LOCALHOSTNONCE =
    0xd93e69e2bbfa5735ULL; // SHA256("localhostnonce")[0:8]
//
...
CNode *CConnman::ConnectNode(CAddress addrConnect, const char *pszDest,
  ...
  if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest,
    ...
    if (pszDest && addrConnect.IsValid()) {
      // It is possible that we already have a connection to the IP/port pszDest
      // resolved to.
      // In that case, drop the connection that was just created, and return the
      // existing CNode instead.
      // Also store the name we used to connect in that CNode, so that future
      // FindNode() calls to that
    ...
    // If connecting to the node failed, and failure is not caused by a problem
    // connecting to
...
void CNode::copyStats(CNodeStats &stats) {
  ...
  // Merely reporting pingtime might fool the caller into thinking the node was
  // still responsive,
  // since pingtime does not update until the ping is complete, which might take
  // a while.
  ...
  // (JFKBitcoin1776 users should be well used to small numbers with many
  // decimal places by now :)
...
void CNode::SetSendVersion(int nVersionIn) {
  ...
  if (nSendVersion != 0) {
    error("Send version already set for node: %i. Refusing to change from %i "
          "to %i",
...
size_t CConnman::SocketSendData(CNode *pnode) const {
  ...
  while (it != pnode->vSendMsg.end()) {
    ...
    int nBytes = 0;
    {
      ...
      nBytes =
          send(pnode->hSocket,
               reinterpret_cast<const char *>(data.data()) + pnode->nSendOffset,
...
bool CConnman::AttemptToEvictConnection() {
  ...
  // longest.
  // This replicates the non-eviction implicit behavior, and precludes attacks
  // that start later.
  std::sort(vEvictionCandidates.begin(), vEvictionCandidates.end(),
  ...
  for (const NodeEvictionCandidate &node : vEvictionCandidates) {
    ...
    if (groupsize > nMostConnections ||
        (groupsize == nMostConnections && grouptime > nMostConnectionsTime)) {
  ...
  while (!interruptNet) {
...

    {
      ...
      for (CNode *pnode : vNodes) {
        ...
        //   needlessly queueing received data, if the remote peer is not
        //   themselves
        //   receiving data. This means properly utilizing TCP flow control
        //   signalling.
        // * Otherwise, if there is space left in the receive buffer, select()
        // for
        //   receiving data.
...
void ThreadMapPort() {
  ...
  if (r == 1) {
    ...
    std::string strDesc = "JFKBitcoin1776 " + FormatFullVersion();
  ...
  int nOutbound = 0;
  {
    ...
    for (CNode *pnode : vNodes) {
      if (!pnode->fInbound && !pnode->m_manual_connection && !pnode->fFeeler &&
          !pnode->fDisconnect && !pnode->fOneShot &&
          pnode->fSuccessfullyConnected) {
        ++nOutbound;
      }
    }
...
void CConnman::ThreadOpenConnections() {
  ...
  while (!interruptNet) {
    ...
    // this check.
    if (!fork_conforksus.active && addrman.size() == 0 &&
        (GetTime() - nStart > 60)) {
    ...
    std::set<std::vector<unsigned char>> setConnected;
    {
      ...
      for (CNode *pnode : vNodes) {
        if (!pnode->fInbound && !pnode->m_manual_connection) {
          ...
          // Netgroups for inbound and addnode peers are not excluded because
          // our goal here
          // is to not use multiple of our limited outbound slots on a single
          // netgroup
          // but inbound and addnode peers do not use our outbound slots.
          // Inbound peers
          // also have the added issue that they're attacker controlled and
          // could be used
          // to prevent us from connecting to particular hosts if we used them
          // here.
    ...
    //
    // Method:
    //  * Choose a random address from new and attempt to connect to it if we
    //  can connect
    //    successfully it is added to tried.
    //  * Start attempting feeler connections only after node finishes making
    //  outbound
    //    connections.
    ...
    while (!interruptNet) {
      ...
      // If we didn't find an appropriate destination after trying 100 addresses
      // fetched from addrman,
      // stop this loop, and let the outer loop run again (which sleeps, adds
      // seed nodes, recalculates
      ...
      // and only if less than half the outbound are up.
      ...
      // regardless of the services assumed to be available, only require the
      // minimum if half or more outbound have relevant services
...
std::vector<AddedNodeInfo> CConnman::GetAddedNodeInfo() {
  ...
  std::map<std::string, std::pair<bool, CService>> mapConnectedByName;
  {
    ...
    for (const CNode *pnode : vNodes) {
      ...
      if (!addrName.empty()) {
        mapConnectedByName[std::move(addrName)] = std::make_pair(
            pnode->fInbound, static_cast<const CService &>(pnode->addr));
      }
    }
  }
...
void CConnman::ThreadOpenAddedConnections() {
  ...
  while (true) {
    ...
    for (const AddedNodeInfo &info : vInfo) {
      if (!info.fConnected) {
        if (!grant.TryAcquire()) {
          // If we've used up our semaphore and need a new one, lets not wait
          // here since while we are waiting
        ...
        OpenNetworkConnection(CAddress(service, NODE_NONE), false, &grant,
                              info.strAddedNode.c_str(), false, false, true);
        if (!interruptNet.sleep_for(std::chrono::milliseconds(500)))
          return;
      }
    }
...
bool CConnman::BindListenPort(const CService &addrBind, std::string &strError,
  ...
  if (::bind(hListenSocket, (struct sockaddr *)&sockaddr, len) ==
    ...
    if (nErr == WSAEADDRINUSE)
      strError = strprintf(_("Unable to bind to %s on this computer. %s is "
                             "probably already running."),
                           addrBind.ToString(), _(PACKAGE_NAME));
    else
      strError = strprintf(
          _("Unable to bind to %s on this computer (bind returned error %s)"),
  ...
  if (listen(hListenSocket, SOMAXCONN) == SOCKET_ERROR) {
    strError = strprintf(_("Error: Listening for incoming connections failed "
...
bool CConnman::Bind(const CService &addr, unsigned int flags) {
  ...
  if (!BindListenPort(addr, strError, (flags & BF_WHITELIST) != 0)) {
    if ((flags & BF_REPORT_ERROR) && clientInterface) {
      clientInterface->ThreadSafeMessageBox(strError, "",
...
void CConnman::InitiatedConsensusChange() {
  ...
  // rebind ports unless set
  if (gArgs.GetArgs("-port").size() == 0) {
...
bool CConnman::Start(CScheduler &scheduler, const Options &connOptions) {
  ...
  int64_t nStart = GetTimeMillis();
  {
    ...
    if (adb.Read(addrman))
      ...
      addrman.Clear(); // Addrman can be in an inconsistent state after failure,
                       // reset it
  ...
  if (semOutbound == nullptr) {
    ...
    semOutbound =
        new CSemaphore(std::min((nMaxOutbound + nMaxFeeler), nMaxConnections));
  }
  ...
  // Process messages
  threadMessageHandler = std::thread(
      &TraceThread<std::function<void()>>, "msghand",
...
void CNode::AskFor(const CInv &inv) {
  if (mapAskFor.size() > MAPASKFOR_MAX_SZ ||
    ...
    return;
  // a peer may not have multiple non-responded queue positions for a single inv
  // item
  ...
  LogPrint(BCLog::NET, "askfor %s  %d (%s) peer=%d\n", inv.ToString(),
           nRequestTime, DateTimeStrFormat("%H:%M:%S", nRequestTime / 1000000),
           id);
