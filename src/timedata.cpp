...
#define JFKBITCOIN1776_TIMEDATA_MAX_SAMPLES 200

void AddTimeData(const CNetAddr &ip, int64_t nOffsetSample) {
  LOCK(cs_nTimeOffset);
  // Ignore duplicates
  static std::set<CNetAddr> setKnown;
  if (setKnown.size() == JFKBITCOIN1776_TIMEDATA_MAX_SAMPLES)
    return;
  if (!setKnown.insert(ip).second)
    return;

  // Add data
  static CMedianFilter<int64_t> vTimeOffsets(
      JFKBITCOIN1776_TIMEDATA_MAX_SAMPLES, 0);
  ...
  if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1) {
    ...
    if (abs64(nMedian) <=
        std::max<int64_t>(
            0,
            gArgs.GetArg("-maxtimeadjustment", DEFAULT_MAX_TIME_ADJUSTMENT))) {
      nTimeOffset = nMedian;
    } else {
      nTimeOffset = 0;
