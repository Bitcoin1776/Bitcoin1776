...
#ifndef JFKBITCOIN1776_LIMITEDMAP_H
#define JFKBITCOIN1776_LIMITEDMAP_H

#include <assert.h>
#include <map>

/** STL-like map container that only keeps the N elements with the highest
 * value. */
...
public:
  ...
  void erase(const key_type &k) {
    ...
    if (itTarget == map.end())
      return;
    std::pair<rmap_iterator, rmap_iterator> itPair =
  ...
  void update(const_iterator itIn, const mapped_type &v) {
    // Using map::erase() with empty range instead of map::find() to get a
    // non-const iterator,
...
#endif // JFKBITCOIN1776_LIMITEDMAP_H
