...
bool EnsureWalletIsAvailable(CWallet *const pwallet, bool avoidException) {
  ...
  if (::vpwallets.empty()) {
    ...
    throw JSONRPCError(RPC_METHOD_NOT_FOUND, "Method not found (wallet method "
                                             "is disabled because no wallet is "
                                             "loaded)");
  }
...
UniValue getnewaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "\nReturns a new JFKBitcoin1776 address for receiving payments.\n"
        ...
        "for the address to be linked to. If not provided, the default account "
        "\"\" is used. It can also be set to the empty string \"\" to "
        ...
        "\nResult:\n"
        "\"address\"    (string) The new jfkbitcoin1776 address\n"
        "\nExamples:\n" +
  ...
  return CJFKBitcoin1776Address(keyID).ToString();
}

CJFKBitcoin1776Address GetAccountAddress(CWallet *const pwallet,
                                         std::string strAccount,
  ...
  return CJFKBitcoin1776Address(pubKey.GetID());
}
...
UniValue getaccountaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nDEPRECATED. Returns the current JFKBitcoin1776 address for "
        "receiving payments to this account.\n"
        "\nArguments:\n"
        "1. \"account\"       (string, required) The account name for the "
        "address. It can also be set to the empty string \"\" to represent the "
        "default account. The account does not need to exist, it will be "
        "created and a new address created  if there is no account by the "
        "given name.\n"
        "\nResult:\n"
        "\"address\"          (string) The account jfkbitcoin1776 address\n"
        "\nExamples:\n" +
...
UniValue getrawchangeaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 0)
    throw std::runtime_error(
        "getrawchangeaddress\n"
        "\nReturns a new JFKBitcoin1776 address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"         (string, required) The jfkbitcoin1776 address "
        ...
        HelpExampleRpc("setaccount",
                       "\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\", \"tabby\""));

  LOCK2(cs_main, pwallet->cs_wallet);

  CJFKBitcoin1776Address address(request.params[0].get_str());
  if (!address.IsValid())
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid JFKBitcoin1776 address");
...
UniValue getaccount(const JSONRPCRequest &request) {
  ...
  std::map<CTxDestination, CAddressBookData>::iterator mi =
      pwallet->mapAddressBook.find(address.Get());
...
UniValue getaddressesbyaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "  \"address\"         (string) a jfkbitcoin1776 address associated "
  ...
  for (const std::pair<CJFKBitcoin1776Address, CAddressBookData> &item :
       pwallet->mapAddressBook) {
    const CJFKBitcoin1776Address &address = item.first;
...
UniValue sendtoaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"            (string, required) The jfkbitcoin1776 "
        "address to send to.\n"
        "2. \"amount\"             (numeric or string, required) The amount "
        "in " +
        CURRENCY_UNIT +
        " to send. eg 0.1\n"
        "3. \"comment\"            (string, optional) A comment used to store "
        "what the transaction is for. \n"
        "                             This is not part of the transaction, "
        "just kept in your wallet.\n"
        "4. \"comment_to\"         (string, optional) A comment to store the "
        "name of the person or organization \n"
        "                             to which you're sending the transaction. "
        "This is not part of the \n"
        "                             transaction, just kept in your wallet.\n"
        "5. subtractfeefromamount  (boolean, optional, default=false) The fee "
        "will be deducted from the amount being sent.\n"
        "                             The recipient will receive less "
        "jfkbitcoin1776s than you enter in the amount field.\n"
        "6. replaceable            (boolean, optional) Allow this transaction "
        "to be replaced by a transaction with higher fees via BIP 125\n"
        "7. conf_target            (numeric, optional) Confirmation target (in "
        "blocks)\n"
        ...
        HelpExampleRpc("sendtoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\""
                                        ", 0.1, \"donation\", \"seans "
                                        "outpost\""));
...
UniValue listaddressgroupings(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "  [\n"
        "    [\n"
        "      \"address\",            (string) The jfkbitcoin1776 address\n"
  ...
  for (std::set<CTxDestination> grouping : pwallet->GetAddressGroupings()) {
    ...
    for (CTxDestination address : grouping) {
      ...
      addressInfo.push_back(CJFKBitcoin1776Address(address).ToString());
      addressInfo.push_back(ValueFromAmount(balances[address]));
      {
        if (pwallet->mapAddressBook.find(
                CJFKBitcoin1776Address(address).Get()) !=
            pwallet->mapAddressBook.end()) {
          addressInfo.push_back(pwallet->mapAddressBook
                                    .find(CJFKBitcoin1776Address(address).Get())
                                    ->second.name);
        }
      }
...
UniValue signmessage(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 2)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"         (string, required) The jfkbitcoin1776 address "
        "to use for the private key.\n"
        "2. \"message\"         (string, required) The message to create a "
        "signature of.\n"
        "\nResult:\n"
        "\"signature\"          (string) The signature of the message encoded "
        "in base 64\n"
        "\nExamples:\n"
        ...
        HelpExampleCli("verifymessage", "\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\""
  ...
  CJFKBitcoin1776Address addr(strAddress);
  if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const JSONRPCRequest &request) {
  ...
  // JFKBitcoin1776 address
  CJFKBitcoin1776Address address =
      CJFKBitcoin1776Address(request.params[0].get_str());
  if (!address.IsValid())
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid JFKBitcoin1776 address");
...
UniValue getreceivedbyaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"account\"      (string, required) The selected account, may be "
        ...
        "\nAmount received at the tabby account including unconfirmed amounts "
...
UniValue getbalance(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 3)
    throw std::runtime_error(
        ...
        "account.\n"
        "Note that the account \"\" is not the same as leaving the parameter "
        "out.\n"
        ...
        "                     When this option is specified, it calculates the "
        "balance in a different\n"
        "                     way than when it is not specified, and which can "
        "count spends twice when\n"
        "                     there are conflicting pending transactions (such "
        "as those created by\n"
        "                     the bumpfee command), temporarily resulting in "
        "low or even negative\n"
        "                     balances. In general, account balance "
        "calculation is not considered\n"
        "                     reliable and has resulted in confusing outcomes, "
...
UniValue movecmd(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 3 || request.params.size() > 5)
    throw std::runtime_error(
        ...
        "\nDEPRECATED. Move a specified amount from one account in your wallet "
        "to another.\n"
        "\nArguments:\n"
        ...
        CURRENCY_UNIT + " to move between accounts.\n"
                        "4. (dummy)           (numeric, optional) Ignored. "
                        "Remains for backward compatibility.\n"
                        "5. \"comment\"       (string, optional) An optional "
                        "comment, stored in the wallet only.\n"
                        "\nResult:\n"
...
UniValue sendfrom(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
    throw std::runtime_error(
        "sendfrom \"fromaccount\" \"toaddress\" amount ( minconf \"comment\" "
        "\"comment_to\" )\n"
        "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a "
        "jfkbitcoin1776 address." +
        HelpRequiringPassphrase(pwallet) +
        "\n"
        "\nArguments:\n"
        "1. \"fromaccount\"       (string, required) The name of the account "
        "to send funds from. May be the default account using \"\".\n"
        "                       Specifying an account does not influence coin "
        "selection, but it does associate the newly created\n"
        "                       transaction with the account, so the account's "
        "balance computation and transaction history can reflect\n"
        "                       the spend.\n"
        "2. \"toaddress\"         (string, required) The jfkbitcoin1776 "
        ...
        "5. \"comment\"           (string, optional) A comment used to store "
        "what the transaction is for. \n"
        "                                     This is not part of the "
        "transaction, just kept in your wallet.\n"
        "6. \"comment_to\"        (string, optional) An optional comment to "
        "store the name of the person or organization \n"
        ...
        "\nExamples:\n"
        "\nSend 0.01 " +
        CURRENCY_UNIT + " from the default account to the address, must have "
                        "at least 1 confirmation\n" +
        HelpExampleCli("sendfrom",
                       "\"\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.01") +
        "\nSend 0.01 from the tabby account to the given address, funds must "
        "have at least 6 confirmations\n" +
        HelpExampleCli("sendfrom", "\"tabby\" "
                                   "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" "
                                   "0.01 6 \"donation\" \"seans outpost\"") +
        "\nAs a json rpc call\n" +
        HelpExampleRpc("sendfrom", "\"tabby\", "
                                   "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", "
                                   "0.01, 6, \"donation\", \"seans outpost\""));
  ...
  std::string strAccount = AccountFromValue(request.params[0]);
  CJFKBitcoin1776Address address(request.params[1].get_str());
  if (!address.IsValid())
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid JFKBitcoin1776 address");
  CAmount nAmount = AmountFromValue(request.params[2]);
...
UniValue sendmany(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "\"comment\" [\"address\",...] replaceable conf_target "
        "\"estimate_mode\")\n"
        ...
        "\n"
        "\nArguments:\n"
        "1. \"fromaccount\"         (string, required) DEPRECATED. The account "
        "to send the funds from. Should be \"\" for the default account\n"
        "2. \"amounts\"             (string, required) A json object with "
        "addresses and amounts\n"
        "    {\n"
        "      \"address\":amount   (numeric or string) The jfkbitcoin1776 "
        "address is the key, the numeric amount (can be string) in " +
        CURRENCY_UNIT +
        " is the value\n"
        "      ,...\n"
        "    }\n"
        "3. minconf                 (numeric, optional, default=1) Only use "
        "the balance confirmed at least this many times.\n"
        "4. \"comment\"             (string, optional) A comment\n"
        "5. subtractfeefrom         (array, optional) A json array with "
        "addresses.\n"
        "                           The fee will be equally deducted from the "
        "amount of each selected address.\n"
        "                           Those recipients will receive less "
        "jfkbitcoin1776s than you enter in their corresponding amount field.\n"
        "                           If no addresses are specified here, the "
        "sender pays the fee.\n"
        "    [\n"
        ...
        HelpExampleCli("sendmany",
                       "\"\" "
                       "\"{\\\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\\\":0.01,"
                       "\\\"1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\" 6 "
                       "\"testing\"") +
  ...
  std::set<CJFKBitcoin1776Address> setAddress;
  std::vector<CRecipient> vecSend;

  CAmount totalAmount = 0;
  std::vector<std::string> keys = sendTo.getKeys();
  for (const std::string &name_ : keys) {
    CJFKBitcoin1776Address address(name_);
    if (!address.IsValid())
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                         std::string("Invalid JFKBitcoin1776 address: ") +
                             name_);
  ...
  bool fCreated =
      pwallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired,
                                 nChangePosRet, strFailReason, coin_control);
  if (!fCreated)
...
extern CScript _createmultisig_redeemScript(CWallet *const pwallet,
...
UniValue addmultisigaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 3) {
    std::string msg =
        ...
        "Each key is a JFKBitcoin1776 address or hex-encoded public key.\n"
        ...
        "2. \"keys\"         (string, required) A json array of jfkbitcoin1776 "
        "addresses or hex-encoded public keys\n"
        "     [\n"
        "       \"address\"  (string) jfkbitcoin1776 address or hex-encoded "
        "public key\n"
        "       ...,\n"
        "     ]\n"
        ...
        "\nResult:\n"
        "\"address\"         (string) A jfkbitcoin1776 address associated with "
        "the keys.\n"
  ...
  return CJFKBitcoin1776Address(innerID).ToString();
}
...
public:
  ...
  bool operator()(const CKeyID &keyID) {
    if (pwallet) {
      ...
      // solved for and signed by us
      // if we were to have the private keys. This is just to make sure that the
      // script is valid and that,
      // if found in a transaction, we would still accept and relay that
      // transcation.
      if (!ProduceSignature(DummySignatureCreator(pwallet), witscript, sigs) ||
          !VerifyScript(sigs.scriptSig, witscript, &sigs.scriptWitness,
                        MANDATORY_SCRIPT_VERIFY_FLAGS |
                            SCRIPT_VERIFY_WITNESS_PUBKEYTYPE,
                        DummySignatureCreator(pwallet).Checker())) {
        return false;
      }
...
UniValue addwitnessaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 1) {
    std::string msg = "addwitnessaddress \"address\"\n"
                      "\nAdd a witness address for a script (with pubkey or "
                      "redeemscript known). Requires a new wallet backup.\n"
                      ...
                      "\nResult:\n"
                      "\"witnessaddress\",  (string) The value of the new "
                      "address (P2SH of witness script).\n"
                      "}\n";
    throw std::runtime_error(msg);
  }

  {
    ...
    if (!IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()) &&
        !gArgs.GetBoolArg("-walletprematurewitness", false)) {
  ...
  if (!ret) {
    throw JSONRPCError(RPC_WALLET_ERROR, "Public key or redeemscript not known "
                                         "to wallet, or the key is "
                                         "uncompressed");
  }

  pwallet->SetAddressBook(w.result, "", "receive");

  return CJFKBitcoin1776Address(w.result).ToString();
}
...
UniValue ListReceived(CWallet *const pwallet, const UniValue &params,
                      bool fByAccounts) {
  // Minimum confirmations
  int nMinDepth = 1;
  if (!params[0].isNull())
  ...
  // Tally
  std::map<CJFKBitcoin1776Address, tallyitem> mapTally;
  for (const std::pair<uint256, CWalletTx> &pairWtx : pwallet->mapWallet) {
    const CWalletTx &wtx = pairWtx.second;
  ...
  for (const std::pair<CJFKBitcoin1776Address, CAddressBookData> &item :
    ...
    std::map<CJFKBitcoin1776Address, tallyitem>::iterator it =
  ...
  if (request.fHelp || request.params.size() > 3)
    throw std::runtime_error(
        ...
        CURRENCY_UNIT + " received by the address\n"
                        ...
                        "    \"label\" : \"label\",               (string) A "
                        "comment for the address/transaction, if any\n"
                        "    \"txids\": [\n"
                        "       n,                                (numeric) "
...
UniValue listreceivedbyaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 3)
    throw std::runtime_error(
        ...
        "[\n"
        "  {\n"
        "    \"involvesWatchonly\" : true,   (bool) Only returned if imported "
        "addresses were involved in transaction\n"
        "    \"account\" : \"accountname\",  (string) The account name of the "
        "receiving account\n"
        "    \"amount\" : x.xxx,             (numeric) The total amount "
...
UniValue listtransactions(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "                                                It will be \"\" for "
        "the default account.\n"
        "    \"address\":\"address\",    (string) The jfkbitcoin1776 address "
        "of the transaction. Not present for \n"
        "                                                move transactions "
        "(category = move).\n"
        "    \"category\":\"send|receive|move\", (string) The transaction "
        "category. 'move' is a local (off blockchain)\n"
        "                                                transaction between "
        "accounts, and not associated with an address,\n"
        "                                                transaction id or "
        ...
        "outbound. It is positive for the 'receive' category,\n"
        "                                         and for the 'move' category "
        "for inbound funds.\n"
        "    \"label\": \"label\",       (string) A comment for the "
        "address/transaction, if any\n"
        ...
        "for the transaction. Available for 'send' and \n"
        "                                         'receive' category of "
        ...
        "    \"blockhash\": \"hashvalue\", (string) The block hash containing "
        "the transaction. Available for 'send' and 'receive'\n"
        "                                          category of transactions.\n"
        "    \"blockindex\": n,          (numeric) The index of the "
        "transaction in the block that includes it. Available for 'send' and "
        "'receive'\n"
        "                                          category of transactions.\n"
        "    \"blocktime\": xxx,         (numeric) The block time in seconds "
        ...
        "    \"otheraccount\": \"accountname\",  (string) DEPRECATED. For the "
        "'move' category of transactions, the account the funds came \n"
        "                                          from (for receiving funds, "
        "positive amounts), or went to (for sending funds,\n"
        "                                          negative amounts).\n"
        "    \"bip125-replaceable\": \"yes|no|unknown\",  (string) Whether "
        "this transaction could be replaced due to BIP125 (replace-by-fee);\n"
        "                                                     may be unknown "
        "for unconfirmed transactions not in the mempool\n"
        "    \"abandoned\": xxx          (bool) 'true' if the transaction has "
        "been abandoned (inputs are respendable). Only available for the \n"
        "                                         'send' category of "
        "transactions.\n"
        "  }\n"
        "]\n"
...
UniValue listaccounts(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "\nDEPRECATED. Returns Object that has account names as keys, account "
        "balances as values.\n"
        "\nArguments:\n"
        ...
        "\nResult:\n"
        "{                      (json object where keys are account names, and "
        "values are numeric balances\n"
  ...
  for (const std::pair<uint256, CWalletTx> &pairWtx : pwallet->mapWallet) {
    ...
    if (nDepth >= nMinDepth) {
      for (const COutputEntry &r : listReceived)
        if (pwallet->mapAddressBook.count(r.destination)) {
          mapAccountBalances[pwallet->mapAddressBook[r.destination].name] +=
              r.amount;
        } else
...
UniValue listsinceblock(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "transactions since\n"
        "2. target_confirmations:    (numeric, optional, default=1) Return the "
        "nth block hash from the main chain. e.g. 1 would mean the best block "
        "hash. Note: this is not used as a filter, but only affects "
        "[lastblock] in the return value\n"
        "3. include_watchonly:       (bool, optional, default=false) Include "
        "transactions to watch-only addresses (see 'importaddress')\n"
        ...
        "  \"transactions\": [\n"
        "    \"account\":\"accountname\",       (string) DEPRECATED. The "
        "account name associated with the transaction. Will be \"\" for the "
        "default account.\n"
        "    \"address\":\"address\",    (string) The jfkbitcoin1776 address "
        "of the transaction. Not present for move transactions (category = "
        "move).\n"
        ...
        "category for moves \n"
        "                                          outbound. It is positive "
        "for the 'receive' category, and for the 'move' category for inbound "
        "funds.\n"
        "    \"vout\" : n,               (numeric) the vout value\n"
        "    \"fee\": x.xxx,             (numeric) The amount of the fee in " +
        CURRENCY_UNIT +
        ...
        "transactions.\n"
        "    \"confirmations\": n,       (numeric) The number of confirmations "
        "for the transaction. Available for 'send' and 'receive' category of "
        "transactions.\n"
        "                                          When it's < 0, it means the "
        ...
        "this transaction could be replaced due to BIP125 (replace-by-fee);\n"
        "                                                   may be unknown for "
        "unconfirmed transactions not in the mempool\n"
        "    \"abandoned\": xxx,         (bool) 'true' if the transaction has "
        "been abandoned (inputs are respendable). Only available for the "
        ...
        "appear as-is in this array, and may thus have a positive confirmation "
        "count.\n"
        "  ],\n"
        "  \"lastblock\": \"lastblockhash\"     (string) The hash of the block "
        "(target_confirmations-1) from the best block on the main chain. This "
        "is typically used to feed back into listsinceblock the next time you "
        "call it. So you would generally use a target_confirmations of say 6, "
        "so you will be continually re-notified of transactions until they've "
        "reached 6 confirmations plus any new ones\n"
        "}\n"
        "\nExamples:\n" +
        HelpExampleCli("listsinceblock", "") +
        HelpExampleCli("listsinceblock", "\"000000000000000bacf66f7497b7dc45ef7"
                                         "53ee9a7d38571037cdb1a57f663ad\" 6") +
        HelpExampleRpc("listsinceblock", "\"000000000000000bacf66f7497b7dc45ef7"
                                         "53ee9a7d38571037cdb1a57f663ad\", 6"));

  LOCK2(cs_main, pwallet->cs_wallet);

  const CBlockIndex *pindex = nullptr; // Block index of the specified block or
                                       // the common ancestor, if the block
                                       // provided was in a deactivated chain.
  const CBlockIndex *paltindex = nullptr; // Block index of the specified block,
                                          // even if it's in a deactivated
                                          // chain.
  ...
  while (include_removed && paltindex && paltindex != pindex) {
    ...
    for (const CTransactionRef &tx : block.vtx) {
      if (pwallet->mapWallet.count(tx->GetHash()) > 0) {
        // We want all transactions regardless of confirmation count to appear
        // here,
        // even negative confirmation ones, hence the big negative.
        ListTransactions(pwallet, pwallet->mapWallet[tx->GetHash()], "*",
...
UniValue gettransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "2. \"include_watchonly\"     (bool, optional, default=false) Whether "
        "to include watch-only addresses in balance calculation and details[]\n"
        "\nResult:\n"
        "{\n"
        ...
        "  \"blockindex\" : xx,       (numeric) The index of the transaction "
        ...
        "  \"timereceived\" : ttt,    (numeric) The time received in seconds "
        "since epoch (1 Jan 1970 GMT)\n"
        "  \"bip125-replaceable\": \"yes|no|unknown\",  (string) Whether this "
        "transaction could be replaced due to BIP125 (replace-by-fee);\n"
        "                                                   may be unknown for "
        "unconfirmed transactions not in the mempool\n"
        "  \"details\" : [\n"
        "    {\n"
        "      \"account\" : \"accountname\",      (string) DEPRECATED. The "
        "account name involved in the transaction, can be \"\" for the default "
        "account.\n"
        "      \"address\" : \"address\",          (string) The jfkbitcoin1776 "
        ...
        "\nExamples:\n" +
        HelpExampleCli("gettransaction", "\"1075db55d416d3ca199f55b6084e2115b93"
                                         "45e16c5cf302fc80e9d5fbf5d48d\"") +
        HelpExampleCli("gettransaction", "\"1075db55d416d3ca199f55b6084e2115b93"
                                         "45e16c5cf302fc80e9d5fbf5d48d\" "
                                         "true") +
        HelpExampleRpc("gettransaction", "\"1075db55d416d3ca199f55b6084e2115b93"
                                         "45e16c5cf302fc80e9d5fbf5d48d\""));
...
UniValue abandontransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nResult:\n"
        "\nExamples:\n" +
        HelpExampleCli("abandontransaction", "\"1075db55d416d3ca199f55b6084e211"
                                             "5b9345e16c5cf302fc80e9d5fbf5d48d"
                                             "\"") +
        HelpExampleRpc("abandontransaction", "\"1075db55d416d3ca199f55b6084e211"
                                             "5b9345e16c5cf302fc80e9d5fbf5d48d"
                                             "\""));
...
UniValue walletpassphrase(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 2) {
    throw std::runtime_error(
        ...
        "keys such as sending jfkbitcoin1776s\n"
        "\nArguments:\n"
  ...
  // Note that the walletpassphrase is stored in request.params[0] which is not
  // mlock()ed
...
UniValue walletlock(const JSONRPCRequest &request) {
  ...
  if (request.fHelp)
    return true;
  if (!pwallet->IsCrypted()) {
    throw JSONRPCError(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an "
                                                   "unencrypted wallet, but "
...
UniValue encryptwallet(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1) {
    throw std::runtime_error(
        ...
        "\nNow set the passphrase to use the wallet, such as for signing or "
        "sending jfkbitcoin1776\n" +
  ...
  StartShutdown();
  return "wallet encrypted; JFKBitcoin1776 server stopping, restart to run "
         "with encrypted wallet. The keypool has been flushed and a new HD "
         "seed was generated (if you are using HD). You need to make a new "
         "backup.";
}
...
UniValue lockunspent(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "selection, when spending jfkbitcoin1776s.\n"
        ...
        "2. \"transactions\"  (string, optional) A json array of objects. Each "
        "object the txid (string) vout (numeric)\n"
        ...
        HelpExampleCli("lockunspent", "false "
                                      "\"[{\\\"txid\\\":"
                                      "\\\"a08e6907dbbd3d809776dbfc5d82e371b764"
                                      "ed838b5655e72f463568df1aadf0\\\","
                                      "\\\"vout\\\":1}]\"") +
...
UniValue settxfee(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. amount         (numeric or string, required) The transaction fee "
        "in " +
        CURRENCY_UNIT +
        "/kB\n"
        "\nResult\n"
...
UniValue getwalletinfo(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        CURRENCY_UNIT +
        "\n"
        "  \"txcount\": xxxxxxx,              (numeric) the total number of "
        "transactions in the wallet\n"
        "  \"keypoololdest\": xxxxxx,         (numeric) the timestamp (seconds "
        "since Unix epoch) of the oldest pre-generated key in the key pool\n"
        "  \"keypoolsize\": xxxx,             (numeric) how many new keys are "
        "pre-generated (only counts external keys)\n"
        "  \"keypoolsize_hd_internal\": xxxx, (numeric) how many new keys are "
        "pre-generated for internal use (used for change outputs, only appears "
        "if the wallet is using this feature, otherwise external keys are "
        "used)\n"
  ...
  obj.push_back(
      Pair("immature_balance", ValueFromAmount(pwallet->GetImmatureBalance())));
  obj.push_back(Pair("txcount", (int)pwallet->mapWallet.size()));
  obj.push_back(Pair("keypoololdest", pwallet->GetOldestKeyPoolTime()));
  obj.push_back(Pair("keypoolsize", (int64_t)kpExternalSize));
  CKeyID masterKeyID = pwallet->GetHDChain().masterKeyID;
  if (!masterKeyID.IsNull() && pwallet->CanSupportFeature(FEATURE_HD_SPLIT)) {
    obj.push_back(Pair("keypoolsize_hd_internal",
...
UniValue listunspent(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 5)
    throw std::runtime_error(
        ...
        "confirmations to filter\n"
        "3. \"addresses\"      (string) A json array of jfkbitcoin1776 "
        "addresses to filter\n"
        "    [\n"
        "      \"address\"     (string) jfkbitcoin1776 address\n"
        "      ,...\n"
        "    ]\n"
        "4. include_unsafe (bool, optional, default=true) Include outputs that "
        "are not safe to spend\n"
        ...
        "    \"address\" : \"address\",    (string) the jfkbitcoin1776 "
        "address\n"
        "    \"account\" : \"account\",    (string) DEPRECATED. The associated "
        "account, or \"\" for the default account\n"
        ...
        "    \"spendable\" : xxx,        (bool) Whether we have the private "
        "keys to spend this output\n"
        "    \"solvable\" : xxx,         (bool) Whether we know how to spend "
        "this output, ignoring the lack of keys\n"
        "    \"safe\" : xxx              (bool) Whether this output is "
        "considered safe to spend. Unconfirmed transactions\n"
        "                              from outside keys and unconfirmed "
        "replacement transactions are considered unsafe\n"
        "                              and are not eligible for spending by "
  ...
  if (request.params.size() > 2 && !request.params[2].isNull()) {
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
      ...
      CJFKBitcoin1776Address address(input.get_str());
      if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           std::string("Invalid JFKBitcoin1776 address: ") +
                               input.get_str());
  ...
  for (const COutput &out : vecOutputs) {
    ...
    if (fValidAddress) {
      entry.push_back(
          Pair("address", CJFKBitcoin1776Address(address).ToString()));
      ...
      if (scriptPubKey.IsPayToScriptHash()) {
        ...
        if (pwallet->GetCScript(hash, redeemScript)) {
          entry.push_back(Pair("redeemScript", HexStr(redeemScript.begin(),
                                                      redeemScript.end())));
        }
      }
    }
    ...
    entry.push_back(
        Pair("scriptPubKey", HexStr(scriptPubKey.begin(), scriptPubKey.end())));
    entry.push_back(
...
UniValue fundrawtransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "address) The jfkbitcoin1776 address to receive the change\n"
        "     \"changePosition\"         (numeric, optional, default random) "
        "The index of the change output\n"
        "     \"includeWatching\"        (boolean, optional, default false) "
        "Also select inputs which are watch only\n"
        "     \"lockUnspents\"           (boolean, optional, default false) "
        "Lock selected unspent outputs\n"
        "     \"feeRate\"                (numeric, optional, default not set: "
        "makes wallet determine the fee) Set a specific feerate (" +
        CURRENCY_UNIT +
        " per KB)\n"
        "     \"subtractFeeFromOutputs\" (array, optional) A json array of "
        "integers.\n"
        "                              The fee will be equally deducted from "
        "the amount of each specified output.\n"
        "                              The outputs are specified by their "
        "zero-based index, before any change output is added.\n"
        "                              Those recipients will receive less "
        "jfkbitcoin1776s than you enter in their corresponding amount field.\n"
        "                              If no outputs are specified here, the "
        "sender pays the fee.\n"
        "                                  [vout_index,...]\n"
        "     \"replaceable\"            (boolean, optional) Marks this "
        "transaction as BIP125 replaceable.\n"
        "                              Allows this transaction to be replaced "
        "by a transaction with higher fees\n"
        "     \"conf_target\"            (numeric, optional) Confirmation "
        "target (in blocks)\n"
        "     \"estimate_mode\"          (string, optional, default=UNSET) The "
        "fee estimate mode, must be one of:\n"
        "         \"UNSET\"\n"
        "         \"ECONOMICAL\"\n"
        "         \"CONSERVATIVE\"\n"
        "   }\n"
        "                         for backward compatibility: passing in a "
        "true instead of an object will result in {\"includeWatching\":true}\n"
        "\nResult:\n"
        "{\n"
        "  \"hex\":       \"value\", (string)  The resulting raw transaction "
        "(hex-encoded string)\n"
    ...
    if (request.params[1].type() == UniValue::VBOOL) {
      ...
      RPCTypeCheckObj(
          options,
          {
              ...
              {"reserveChangeKey",
               UniValueType(UniValue::VBOOL)}, // DEPRECATED (and ignored),
                                               // should be removed in 0.16 or
                                               // so.
      ...
      if (options.exists("changeAddress")) {
        CJFKBitcoin1776Address address(options["changeAddress"].get_str());

        if (!address.IsValid())
          throw JSONRPCError(
              RPC_INVALID_ADDRESS_OR_KEY,
              "changeAddress must be a valid jfkbitcoin1776 address");
      ...
      if (options.exists("conf_target")) {
        ...
        coinControl.m_confirm_target =
            ParseConfirmTarget(options["conf_target"]);
      }
      if (options.exists("estimate_mode")) {
        ...
        if (!FeeModeFromString(options["estimate_mode"].get_str(),
...
UniValue bumpfee(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
    throw std::runtime_error(
        ...
        "\nBumps the fee of an opt-in-RBF transaction T, replacing it with a "
        "new transaction B.\n"
        ...
        "the command will currently fail\n"
        "instead of adding new inputs to compensate. (A future implementation "
        "could improve this.)\n"
        "The command will fail if the wallet or mempool contains a transaction "
        "that spends one of T's outputs.\n"
        ...
        "additional new relay fee (incrementalfee\n"
        "returned by getnetworkinfo) to enter the node's mempool.\n"
        "\nArguments:\n"
        "1. txid                  (string, required) The txid to be bumped\n"
        "2. options               (object, optional)\n"
        "   {\n"
        "     \"confTarget\"        (numeric, optional) Confirmation target "
        "(in blocks)\n"
        "     \"totalFee\"          (numeric, optional) Total fee (NOT "
        "feerate) to pay, in satoshis.\n"
        "                         In rare cases, the actual fee paid might be "
        "slightly higher than the specified\n"
        "                         totalFee if the tx change output has to be "
        "removed because it is too close to\n"
        "                         the dust threshold.\n"
        "     \"replaceable\"       (boolean, optional, default true) Whether "
        "the new transaction should still be\n"
        "                         marked bip-125 replaceable. If true, the "
        "sequence numbers in the transaction will\n"
        "                         be left unchanged from the original. If "
        "false, any input sequence numbers in the\n"
        "                         original transaction that were less than "
        "0xfffffffe will be increased to 0xfffffffe\n"
        "                         so the new transaction will not be "
        "explicitly bip-125 replaceable (though it may\n"
        "                         still be replaceable in practice, for "
        "example if it has unconfirmed ancestors which\n"
        "                         are replaceable).\n"
        "     \"estimate_mode\"     (string, optional, default=UNSET) The fee "
        "estimate mode, must be one of:\n"
        "         \"UNSET\"\n"
        "         \"ECONOMICAL\"\n"
        "         \"CONSERVATIVE\"\n"
        ...
        "  \"errors\":  [ str... ] (json array of strings) Errors encountered "
        "during processing (may be empty)\n"
        "}\n"
        "\nExamples:\n"
  ...
  if (!request.params[1].isNull()) {
    ...
    if (options.exists("confTarget") && options.exists("totalFee")) {
      throw JSONRPCError(RPC_INVALID_PARAMETER,
                         "confTarget and totalFee options should not both be "
                         "set. Please provide either a confirmation target for "
                         "fee estimation or an explicit total fee for the "
                         "transaction.");
    } else if (options.exists(
                   "confTarget")) { // TODO: alias this to conf_target
      ...
      if (totalFee <= 0) {
        throw JSONRPCError(
            RPC_INVALID_PARAMETER,
            strprintf("Invalid totalFee %s (must be greater than 0)",
...
UniValue getgenerate(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0) {
    throw std::runtime_error(
        "getgenerate\n"
        "\nReturn if the server is set to generate coins or not. The default "
        "is false.\n"
        "It is set with the command line argument -gen (or " +
        std::string(JFKBITCOIN1776_CONF_FILENAME) +
        " setting gen)\n"
        "It can also be set with the setgenerate call.\n"
        "\nResult\n"
        "true|false      (boolean) If the server is set to generate coins or "
        "not\n"
        "\nExamples:\n" +
...
UniValue setgenerate(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
    throw std::runtime_error(
        ...
        "generation, off to turn off.\n"
        "2. genproclimit     (numeric, optional) Set the processor limit for "
        "when generation is on. Can be -1 for unlimited.\n"
        "\nExamples:\n"
  ...
  GenerateJFKBitcoin1776s(fGenerate, nGenProcLimit, Params());
...
static const CRPCCommand commands[] = {
    ...
    {"wallet",
     ...
     &importprivkey,
     true,
     {"privkey", "label", "rescan", "force_version"}},
    ...
    {"wallet",
     ...
     &sendfrom,
     false,
     {"fromaccount", "toaddress", "amount", "minconf", "comment",
      "comment_to"}},
    {"wallet",
     "sendmany",
     &sendmany,
     false,
     {"fromaccount", "amounts", "minconf", "comment", "subtractfeefrom",
