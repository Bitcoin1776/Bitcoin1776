...
bool CWalletDB::EraseName(const std::string &strAddress) {
  ...
  // addresses,
  // receiving addresses must always have an address book entry if they're not
  // change return.
...
bool CWalletDB::WritePurpose(const std::string &strAddress,
  ...
  return WriteIC(std::make_pair(std::string("purpose"), strAddress),
                 strPurpose);
}
...
bool CWalletDB::WriteKey(const CPubKey &vchPubKey, const CPrivKey &vchPrivKey,
  ...
  return WriteIC(std::make_pair(std::string("key"), vchPubKey),
...
bool CWalletDB::WriteCryptedKey(
  ...
  if (!WriteIC(std::make_pair(std::string("ckey"), vchPubKey), vchCryptedSecret,
               false)) {
    return false;
  }
...
bool CWalletDB::WriteBestBlock(const CBlockLocator &locator) {
  WriteIC(std::string("bestblock"),
          CBlockLocator()); // Write empty block locator so versions that
                            // require a merkle branch automatically rescan
  return WriteIC(std::string("bestblock_nomerkle"), locator);
}

bool CWalletDB::ReadBestBlock(CBlockLocator &locator) {
  if (batch.Read(std::string("bestblock"), locator) && !locator.vHave.empty())
    return true;
...
bool ReadKeyValue(CWallet *pwallet, CDataStream &ssKey, CDataStream &ssValue,
  ...
  try {
    ...
    if (strType == "name") {
      ...
      ssKey >> strAddress;
      ssValue >>
          pwallet->mapAddressBook[CJFKBitcoin1776Address(strAddress).Get()]
              .name;
      ...
      // key is re-derived from the private key
      // using EC operations as a checksum.
      // Newer wallets store keys as "key"[pubkey] =>
      ...
      ssKey >> strKey;
      ssValue >> strValue;
      if (!pwallet->LoadDestData(CJFKBitcoin1776Address(strAddress).Get(),
                                 strKey, strValue)) {
...
DBErrors CWalletDB::LoadWallet(CWallet *pwallet) {
    ...
    while (true) {
      ...
      if (!ReadKeyValue(pwallet, ssKey, ssValue, wss, strType, strErr)) {
        ...
        if (IsKeyType(strType))
          ...
          // Leave other errors alone, if we try to fix them we might make
          // things worse.
          fNoncriticalErrors =
  ...
  for (CAccountingEntry &entry : pwallet->laccentries) {
    pwallet->wtxOrdered.insert(
        make_pair(entry.nOrderPos, CWallet::TxPair((CWalletTx *)0, &entry)));
  }
...
DBErrors CWalletDB::ZapSelectTx(std::vector<uint256> &vTxHashIn,
  ...
  for (uint256 hash : vTxHash) {
    ...
    if (it == vTxHashIn.end()) {
      ...
      if (!EraseTx(hash)) {
        LogPrint(BCLog::DB, "Transaction was found for deletion but returned "
                            "database error: %s\n",
                 hash.GetHex());
        delerror = true;
      }
...
bool CWalletDB::RecoverKeysOnlyFilter(void *callbackData, CDataStream ssKey,
  ...
  bool fReadOK;
  {
    ...
    fReadOK =
        ReadKeyValue(dummyWallet, ssKey, ssValue, dummyWss, strType, strErr);
  }
...
bool CWalletDB::TxnBegin() { return batch.TxnBegin(); }

bool CWalletDB::TxnCommit() { return batch.TxnCommit(); }

bool CWalletDB::TxnAbort() { return batch.TxnAbort(); }
