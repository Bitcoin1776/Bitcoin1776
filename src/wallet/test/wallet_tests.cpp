...
static void add_coin(const CAmount &nValue, int nAge = 6 * 24,
  ...
  if (fIsFromMe) {
    ...
    // so stop vin being empty, and cache a non-zero Debit to fake out
    // IsFromMe()
    tx.vin.resize(1);
  }
...
BOOST_AUTO_TEST_CASE(coin_selection_tests) {
  ...
  for (int i = 0; i < RUN_TESTS; i++) {
    ...
    add_coin(10 * CENT, 3,
             true); // a new 10 cent coin sent from one of our own addresses
    ...
    BOOST_CHECK(!testWallet.SelectCoinsMinConf(38 * CENT, 1, 6, 0, vCoins,
                                               setCoinsRet, nValueRet));
    // we can't even make 37 cents if we don't allow new coins even if they're
    // from us
    ...
    BOOST_CHECK_EQUAL(setCoinsRet.size(), 3U); // the best should be 20+10+5.
                                               // it's incredibly unlikely the 1
                                               // or 2 got included (but
                                               // possible)
    ...
    // when we try making 9 cents, no subset of smaller coins is enough, and we
    // get the next bigger coin (10)
    BOOST_CHECK(testWallet.SelectCoinsMinConf(9 * CENT, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet, 10 * CENT);
    BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);

    // now clear out the wallet and start again to test choosing between subsets
    // of smaller coins and the next biggest coin
    empty_wallet();
    ...
    // now try making 16 cents.  the best smaller coins can do is 6+7+8 = 21;
    // not as good at the next biggest coin, 20
    ...
    // and now if we try making 16 cents again, the smaller coins can make 5+6+7
    // = 18 cents, the same as the next biggest coin, 18
    BOOST_CHECK(testWallet.SelectCoinsMinConf(16 * CENT, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet, 18 * CENT); // we should get 18 in 1 coin
    BOOST_CHECK_EQUAL(
        setCoinsRet.size(),
        1U); // because in the event of a tie, the biggest coin wins
    ...
    BOOST_CHECK(testWallet.SelectCoinsMinConf(95 * CENT, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet, 1 * COIN); // we should get 1 JFK in 1 coin
    BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);

    BOOST_CHECK(testWallet.SelectCoinsMinConf(195 * CENT, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet, 2 * COIN); // we should get 2 JFK in 1 coin
    BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);

    // empty the wallet and start again, now with fractions of a cent, to test
    // small change avoidance
    ...
    // we'll get change smaller than MIN_CHANGE whatever happens, so can expect
    // MIN_CHANGE exactly
    ...
    BOOST_CHECK_EQUAL(nValueRet,
                      1 * MIN_CHANGE); // we should get the exact amount
    ...
    // they tried to consolidate 10 50k coins into one 500k coin, and ended up
    // with 50k in change
    empty_wallet();
    for (int j = 0; j < 20; j++)
      add_coin(50000 * COIN);
    ...
    BOOST_CHECK(testWallet.SelectCoinsMinConf(500000 * COIN, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet,
                      500000 * COIN); // we should get the exact amount
    BOOST_CHECK_EQUAL(setCoinsRet.size(), 10U); // in ten coins

    // if there's not enough in the smaller coins to make at least 1 *
    // MIN_CHANGE change (0.5+0.6+0.7 < 1.0+1.0),
    ...
    BOOST_CHECK_EQUAL(nValueRet,
                      MIN_CHANGE * 10105 / 100); // we should get all coins
    BOOST_CHECK_EQUAL(setCoinsRet.size(), 3U);

    // but if we try to make 99.9, we should take the bigger of the two small
    // coins to avoid small change
    ...
    for (CAmount amt = 1500; amt < COIN; amt *= 10) {
      empty_wallet();
      // Create 676 inputs (=  (old MAX_STANDARD_TX_SIZE == 100000)  / 148 bytes
      // per input)
    ...
    // test randomness
    {
      ...
      for (int j = 0; j < RANDOM_REPEATS; j++) {
        // selecting 1 from 100 identical coins depends on the shuffle; this
        // test will fail 1% of the time
        // run the test RANDOM_REPEATS times and only complain if all of them
        // fail
      ...
      // then try making 90 cents.  there are multiple competing "smallest
      // bigger" coins,
  ...
  // after.
  {
    ...
    BOOST_CHECK_EQUAL(
        ...
        strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":"
                  ...
                  "by pruning or data corruption (see jfkbitcoin1776d log for "
