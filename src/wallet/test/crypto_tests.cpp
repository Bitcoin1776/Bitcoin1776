...
bool OldEncrypt(const CKeyingMaterial &vchPlaintext,
  ...
  if (fOk)
    fOk = EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, chKey, chIV) != 0;
  if (fOk)
    fOk = EVP_EncryptUpdate(ctx, &vchCiphertext[0], &nCLen, &vchPlaintext[0],
                            nLen) != 0;
  if (fOk)
...
bool OldDecrypt(const std::vector<unsigned char> &vchCiphertext,
  ...
  EVP_CIPHER_CTX_init(ctx);
  if (fOk)
    fOk = EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, chKey, chIV) != 0;
  if (fOk)
    fOk = EVP_DecryptUpdate(ctx, &vchPlaintext[0], &nPLen, &vchCiphertext[0],
                            nLen) != 0;
  if (fOk)
...
public:
  ...
  TestPassphraseSingle(const std::vector<unsigned char> &vchSalt,
    ...
    BOOST_CHECK_MESSAGE(memcmp(chIV, crypt.vchIV.data(), crypt.vchIV.size()) ==
                        ...
                        HexStr(chIV, chIV + sizeof(chIV)) +
                            std::string(" != ") + HexStr(crypt.vchIV));

    if (!correctKey.empty())
      BOOST_CHECK_MESSAGE(memcmp(chKey, &correctKey[0], sizeof(chKey)) == 0,
                          HexStr(chKey, chKey + sizeof(chKey)) +
                              std::string(" != ") +
  ...
  static void TestDecrypt(const CCrypter &crypt,
    ...
    if (vchDecrypted1 != vchDecrypted2 &&
      ...
      for (CKeyingMaterial::iterator it = vchDecrypted1.end() - AES_BLOCK_SIZE;
    ...
    if (vchPlaintext.size())
      BOOST_CHECK(CKeyingMaterial(vchPlaintext.begin(), vchPlaintext.end()) ==
                  vchDecrypted2);
  }

  static void
  TestEncryptSingle(const CCrypter &crypt, const CKeyingMaterial &vchPlaintext,
    ...
    int result2 = OldEncrypt(vchPlaintext, vchCiphertext2, crypt.vchKey.data(),
                             crypt.vchIV.data());
    BOOST_CHECK(result1 == result2);
  ...
  TestEncrypt(const CCrypter &crypt,
              ...
              const std::vector<unsigned char> &vchCiphertextCorrect =
                  std::vector<unsigned char>()) {
    TestEncryptSingle(
        crypt, CKeyingMaterial(vchPlaintextIn.begin(), vchPlaintextIn.end()),
        vchCiphertextCorrect);
    for (std::vector<unsigned char>::const_iterator i(vchPlaintextIn.begin());
