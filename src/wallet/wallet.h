...
#ifndef JFKBITCOIN1776_WALLET_WALLET_H
#define JFKBITCOIN1776_WALLET_WALLET_H
...
enum WalletFeature {
  ...
  FEATURE_LATEST = FEATURE_COMPRPUBKEY // HD is optional, use
                                       // FEATURE_COMPRPUBKEY as latest version
};
    ...
    if (ser_action.ForRead()) {
      try {
        ...
        /* flag as external address if we can't read the internal boolean
           (this will be the case for any wallet before the HD chain split
           version) */
        fInternal = false;
      }
    } else {
...
public:
  ...
  inline void SerializationOp(Stream &s, Operation ser_action) {
    std::vector<uint256>
        vMerkleBranch; // For compatibility with older versions.
    READWRITE(tx);
    READWRITE(hashBlock);
  ...
  /** Pass this transaction to the mempool. Fails if absolute fee exceeds absurd
   * fee. */
...
public:
   ...
   * on this jfkbitcoin1776 node, and set to 0 for transactions that were
   * created
...
public:
  ...
  //! check whether we are allowed to upgrade (or already support) to the named
  //! feature
  ...
  bool LoadDestData(const CTxDestination &dest, const std::string &key,
                    const std::string &value);
  //! Look up a destination data tuple in the store, return true if found false
  //! otherwise
  ...
  //! Adds a watch-only address to the store, without saving it to disk (used by
  //! LoadWallet)
  bool LoadWatchOnly(const CScript &dest);

  //! Holds a timestamp at which point the wallet is scheduled (externally) to
  //! be relocked. Caller must arrange for actual relocking to occur via Lock().
  int64_t nRelockTime;
  ...
  //! change which version we're allowed to upgrade to (note that this does not
  //! immediately imply upgrading to that format)
  ...
  /* Initializes the wallet, returns a new CWallet instance or a null pointer in
   * case of an error */
...
#endif // JFKBITCOIN1776_WALLET_WALLET_H
