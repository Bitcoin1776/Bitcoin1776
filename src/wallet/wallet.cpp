...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CJFKBitcoin1776Address(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/jfkbitcoin1776/jfkbitcoin1776/pull/10340, but that change has been
    // postponed until after https://github.com/jfkbitcoin1776/jfkbitcoin1776/pull/10244 to
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CJFKBitcoin1776Address(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CJFKBitcoin1776Address(address).ToString(), strName);
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CJFKBitcoin1776Address(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CJFKBitcoin1776Address(dest).ToString(), key);
}
