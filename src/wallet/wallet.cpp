...
CPubKey CWallet::GenerateNewKey(CWalletDB &walletdb, bool internal) {
  ...
  bool fCompressed =
      CanSupportFeature(FEATURE_COMPRPUBKEY); // default to compressed public
                                              // keys if we want 0.6.0 wallets
...
void CWallet::DeriveNewChildKey(CWalletDB &walletdb, CKeyMetadata &metadata,
  ...
  do {
    ...
    if (internal) {
      chainChildKey.Derive(
          childKey, hdChain.nInternalChainCounter | BIP32_HARDENED_KEY_LIMIT);
      metadata.hdKeypath =
...
bool CWallet::LoadCScript(const CScript &redeemScript) {
  ...
  if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
    std::string strAddr =
        CJFKBitcoin1776Address(CScriptID(redeemScript)).ToString();
...
bool CWallet::Verify() {
  ...
  for (const std::string &walletFile : gArgs.GetArgs("-wallet")) {
    if (boost::filesystem::path(walletFile).filename() != walletFile) {
      return InitError(
          strprintf(_("Error loading wallet %s. -wallet parameter must only "
                      "specify a filename (not a path)."),
                    walletFile));
    }

    if (SanitizeString(walletFile, SAFE_CHARS_FILENAME) != walletFile) {
      return InitError(strprintf(
          _("Error loading wallet %s. Invalid characters in -wallet filename."),
          walletFile));
    }
    ...
    if (fs::exists(wallet_path) &&
        (!fs::is_regular_file(wallet_path) || fs::is_symlink(wallet_path))) {
      return InitError(strprintf(_("Error loading wallet %s. -wallet filename "
    ...
    if (!wallet_paths.insert(wallet_path).second) {
      return InitError(strprintf(
          _("Error loading wallet %s. Duplicate -wallet filename specified."),
          walletFile));
    }
...
bool CWallet::EncryptWallet(const SecureString &strWalletPassphrase) {
...

  {
    ...
    if (!EncryptKeys(_vMasterKey)) {
      ...
      delete pwalletdbEncryption;
      // We now probably have half of our keys encrypted in memory, and half
      // not...
...
DBErrors CWallet::ReorderTransactions() {
  ...
  for (std::map<uint256, CWalletTx>::iterator it = mapWallet.begin();
    ...
    txByTime.insert(
        std::make_pair(wtx->nTimeReceived, TxPair(wtx, (CAccountingEntry *)0)));
  }
  std::list<CAccountingEntry> acentries;
  walletdb.ListAccountCreditDebit("", acentries);
  for (CAccountingEntry &entry : acentries) {
    txByTime.insert(
        std::make_pair(entry.nTime, TxPair((CWalletTx *)0, &entry)));
  }
...
bool CWallet::AddToWallet(const CWalletTx &wtxIn, bool fFlushOnClose) {
  ...
  if (fInsertedNew) {
    ...
    wtxOrdered.insert(
        std::make_pair(wtx.nOrderPos, TxPair(&wtx, (CAccountingEntry *)0)));
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef &ptx,
  ...
  const CTransaction &tx = *ptx;
  {
    ...
    if (fExisted || IsMine(tx) || IsFromMe(tx)) {
       ...
       * have appeared in a new transaction. If so, remove those keys from the
       * keypool.
       * This can happen when restoring an old wallet backup that does not
       * contain
       * the mostly recently created transactions from newer versions of the
       * wallet.
       */
...
bool CWallet::AbandonTransaction(const uint256 &hashTx) {
  ...
  while (!todo.empty()) {
    ...
    // if (currentconfirm < 0) {Tx and spends are already conflicted, no need to
    // abandon}
    if (currentconfirm == 0 && !wtx.isAbandoned()) {
      // If the orig tx was not in block/mempool, none of its spends can be in
      // mempool
      ...
      // Iterate over all its outputs, and mark transactions in the wallet that
      // spend them abandoned too
...
bool CWallet::IsChange(const CTxOut &txout) const {
  ...
  // is change. That assumption is likely to break when we implement
  // multisignature
  // wallets that return change back into a multi-signature-protected address;
  // a better way of identifying which outputs are 'the send' and which are
  // 'the change' will need to be implemented (maybe extend CWalletTx to
  // remember
...
CBlockIndex *CWallet::ScanForWalletTransactions(CBlockIndex *pindexStart,
  ...
  CBlockIndex *ret = nullptr;
  {
    ...
    ShowProgress(_("Rescanning..."), 0); // show rescan progress in GUI as
                                         // dialog or on splashscreen, if
                                         // -rescan on startup
    ...
    while (pindex && !fAbortRescan) {
      if (pindex->nHeight % 100 == 0 && dProgressTip - dProgressStart > 0.0)
        ShowProgress(
            _("Rescanning..."),
            std::max(
                1, std::min(99, (int)((GuessVerificationProgress(
...
void CWallet::ReacceptWalletTransactions() {
  // If transactions aren't being broadcasted, don't let them into local mempool
  // either
...
CAmount CWalletTx::GetCredit(const isminefilter &filter) const {
  // Must wait until coinbase is safely deep enough in the chain before valuing
  // it
...
bool CWalletTx::IsTrusted() const {
  ...
  if (!bSpendZeroConfChange ||
      !IsFromMe(ISMINE_ALL)) // using wtx's cached debit
    return false;
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const {
  ...
  // https://github.com/jfkbitcoin1776/jfkbitcoin1776/pull/10340, but that
  // change has been
  // postponed until after
  // https://github.com/jfkbitcoin1776/jfkbitcoin1776/pull/10244 to
      ...
      if (depth >= 0 && output.n < it->second.tx->vout.size() &&
        ...
        if (ExtractDestination(
                FindNonChangeParentOutput(*it->second.tx, output.n)
                    .scriptPubKey,
                address)) {
          result[address].emplace_back(&it->second, output.n, depth,
                                       true /* spendable */,
...
bool CWallet::SelectCoins(const std::vector<COutput> &vAvailableCoins,
  ...
  // because SelectCoinsMinConf clears the setCoinsRet, we now add the possible
  // inputs to the coinset
...
static CFeeRate GetDiscardRate(const CBlockPolicyEstimator &estimator) {
  ...
  CFeeRate discard_rate = estimator.estimateSmartFee(
      highest_target, nullptr /* FeeCalculation */, false /* conservative */);
  // Don't let discard_rate be greater than longest possible fee estimate if we
  // get a valid fee estimate
...
bool CWallet::CreateTransaction(const std::vector<CRecipient> &vecSend,
  ...
  unsigned int nBytes;
  {
    ...
    LOCK2(cs_main, cs_wallet);
    {
      ...
      // change transaction isn't always pay-to-jfkbitcoin1776-address
      ...
      if (!boost::get<CNoDestination>(&coin_control.destChange)) {
        ...
        // Note: We use a new key here to keep it from being obvious which side
        // is the change.
        //  The drawback is that by not reusing a previous key, the change may
        //  be lost if a
        //  backup is restored, if the backup doesn't have the new private key
        //  for the change.
        //  If we reused the old key, it would be possible to add code to look
        //  for and
        //  rediscover unknown transactions that were written with keys of ours
        //  to recover
        //  post-backup change.
      ...
      while (true) {
        ...
        for (const auto &recipient : vecSend) {
          ...
          if (recipient.fSubtractFeeFromAmount) {
            txout.nValue -=
                nFeeRet / nSubtractFeeFromAmount; // Subtract fee equally from
        ...
        // If we made it here and we aren't even able to meet the relay fee on
        // the next pass, give up
        ...
        if (nFeeRet >= nFeeNeeded) {
          ...
          if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 &&
            ...
            unsigned int tx_size_with_change =
                nBytes + change_prototype_size + 2; // Add 2 as a buffer in case
                                                    // increasing # of outputs
                                                    // changes compact size
        ...
        if (nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
          ...
          std::vector<CTxOut>::iterator change_position =
              txNew.vout.begin() + nChangePosInOut;
          // Only reduce change if remaining amount is still a large enough
          // output.
    ...
    if (sign) {
      ...
      for (const auto &coin : setCoins) {
        ...
        if (!ProduceSignature(
                TransactionSignatureCreator(this, &txNewConst, nIn,
  ...
  LogPrintf("Fee Calculation: Fee:%d Bytes:%u Needed:%d Tgt:%d (requested %d) "
            "Reason:\"%s\" Decay %.5f: Estimation: (%g - %g) %.2f%% %.1f/(%.1f "
            "%d mem %.1f out) Fail: (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f "
            "out)\n",
...
bool CWallet::AddAccountingEntry(const CAccountingEntry &acentry,
  ...
  wtxOrdered.insert(
      std::make_pair(entry.nOrderPos, TxPair((CWalletTx *)0, &entry)));
...
CAmount CWallet::GetMinimumFee(unsigned int nTxBytes,
                               ...
                               FeeCalculation *feeCalc) {
  /* User control of how to calculate fee uses the following parameter
     precedence:
  ...
  if (coin_control.m_feerate) { // 1.
    ...
    if (coin_control.fOverrideFeeRate)
      return fee_needed;
             ...
             ::payTxFee != CFeeRate(0)) { // 3. TODO: remove magic value of 0
                                          // for global payTxFee
    ...
    // Allow to override the default fee estimate mode over the CoinControl
    // instance
...
bool CWallet::SetAddressBook(const CTxDestination &address,
  ...
  if (!strPurpose.empty() &&
      !CWalletDB(*dbw).WritePurpose(CJFKBitcoin1776Address(address).ToString(),
                                    strPurpose))
    return false;
  return CWalletDB(*dbw).WriteName(CJFKBitcoin1776Address(address).ToString(),
                                   strName);
}
...
bool CWallet::DelAddressBook(const CTxDestination &address) {
  {
    ...
    std::string strAddress = CJFKBitcoin1776Address(address).ToString();
  ...
  CWalletDB(*dbw).ErasePurpose(CJFKBitcoin1776Address(address).ToString());
  return CWalletDB(*dbw).EraseName(CJFKBitcoin1776Address(address).ToString());
}
...
bool CWallet::TopUpKeyPool(unsigned int kpSize) {
  {
    ...
    for (int64_t i = missingInternal + missingExternal; i--;) {
      ...
      assert(m_max_keypool_index <
             std::numeric_limits<int64_t>::max()); // How in the hell did you
                                                   // use so many keys?
...
std::set<std::set<CTxDestination>> CWallet::GetAddressGroupings() {
  ...
  std::set<std::set<CTxDestination> *>
      uniqueGroupings; // a set of pointers to groups of addresses
...
void CWallet::GetKeyBirthTimes(
  ...
  CBlockIndex *pindexMax =
      chainActive[std::max(0, chainActive.Height() - 144)]; // the tip can be
                                                            // reorganized; use
                                                            // a 144-block
                                                            // safety margin
  ...
  for (std::map<CKeyID, CBlockIndex *>::const_iterator it =
    ...
    mapKeyBirth[it->first] = it->second->GetBlockTime() -
                             TIMESTAMP_WINDOW; // block times can be 2h off
}
 ...
 * https://jfkbitcoin1776talk.org/?topic=54527, or
 * https://github.com/jfkbitcoin1776/jfkbitcoin1776/pull/1393.
 */
unsigned int CWallet::ComputeTimeSmart(const CWalletTx &wtx) const {
  ...
  if (!wtx.hashUnset()) {
    if (mapBlockIndex.count(wtx.hashBlock)) {
      ...
      // Tolerate times up to the last timestamp in the wallet not more than 5
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key,
  ...
  return CWalletDB(*dbw).WriteDestData(CJFKBitcoin1776Address(dest).ToString(),
                                       key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest,
  ...
  if (!mapAddressBook[dest].destdata.erase(key))
    return false;
  return CWalletDB(*dbw).EraseDestData(CJFKBitcoin1776Address(dest).ToString(),
                                       key);
}
...
std::string CWallet::GetWalletHelpString(bool showDebug) {
  ...
  strUsage += HelpMessageOpt(
      "-fallbackfee=<amt>",
      strprintf(_("A fee rate (in %s/kB) that will be used when fee estimation "
                  "has insufficient data (default: %s)"),
                CURRENCY_UNIT, FormatMoney(DEFAULT_FALLBACK_FEE)));
  strUsage += HelpMessageOpt(
      "-discardfee=<amt>",
      strprintf(_("The fee rate (in %s/kB) that indicates your tolerance for "
                  "discarding change by adding it to the fee (default: %s). "
                  "Note: An output is discarded if it is dust at this rate, "
                  "but we will always discard up to the dust relay fee and a "
                  "discard fee above that is limited by the fee estimate for "
                  "the longest target"),
  ...
  strUsage += HelpMessageOpt(
      "-spendzeroconfchange",
      strprintf(
          _("Spend unconfirmed change when sending transactions (default: %u)"),
  ...
  strUsage += HelpMessageOpt(
      "-usehd",
      _("Use hierarchical deterministic key generation (HD) after BIP32. Only "
        "has effect during wallet creation/first start") +
          " " + strprintf(_("(default: %u)"), DEFAULT_USE_HD_WALLET));
  strUsage += HelpMessageOpt(
      "-walletrbf",
      strprintf(
          _("Send transactions with full-RBF opt-in enabled (default: %u)"),
          DEFAULT_WALLET_RBF));
  ...
  if (showDebug) {
    ...
    strUsage += HelpMessageOpt("-privdb",
                               strprintf("Sets the DB_PRIVATE flag in the "
                                         "wallet db environment (default: %u)",
                                         DEFAULT_WALLET_PRIVDB));
    strUsage += HelpMessageOpt(
        "-walletrejectlongchains",
        strprintf(_("Wallet will not create transactions that violate mempool "
                    "chain limits (default: %u)"),
                  DEFAULT_WALLET_REJECT_LONG_CHAINS));
  }
...
CWallet *CWallet::CreateWalletFromFile(const std::string walletFile) {
  ...
  if (gArgs.GetBoolArg("-upgradewallet", fFirstRun)) {
    ...
    if (nMaxVersion == 0) // the -upgradewallet without argument case
    {
      ...
      walletInstance->SetMinVersion(
          FEATURE_LATEST); // permanently upgrade the wallet immediately
    } else
  ...
  if (fFirstRun) {
    ...
    if (gArgs.GetBoolArg("-usehd", DEFAULT_USE_HD_WALLET) &&
      ...
      // ensure this wallet.dat can only be opened by clients supporting HD with
      // chain split
    ...
    if (walletInstance->GetKeyFromPool(newDefaultKey, false)) {
      ...
      if (!walletInstance->SetAddressBook(walletInstance->vchDefaultKey.GetID(),
                                          "", "receive")) {
...
bool CWallet::ParameterInteraction() {
  ...
  if (gArgs.GetBoolArg("-salvagewallet", false)) {
    if (is_multiwallet) {
      return InitError(strprintf("%s is only allowed with a single wallet file",
                                 "-salvagewallet"));
    }
  ...
  if (gArgs.IsArgSet("-mintxfee")) {
    ...
    if (n > HIGH_TX_FEE_PER_KB)
      InitWarning(AmountHighWarn("-mintxfee") + " " +
                  _("This is the minimum transaction fee you pay on every "
                    "transaction."));
  ...
  if (gArgs.IsArgSet("-discardfee")) {
    ...
    if (!ParseMoney(gArgs.GetArg("-discardfee", ""), nFeePerK))
      return InitError(
          strprintf(_("Invalid amount for -discardfee=<amount>: '%s'"),
                    gArgs.GetArg("-discardfee", "")));
    if (nFeePerK > HIGH_TX_FEE_PER_KB)
      InitWarning(AmountHighWarn("-discardfee") + " " +
                  _("This is the transaction fee you may discard if change is "
                    "smaller than dust at this level"));
  ...
  if (gArgs.IsArgSet("-paytxfee")) {
    ...
    if (nFeePerK > HIGH_TX_FEE_PER_KB)
      InitWarning(AmountHighWarn("-paytxfee") + " " +
                  _("This is the transaction fee you will pay if you send a "
                    "transaction."));

    payTxFee = CFeeRate(nFeePerK, 1000);
    if (payTxFee < ::minRelayTxFee) {
      return InitError(strprintf(_("Invalid amount for -paytxfee=<amount>: "
  ...
  if (gArgs.IsArgSet("-maxtxfee")) {
    ...
    if (nMaxFee > HIGH_MAX_TX_FEE)
      InitWarning(_("-maxtxfee is set very high! Fees this large could be paid "
                    "on a single transaction."));
    maxTxFee = nMaxFee;
    if (CFeeRate(maxTxFee, 1000) < ::minRelayTxFee) {
      return InitError(strprintf(
          _("Invalid amount for -maxtxfee=<amount>: '%s' (must be at least the "
...
bool CMerkleTx::AcceptToMemoryPool(const CAmount &nAbsurdFee,
                                   CValidationState &state) {
  return ::AcceptToMemoryPool(mempool, state, tx, true, nullptr, nullptr, false,
                              nAbsurdFee);
}
