...
UniValue importprivkey(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
    throw std::runtime_error(
        "importprivkey \"privkey\" ( \"label\" ) ( rescan force_version )\n"
        "\nAdds a private key (as returned by dumpprivkey) to your wallet. "
        "Requires a new wallet backup.\n"
        "\nArguments:\n"
        "1. \"privkey\"          (string, required) The private key (see "
        "dumpprivkey)\n"
        "2. \"label\"            (string, optional, default=\"\") An optional "
        "label\n"
        "3. rescan               (boolean, optional, default=true) Rescan the "
        "wallet for transactions\n"
        "4. force_version        (boolean, optional, default=false) Force the "
        "version of the key to match the node\n"
  ...
  if (!fGood && !force_version)
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid private key encoding; use force_version flag "
                       "to import anyway");
...
UniValue abortrescan(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 0)
    throw std::runtime_error("abortrescan\n"
                             "\nStops current wallet rescan triggered e.g. by "
                             "an importprivkey call.\n"
                             "\nExamples:\n"
...
void ImportAddress(CWallet *, const CJFKBitcoin1776Address &address,
...
void ImportScript(CWallet *const pwallet, const CScript &script,
  ...
  if (isRedeemScript) {
    ...
    ImportAddress(pwallet, CJFKBitcoin1776Address(CScriptID(script)), strLabel);
  } else {
...
void ImportAddress(CWallet *const pwallet,
                   const CJFKBitcoin1776Address &address,
                   const std::string &strLabel) {
...
UniValue importaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "\nAdds a script (in hex) or address that can be watched as if it were "
        "in your wallet but cannot be used to spend. Requires a new wallet "
        "backup.\n"
        "\nArguments:\n"
        ...
        "3. rescan               (boolean, optional, default=true) Rescan the "
        "wallet for transactions\n"
        "4. p2sh                 (boolean, optional, default=false) Add the "
        "P2SH version of the script as well\n"
        "\nNote: This call can take minutes to complete if rescan is true.\n"
  ...
  if (address.IsValid()) {
    ...
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid JFKBitcoin1776 address or script");
  }
...
UniValue importprunedfunds(const JSONRPCRequest &request) {
  ...
  // Search partial merkle tree in proof for our transaction and index in valid
  // block
  ...
  if (merkleBlock.txn.ExtractMatches(vMatch, vIndex) ==
    ...
    if ((it = std::find(vMatch.begin(), vMatch.end(), hashTx)) ==
        vMatch.end()) {
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
...
UniValue removeprunedfunds(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"txid\"           (string, required) The hex-encoded id of the "
        "transaction you are deleting\n"
        "\nExamples:\n" +
        HelpExampleCli("removeprunedfunds", "\"a8d0c0184dde994a09ec054286f1ce58"
                                            "1bebf46446a512166eae7628734ea0a5"
                                            "\"") +
        "\nAs a JSON-RPC call\n" +
        HelpExampleRpc("removeprunedfunds", "\"a8d0c0184dde994a09ec054286f1ce58"
                                            "1bebf46446a512166eae7628734ea0a5"
                                            "\""));
...
UniValue importpubkey(const JSONRPCRequest &request) {
  ...
  ImportAddress(pwallet, CJFKBitcoin1776Address(pubKey.GetID()), strLabel);
  ...
  while (file.good()) {
    ...
    if (pwallet->HaveKey(keyid)) {
      LogPrintf("Skipping import of %s (key already present)\n",
                CJFKBitcoin1776Address(keyid).ToString());
      continue;
    }
...
UniValue dumpprivkey(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"   (string, required) The jfkbitcoin1776 address for "
        "the private key\n"
        "\nResult:\n"
  ...
  return CJFKBitcoin1776Secret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "Imported scripts are not currently included in wallet dumps, these "
        ...
        "\nArguments:\n"
        "1. \"filename\"    (string, required) The filename with path (either "
        "absolute or relative to jfkbitcoin1776d)\n"
        "\nResult:\n"
        "{                           (json object)\n"
        "  \"filename\" : {        (string) The filename with full absolute "
        "path\n"
        "}\n"
        "\nExamples:\n" +
        HelpExampleCli("dumpwallet", "\"test\"") +
   ...
   * https://github.com/jfkbitcoin1776/jfkbitcoin1776/issues/9934
   * It may also avoid other security issues.
   */
  if (boost::filesystem::exists(filepath)) {
    throw JSONRPCError(RPC_INVALID_PARAMETER,
                       filepath.string() + " already exists. If you are sure "
                                           "this is what you want, move it out "
                                           "of the way first");
  }
  ...
  // produce output
  file << strprintf("# Wallet dump created by JFKBitcoin1776 %s\n",
                    CLIENT_BUILD);
  ...
  if (!masterKeyID.IsNull()) {
    ...
    if (pwallet->GetKey(masterKeyID, key)) {
      ...
      CJFKBitcoin1776ExtKey b58extkey;
  ...
  for (std::vector<std::pair<int64_t, CKeyID>>::const_iterator it =
    ...
    std::string strAddr = CJFKBitcoin1776Address(keyid).ToString();
    CKey key;
    if (pwallet->GetKey(keyid, key)) {
      file << strprintf("%s %s ", CJFKBitcoin1776Secret(key).ToString(),
                        strTime);
...
UniValue ProcessImport(CWallet *const pwallet, const UniValue &data,
  ...
  try {
    ...
    if (!isScript) {
      address = CJFKBitcoin1776Address(output);
      if (!address.IsValid()) {
    ...
    if (isP2SH) {
      ...
      CJFKBitcoin1776Address redeemAddress =
          CJFKBitcoin1776Address(CScriptID(redeemScript));
      ...
      if (pubKeys.size() && keys.size() == 0) {
        ...
        CJFKBitcoin1776Address pubKeyAddress =
            CJFKBitcoin1776Address(pubKey.GetID());
        ...
        // Consistency check.
        if (isScript) {
          CJFKBitcoin1776Address scriptAddress;
          CTxDestination destination;

          if (ExtractDestination(script, destination)) {
            scriptAddress = CJFKBitcoin1776Address(destination);
        ...
        if (::IsMine(*pwallet, pubKeyScript) == ISMINE_SPENDABLE) {
          throw JSONRPCError(RPC_WALLET_ERROR, "The wallet already contains "
      ...
      for (const UniValue &request : requests.getValues()) {
        ...
        if (scannedTime <= GetImportTimestamp(request, now) ||
          ...
          result.pushKV(
              ...
              JSONRPCError(
                  RPC_MISC_ERROR,
                  strprintf("Rescan failed for key with creation timestamp %d. "
                            "There was an error reading a "
                            "block from time %d, which is after or within %d "
                            "seconds of key creation, and "
                            "could contain transactions pertaining to the key. "
                            "As a result, transactions "
                            "and coins using this key may not appear in the "
                            "wallet. This error could be "
                            "caused by pruning or data corruption (see "
                            "jfkbitcoin1776d log for details) and could "
                            "be dealt with by downloading and rescanning the "
                            "relevant blocks (see -reindex "
                            "and -rescan options).",
