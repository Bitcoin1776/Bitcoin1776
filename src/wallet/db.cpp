...
//! so jfkbitcoin1776 should never create different databases with the same
//! fileid, but
  ...
  for (const auto &item : env.mapDb) {
    ...
    if (item.second && item.second->get_mpf()->get_fileid(item_fileid) == 0 &&
      ...
      throw std::runtime_error(strprintf(
          ...
          filename, HexStr(std::begin(item_fileid), std::end(item_fileid)),
...
bool CDBEnv::Open(const fs::path &pathIn) {
  ...
  dbenv->set_cachesize(0, 0x100000,
                       1); // 1 MiB should be enough for just the wallet
...
bool CDB::VerifyEnvironment(const std::string &walletFile,
  ...
  if (!bitdb.Open(dataDir)) {
    ...
    // try again
    if (!bitdb.Open(dataDir)) {
      // if it still fails, it probably means we can't even create the database
      // env
      errorStr =
          strprintf(_("Error initializing wallet database environment %s!"),
                    GetDataDir());
      return false;
    }
  }
...
bool CDBEnv::Salvage(const std::string &strFile, bool fAggressive,
  ...
  if (result == DB_VERIFY_BAD) {
    LogPrintf("CDBEnv::Salvage: Database salvage found errors, all data may "
              "not be recoverable.\n");
    if (!fAggressive) {
      LogPrintf("CDBEnv::Salvage: Rerun with aggressive mode to ignore errors "
                "and continue.\n");
      return false;
    }
  }
  ...
  while (!strDump.eof() && keyHex != DATA_END) {
    ...
    if (keyHex != DATA_END) {
      ...
      if (valueHex == DATA_END) {
        LogPrintf("CDBEnv::Salvage: WARNING: Number of keys in data does not "
  ...
  if (keyHex != DATA_END) {
    LogPrintf("CDBEnv::Salvage: WARNING: Unexpected end of file while reading "
              "salvage output.\n");
    return false;
  }
...

  {
    ...
    if (pdb == nullptr) {
      ...
      if (fMockDb) {
        ...
        if (ret != 0) {
          throw std::runtime_error(
              strprintf("CDB: Failed to configure for no temp file backing for "
                        "database %s",
                        strFilename));
        }
      }
...
void CWalletDBWrapper::IncrementUpdateCounter() { ++nUpdateCounter; }
            ...
            while (fSuccess) {
              ...
              if (pszSkip &&
                  strncmp(ssKey.data(), pszSkip,
                          std::min(ssKey.size(), strlen(pszSkip))) == 0)
                continue;
              ...
              int ret2 =
                  pdbCopy->put(nullptr, &datKey, &datValue, DB_NOOVERWRITE);
              if (ret2 > 0)
                fSuccess = false;
            }
          if (fSuccess) {
