...
static int AppInitRawTx(int argc, char *argv[]) {
  ...
  if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
    ...
    std::string strUsage =
        strprintf(_("%s jfkbitcoin1776-tx utility version"), _(PACKAGE_NAME)) +
        " " + FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
        "  jfkbitcoin1776-tx [options] <hex-tx> [commands]  " +
        _("Update hex-encoded jfkbitcoin1776 transaction") + "\n" +
        "  jfkbitcoin1776-tx [options] -create [commands]   " +
        _("Create hex-encoded jfkbitcoin1776 transaction") + "\n" + "\n";
    ...
    strUsage +=
        HelpMessageOpt("-txid", _("Output only the hex-encoded transaction id "
    ...
    strUsage += HelpMessageOpt(
        "replaceable(=N)", _("Set RBF opt-in sequence number for input N (if "
                             "not provided, opt-in all available inputs)"));
    ...
    strUsage +=
        HelpMessageOpt("outpubkey=VALUE:PUBKEY[:FLAGS]",
                       _("Add pay-to-pubkey output to TX") + ". " +
                           _("Optionally add the \"W\" flag to produce a "
    ...
    strUsage += HelpMessageOpt("load=NAME:FILENAME",
...
static void MutateTxSign(CMutableTransaction &tx, const std::string &flagStr) {
  ...
  for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
    ...
    if (!fHashSingle || (i < mergedTx.vout.size()))
      ProduceSignature(MutableTransactionSignatureCreator(&keystore, &mergedTx,
                                                          i, amount, nHashType),
                       prevPubKey, sigdata);
    ...
    for (const CTransaction &txv : txVariants)
      sigdata = CombineSignatures(
          prevPubKey, MutableTransactionSignatureChecker(&mergedTx, i, amount),
...
static void OutputTxHash(const CTransaction &tx) {
  std::string strHexHash = tx.GetHash().GetHex(); // the hex-encoded transaction
                                                  // hash (aka the transaction
                                                  // id)
