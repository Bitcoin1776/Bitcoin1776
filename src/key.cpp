...
CPrivKey CKey::GetPrivKey() const {
  ...
  ret = ec_privkey_export_der(
      secp256k1_context_sign, (unsigned char *)privkey.data(), &privkeylen,
      begin(),
...
bool CKey::Sign(const uint256 &hash, std::vector<unsigned char> &vchSig,
  ...
  secp256k1_ecdsa_signature_serialize_der(
      secp256k1_context_sign, (unsigned char *)vchSig.data(), &nSigLen, &sig);
...
bool CKey::VerifyPubKey(const CPubKey &pubkey) const {
  ...
  unsigned char rnd[8];
  std::string str = "JFKBitcoin1776 key verification\n";
