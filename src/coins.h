...
#ifndef JFKBITCOIN1776_COINS_H
#define JFKBITCOIN1776_COINS_H
...
public:
  ...
  //! at which height this containing transaction was included in the active
  //! block chain
  uint32_t nHeight : 31;
...
struct CCoinsCacheEntry {
  ...
  enum Flags {
    DIRTY = (1 << 0), // This cache entry is potentially different from the
                      // version in the parent view.
    FRESH =
...
public:
  ...
  /**
   * Return a reference to Coin in the cache, or a pruned one if not found. This
   * is
   * more efficient than GetCoin.
   *
   * Generally, do not hold the reference returned for more than a short scope.
   * While the current implementation allows for modifications to the contents
   * of the cache while holding the reference, this behavior should not be
   * relied
   ...
   * Failure to call this method before destruction will cause the changes to be
   * forgotten.
   * If false is returned, the state of this cache (and its backing view) will
   * be undefined.
   */
  bool Flush();
  ...
  /**
   * Amount of jfkbitcoin1776s coming in to a transaction
  ...
  //! Check whether all prevouts of the transaction are present in the UTXO set
