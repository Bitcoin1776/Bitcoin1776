...
void TxToJSON(const CTransaction &tx, const uint256 hashBlock,
              UniValue &entry) {
  // Call into TxToUniv() in jfkbitcoin1776-common to decode the transaction
  // hex.
  //
  // Blockchain contextual information (confirmations and blocktime) is not
  // available to code in jfkbitcoin1776-common, so we query them here and push
  // the
...
UniValue getrawtransaction(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "{\n"
        "  \"hex\" : \"data\",       (string) The serialized, hex-encoded data "
        "for 'txid'\n"
        "  \"txid\" : \"id\",        (string) The transaction id (same as "
        "provided)\n"
        "  \"hash\" : \"id\",        (string) The transaction hash (differs "
        "from txid for witness transactions)\n"
        ...
        "       \"txinwitness\": [\"hex\", ...] (array of string) hex-encoded "
        "witness data (if any)\n"
        "     }\n"
        "     ,...\n"
        "  ],\n"
...
UniValue gettxoutproof(const JSONRPCRequest &request) {
  if (request.fHelp ||
    ...
    throw std::runtime_error(
        ...
        "\nNOTE: By default this function only works sometimes. This is when "
        "there is an\n"
        ...
        "      ,...\n"
        "    ]\n"
        "2. \"blockhash\"   (string, optional) If specified, looks for txid in "
  ...
  if (!request.params[1].isNull()) {
    ...
    // Loop through txids and try to find which block they're in. Exit loop once
    // a block is found.
...
UniValue verifytxoutproof(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "gettxoutproof\n"
        "\nResult:\n"
        "[\"txid\"]      (array, strings) The txid(s) which the proof commits "
        "to, or empty array if the proof is invalid\n");
...
UniValue createrawtransaction(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"inputs\"                (array, required) A json array of json "
        "objects\n"
        "     [\n"
        "       {\n"
        ...
        "       ,...\n"
        "     ]\n"
        "2. \"outputs\"               (object, required) a json object with "
        "outputs\n"
        "    {\n"
        "      \"address\": x.xxx,    (numeric or string, required) The key is "
        "the jfkbitcoin1776 address, the numeric value (can be string) is "
        "the " +
        CURRENCY_UNIT +
        " amount\n"
        ...
        "4. replaceable               (boolean, optional, default=false) Marks "
        "this transaction as BIP125 replaceable.\n"
        "                             Allows this transaction to be replaced "
        "by a transaction with higher fees. If provided, it is an error if "
        "explicit sequence numbers are incompatible.\n"
        "\nResult:\n"
        "\"transaction\"              (string) hex string of the transaction\n"

        "\nExamples:\n" +
        HelpExampleCli("createrawtransaction", "\"[{\\\"txid\\\":\\\"myid\\\","
                                               "\\\"vout\\\":0}]\" "
                                               "\"{\\\"address\\\":0.01}\"") +
        HelpExampleCli("createrawtransaction",
                       "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" "
                       "\"{\\\"data\\\":\\\"00010203\\\"}\"") +
        HelpExampleRpc("createrawtransaction", "\"[{\\\"txid\\\":\\\"myid\\\","
                                               "\\\"vout\\\":0}]\", "
                                               "\"{\\\"address\\\":0.01}\"") +
  ...
  bool rbfOptIn =
      request.params.size() > 3 ? request.params[3].isTrue() : false;
  ...
  if (!request.params[3].isNull() && rbfOptIn != SignalsOptInRBF(rawTx)) {
    throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter combination: "
                                              "Sequence number(s) contradict "
                                              "replaceable option");
  }
...
UniValue decodescript(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "     ,...\n"
        "  ],\n"
        "  \"p2sh\",\"address\" (string) address of P2SH script wrapping this "
        "redeem script (not returned if the script is already a P2SH).\n"
        "}\n"
        "\nExamples:\n" +
  ...
  if (type.isStr() && type.get_str() != "scripthash") {
    ...
    r.push_back(
        Pair("p2sh", CJFKBitcoin1776Address(CScriptID(script)).ToString()));
  }

  return r;
}

/** Pushes a JSON object for script verification or signing errors to
 * vErrorsRet. */
static void TxInErrorToJSON(const CTxIn &txin, UniValue &vErrorsRet,
  ...
  for (unsigned int i = 0; i < txin.scriptWitness.stack.size(); i++) {
    witness.push_back(HexStr(txin.scriptWitness.stack[i].begin(),
                             txin.scriptWitness.stack[i].end()));
  }
  entry.push_back(Pair("witness", witness));
  entry.push_back(
      Pair("scriptSig", HexStr(txin.scriptSig.begin(), txin.scriptSig.end())));
...
UniValue combinerawtransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"txs\"         (string) A json array of hex strings of partially "
        "signed transactions\n"
        "    [\n"
  ...
  CCoinsViewCache view(&viewDummy);
  {
    ...
    for (const CTxIn &txin : mergedTx.vin) {
      view.AccessCoin(
          txin.prevout); // Load entries from viewChain into view; can fail.
    }
...
UniValue signrawtransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "     [               (json array of json objects, or 'null' if none "
        "provided)\n"
        "       {\n"
        ...
        "         \"redeemScript\": \"hex\",   (string, required for P2SH or "
        "P2WSH) redeem script\n"
        "         \"amount\": value            (numeric, required) The amount "
        "spent\n"
        "       }\n"
        "       ,...\n"
        "    ]\n"
  ...
  for (unsigned int i = 0; i < mtx.vin.size(); i++) {
    ...
    if (!fHashSingle || (i < mtx.vout.size()))
      ProduceSignature(MutableTransactionSignatureCreator(&keystore, &mtx, i,
                                                          amount, nHashType),
                       prevPubKey, sigdata);
    sigdata = CombineSignatures(
        prevPubKey, TransactionSignatureChecker(&txConst, i, amount), sigdata,
...
UniValue sendrawtransaction(const JSONRPCRequest &request) {
  ...
  if (!fHaveMempool && !fHaveChain) {
    ...
    if (!AcceptToMemoryPool(mempool, state, std::move(tx), fLimitFree,
                            &fMissingInputs, nullptr, false, nMaxRawTxFee)) {
      if (state.IsInvalid()) {
...
static const CRPCCommand commands[] = {
    ...
    {"rawtransactions",
     ...
     &signrawtransaction,
     false,
     {"hexstring", "prevtxs", "privkeys",
      "sighashtype"}}, /* uses wallet if enabled */
