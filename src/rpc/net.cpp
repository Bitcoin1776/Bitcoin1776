...
UniValue getpeerinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "    \"addr\":\"host:port\",      (string) The IP address and port of "
        "the peer\n"
        "    \"addrbind\":\"ip:port\",    (string) Bind address of the "
        "connection to the peer\n"
        "    \"addrlocal\":\"ip:port\",   (string) Local address as reported "
        "by the peer\n"
        "    \"services\":\"xxxxxxxxxxxxxxxx\",   (string) The services "
        "offered\n"
        "    \"relaytxes\":true|false,    (boolean) Whether peer has asked us "
        "to relay transactions to it\n"
        "    \"lastsend\": ttt,           (numeric) The time in seconds since "
        "epoch (Jan 1 1970 GMT) of the last send\n"
        "    \"lastrecv\": ttt,           (numeric) The time in seconds since "
        "epoch (Jan 1 1970 GMT) of the last receive\n"
        ...
        "    \"version\": v,              (numeric) The peer version, such as "
        "7001\n"
        "    \"subver\": \"/Satoshi:0.8.5/\",  (string) The string version\n"
        "    \"inbound\": true|false,     (boolean) Inbound (true) or Outbound "
        "(false)\n"
        "    \"addnode\": true|false,     (boolean) Whether connection was due "
        "to addnode and is using an addnode slot\n"
        ...
        "    \"synced_blocks\": n,        (numeric) The last block we have in "
        "common with this peer\n"
        "    \"inflight\": [\n"
        ...
        "    },\n"
        "    \"bytesrecv_per_msg\": {\n"
        "       \"addr\": n,              (numeric) The total bytes received "
        "aggregated by message type\n"
        "       ...\n"
        "    }\n"
        "  }\n"
...
UniValue disconnectnode(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() == 0 || request.params.size() >= 3)
    throw std::runtime_error(
        ...
        "node\n"
        "2. \"nodeid\"      (number, optional) The node ID (see getpeerinfo "
        "for node IDs)\n"
        "\nExamples:\n" +
...
UniValue getaddednodeinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "or name (as provided to addnode)\n"
        "    \"connected\" : true|false,          (boolean) If connected\n"
        "    \"addresses\" : [                    (list of objects) Only when "
        "connected = true\n"
        "       {\n"
        "         \"address\" : \"192.168.0.201:8333\",  (string) The "
        "jfkbitcoin1776 server IP and port we're connected to\n"
        "         \"connected\" : \"outbound\"           (string) connection, "
        "inbound or outbound\n"
        "       }\n"
        "     ]\n"
        "  }\n"
...
UniValue getnettotals(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 0)
    throw std::runtime_error(
        ...
        "  \"uploadtarget\":\n"
        "  {\n"
        "    \"timeframe\": n,                         (numeric) Length of the "
        "measuring timeframe in seconds\n"
        ...
        "    \"bytes_left_in_cycle\": t,               (numeric) Bytes left in "
        "current time cycle\n"
        "    \"time_left_in_cycle\": t                 (numeric) Seconds left "
        "in current time cycle\n"
        "  }\n"
        "}\n"
        "\nExamples:\n" +
  ...
  outboundLimit.push_back(
      Pair("serve_historical_blocks", !g_connman->OutboundTargetReached(true)));
  outboundLimit.push_back(
...
static UniValue GetNetworksInfo() {
  ...
  for (int n = 0; n < NET_MAX; ++n) {
    ...
    obj.push_back(
        Pair("proxy",
             proxy.IsValid() ? proxy.proxy.ToStringIPPort() : std::string()));
    obj.push_back(
...
UniValue getnetworkinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "version\n"
        "  \"localservices\": \"xxxxxxxxxxxxxxxx\", (string) the services we "
        "offer to the network\n"
        "  \"localrelay\": true|false,              (bool) true if transaction "
        "relay is requested from peers\n"
        "  \"timeoffset\": xxxxx,                   (numeric) the time offset\n"
        "  \"connections\": xxxxx,                  (numeric) the number of "
        "connections\n"
        "  \"networkactive\": true|false,           (bool) whether p2p "
        "networking is enabled\n"
        "  \"networks\": [                          (array) information per "
        "network\n"
        "  {\n"
        "    \"name\": \"xxx\",                     (string) network (ipv4, "
        "ipv6 or onion)\n"
        "    \"limited\": true|false,               (boolean) is the network "
        "limited using -onlynet?\n"
        "    \"reachable\": true|false,             (boolean) is the network "
        "reachable?\n"
        "    \"proxy\": \"host:port\"               (string) the proxy that is "
        "used for this network, or empty if none\n"
        "    \"proxy_randomize_credentials\": true|false,  (string) Whether "
        "randomized credentials are used\n"
        "  }\n"
        "  ,...\n"
        "  ],\n"
        "  \"relayfee\": x.xxxxxxxx,                (numeric) minimum relay "
        "fee for transactions in " +
        CURRENCY_UNIT + "/kB\n"
                        "  \"incrementalfee\": x.xxxxxxxx,          (numeric) "
                        "minimum fee increment for mempool limiting or BIP 125 "
                        "replacement in " +
        CURRENCY_UNIT +
        "/kB\n"
        "  \"localaddresses\": [                    (array) list of local "
        "addresses\n"
        "  {\n"
...
UniValue setban(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 ||
    ...
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"subnet\"       (string, required) The IP/Subnet (see getpeerinfo "
        "for nodes IP) with an optional netmask (default is /32 = single IP)\n"
        "2. \"command\"      (string, required) 'add' to add an IP/Subnet to "
        "the list, 'remove' to remove an IP/Subnet from the list\n"
        "3. \"bantime\"      (numeric, optional) time in seconds how long (or "
        "until when if [absolute] is set) the IP is banned (0 or empty means "
        "using the default time of 24h which can also be overwritten by the "
        "-bantime startup argument)\n"
        "4. \"absolute\"     (boolean, optional) If set, the bantime must be "
        "an absolute timestamp in seconds since epoch (Jan 1 1970 GMT)\n"
        "\nExamples:\n" +
  ...
  if (strCommand == "add") {
    ...
    if (!(isSubnet ? g_connman->Unban(subNet) : g_connman->Unban(netAddr)))
      throw JSONRPCError(RPC_CLIENT_INVALID_IP_OR_SUBNET,
                         "Error: Unban failed. Requested address/subnet was "
