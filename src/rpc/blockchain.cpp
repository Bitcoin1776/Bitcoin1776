...
UniValue waitfornewblock(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. timeout (int, optional, default=0) Time in milliseconds to wait "
...
UniValue waitforblock(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "}\n"
        "\nExamples:\n" +
        HelpExampleCli("waitforblock", "\"0000000000079f8ef3d2c688c244eb7a4570b"
                                       "24c9ed7b4a8c619eb02596f8862\", 1000") +
        HelpExampleRpc("waitforblock", "\"0000000000079f8ef3d2c688c244eb7a4570b"
                                       "24c9ed7b4a8c619eb02596f8862\", 1000"));
  int timeout = 0;
  ...
  CUpdatedBlock block;
  {
    ...
    if (timeout)
      cond_blockchange.wait_for(
          ...
          [&hash] { return latestblock.hash == hash || !IsRPCRunning(); });
    else
...
std::string EntryDescriptionString() {
  return "    \"size\" : n,             (numeric) virtual transaction size as "
         "defined in BIP 141. This is different from actual serialized size "
         ...
         "in-mempool ancestors (including this one)\n"
         "    \"depends\" : [           (array) unconfirmed transactions used "
         "as inputs for this transaction\n"
...
UniValue getrawmempool(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "mempool.\n"
        "\nArguments:\n"
        "1. verbose (boolean, optional, default=false) True for a json object, "
...
UniValue getmempoolancestors(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
    throw std::runtime_error(
        ...
        "(must be in mempool)\n"
        "2. verbose                  (boolean, optional, default=false) True "
        "for a json object, false for array of transaction ids\n"
        "\nResult (for verbose=false):\n"
...
UniValue getmempoolentry(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 1) {
    throw std::runtime_error("getmempoolentry txid\n"
...
UniValue getblockheader(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "provided)\n"
        "  \"confirmations\" : n,   (numeric) The number of confirmations, or "
        "-1 if the block is not on the main chain\n"
        "  \"height\" : n,          (numeric) The block height or index\n"
        "  \"version\" : n,         (numeric) The block version\n"
        "  \"versionHex\" : \"00000000\", (string) The block version formatted "
        "in hexadecimal\n"
        "  \"merkleroot\" : \"xxxx\", (string) The merkle root\n"
        "  \"time\" : ttt,          (numeric) The block time in seconds since "
        "epoch (Jan 1 1970 GMT)\n"
        "  \"mediantime\" : ttt,    (numeric) The median block time in seconds "
        ...
        "previous block\n"
        "  \"nextblockhash\" : \"hash\",      (string) The hash of the next "
        "block\n"
        "}\n"
        "\nResult (for verbose=false):\n"
        ...
        "\nExamples:\n" +
        HelpExampleCli("getblockheader", "\"00000000c937983704a73af28acdec37b04"
                                         "9d214adbda81d7e2a3dd146f6ed09\"") +
        HelpExampleRpc("getblockheader", "\"00000000c937983704a73af28acdec37b04"
                                         "9d214adbda81d7e2a3dd146f6ed09\""));
...
UniValue getblock(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "encoded data, 1 for a json object, and 2 for json object with "
        "transaction data\n"
        ...
        "  \"strippedsize\" : n,    (numeric) The block size excluding witness "
        "data\n"
        "  \"weight\" : n           (numeric) The block weight as defined in "
        "BIP 141\n"
        "  \"height\" : n,          (numeric) The block height or index\n"
        "  \"version\" : n,         (numeric) The block version\n"
        "  \"versionHex\" : \"00000000\", (string) The block version formatted "
        ...
        "  \"tx\" : [               (array of Objects) The transactions in the "
        "format of the getrawtransaction RPC. Different from verbosity = 1 "
        "\"tx\" result.\n"
        "         ,...\n"
        "  ],\n"
        "  ,...                     Same output as verbosity = 1.\n"
        "}\n"
        "\nExamples:\n" +
        HelpExampleCli("getblock", "\"00000000c937983704a73af28acdec37b049d214a"
                                   "dbda81d7e2a3dd146f6ed09\"") +
        HelpExampleRpc("getblock", "\"00000000c937983704a73af28acdec37b049d214a"
                                   "dbda81d7e2a3dd146f6ed09\""));
...
static void ApplyStats(CCoinsStats &stats, CHashWriter &ss, const uint256 &hash,
  ...
  ss << hash;
  ss << VARINT(outputs.begin()->second.nHeight * 2 +
               outputs.begin()->second.fCoinBase);
  stats.nTransactions++;
  for (const auto output : outputs) {
    ...
    stats.nBogoSize += 32 /* txid */ + 4 /* vout index */ +
                       4 /* height + coinbase */ + 8 /* amount */ +
                       2 /* scriptPubKey len */ +
                       output.second.out.scriptPubKey.size() /* scriptPubKey */;
  }
  ss << VARINT(0);
}
...
UniValue pruneblockchain(const JSONRPCRequest &request) {
  ...
  if (chainHeight < Params().PruneAfterHeight())
    ...
    LogPrint(BCLog::RPC, "Attempt to prune blocks close to the tip.  Retaining "
...
UniValue gettxoutsetinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "transactions\n"
        "  \"bogosize\": n,          (numeric) A meaningless metric for UTXO "
        "set size\n"
...
UniValue gettxout(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    throw std::runtime_error(
        ...
        "     Note that an unspent output that is spent in the mempool won't "
        "appear.\n"
        "\nResult:\n"
        "{\n"
  ...
  if (coin.nHeight == MEMPOOL_HEIGHT) {
    ...
    ret.push_back(
        Pair("confirmations", (int64_t)(pindex->nHeight - coin.nHeight + 1)));
  }
...
UniValue getblockchaininfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "  \"blocks\": xxxxxx,         (numeric) the current number of blocks "
        "processed in the server\n"
        "  \"headers\": xxxxxx,        (numeric) the current number of headers "
        "we have validated\n"
        "  \"bestblockhash\": \"...\", (string) the hash of the currently best "
        "block\n"
        "  \"difficulty\": xxxxxx,     (numeric) the current difficulty\n"
        "  \"mediantime\": xxxxxx,     (numeric) median time for the current "
        "best block\n"
        "  \"verificationprogress\": xxxx, (numeric) estimate of verification "
        "progress [0..1]\n"
        "  \"chainwork\": \"xxxx\"     (string) total amount of work in active "
        "chain, in hexadecimal\n"
        "  \"pruned\": xx,             (boolean) if the blocks are subject to "
        "pruning\n"
        "  \"pruneheight\": xxxxxx,    (numeric) lowest-height complete block "
        "stored\n"
        ...
        "        \"reject\": {            (object) progress toward rejecting "
        "pre-softfork blocks\n"
        "           \"status\": xx,       (boolean) true if threshold reached\n"
        "        },\n"
        "     }, ...\n"
        "  ],\n"
        "  \"bip9_softforks\": {          (object) status of BIP9 softforks in "
        "progress\n"
        ...
        "\"started\" status)\n"
        "        \"startTime\": xx,       (numeric) the minimum median time "
        "past of a block at which the bit gains its meaning\n"
        "        \"timeout\": xx,         (numeric) the median time past of a "
        "block at which the deployment is considered failed if not yet locked "
        "in\n"
        "        \"since\": xx,           (numeric) height of the first block "
        "to which the status applies\n"
        "        \"statistics\": {        (object) numeric statistics about "
        "BIP9 signalling for a softfork (only for \"started\" status)\n"
        "           \"period\": xx,       (numeric) the length in blocks of "
        "the BIP9 signalling period \n"
        "           \"threshold\": xx,    (numeric) the number of blocks with "
        "the version bit set required to activate the feature \n"
        "           \"elapsed\": xx,      (numeric) the number of blocks "
        "elapsed since the beginning of the current period \n"
        "           \"count\": xx,        (numeric) the number of blocks with "
        "the version bit set in the current period \n"
        "           \"possible\": xx      (boolean) returns false if there are "
        "not enough blocks left in this period to pass activation threshold \n"
        "        }\n"
        "     }\n"
        "  }\n"
        "}\n"
  ...
  BIP9SoftForkDescPushBack(bip9_softforks, "segwit", consensusParams,
...
UniValue getchaintips(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "    \"height\": xxxx,\n"
        "    \"hash\": \"xxxx\",\n"
        "    \"branchlen\": 1          (numeric) length of branch connecting "
        "the tip to the main chain\n"
        "    \"status\": \"xxxx\"        (string) status of the chain (active, "
        ...
        "4.  \"valid-fork\"            This branch is not part of the active "
        "chain, but is fully validated\n"
   ...
   * Algorithm:
   *  - Make one pass through mapBlockIndex, picking out the orphan blocks, and
   * also storing a set of the orphan block's pprev pointers.
  ...
  for (const CBlockIndex *block : setTips) {
    ...
    if (chainActive.Contains(block)) {
      ...
      // This block cannot be connected because full block data for it or one of
      // its parents is missing.
      status = "headers-only";
    } else if (block->IsValid(BLOCK_VALID_SCRIPTS)) {
      // This block is fully validated, but no longer part of the active chain.
      // It was probably the active block once, but was reorganized.
      status = "valid-fork";
    } else if (block->IsValid(BLOCK_VALID_TREE)) {
      // The headers for this block are valid, but it has not been validated. It
      // was probably never part of the most-work chain.
...
UniValue getmempoolinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "  \"bytes\": xxxxx,              (numeric) Sum of all virtual "
        "transaction sizes as defined in BIP 141. Differs from actual "
        "serialized size because witness data is discounted\n"
        "  \"usage\": xxxxx,              (numeric) Total memory usage for the "
        "mempool\n"
        "  \"maxmempool\": xxxxx,         (numeric) Maximum memory usage for "
        "the mempool\n"
...
UniValue preciousblock(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"blockhash\"   (string, required) the hash of the block to mark "
        "as precious\n"
        "\nResult:\n"
        "\nExamples:\n" +
...
UniValue getchaintxstats(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "in the chain.\n"
        "\nArguments:\n"
        "1. nblocks      (numeric, optional) Size of the window in number of "
        "blocks (default: one month).\n"
        "2. \"blockhash\"  (string, optional) The hash of the block that ends "
        "the window.\n"
        "\nResult:\n"
        "{\n"
        "  \"time\": xxxxx,        (numeric) The timestamp for the statistics "
        "in UNIX format.\n"
        "  \"txcount\": xxxxx,     (numeric) The total number of transactions "
        "in the chain up to that point.\n"
        "  \"txrate\": x.xx,       (numeric) The average rate of transactions "
  ...
  int blockcount =
      30 * 24 * 60 * 60 /
      Params().GetConsensus().nPowTargetSpacing; // By default: 1 month
