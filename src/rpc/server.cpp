...
std::string CRPCTable::help(const std::string &strCommand,
...
UniValue stop(const JSONRPCRequest &jsonRequest) {
  ...
  if (jsonRequest.fHelp || jsonRequest.params.size() > 1)
    throw std::runtime_error("stop\n"
                             "\nStop JFKBitcoin1776 server.");
  // Event loop will exit after current HTTP requests have been handled, so
  // this reply will get back to the client.
  StartShutdown();
  return "JFKBitcoin1776 server stopping";
}
...
UniValue uptime(const JSONRPCRequest &jsonRequest) {
  if (jsonRequest.fHelp || jsonRequest.params.size() > 1)
    throw std::runtime_error("uptime\n"
                             ...
                             "\nResult:\n"
                             "ttt        (numeric) The number of seconds that "
...
transformNamedArguments(const JSONRPCRequest &in,
  ...
  // that we can throw a focused error if
...
std::string HelpExampleCli(const std::string &methodname,
  ...
  return "> jfkbitcoin1776-cli " + methodname + " " + args + "\n";
}
