...
#ifndef JFKBITCOIN1776_ADDRMAN_H
#define JFKBITCOIN1776_ADDRMAN_H
...
public:
   ...
   *
   * 2**30 is xorred with the number of buckets to make addrman deserializer v0
   * detect it
   * as incompatible. This is necessary because it did not check the version
   * number on
   * deserialization.
   *
  ...
  template <typename Stream> void Unserialize(Stream &s) {
    ...
    for (int n = 0; n < nNew; n++) {
      ...
      if (nVersion != 1 || nUBuckets != ADDRMAN_NEW_BUCKET_COUNT) {
        // In case the new table data cannot be used (nVersion unknown, or
        // bucket count wrong),
        // immediately try to give them a reference based on their primary
        // source address.
...
#endif // JFKBITCOIN1776_ADDRMAN_H
