...
static bool InitHTTPAllowList() {
  ...
  rpc_allow_subnets.push_back(
      CSubNet(localv4, 8)); // always allow IPv4 local subnet
  ...
  for (const std::string &strAllow : gArgs.GetArgs("-rpcallowip")) {
    ...
    if (!subnet.IsValid()) {
      uiInterface.ThreadSafeMessageBox(
          strprintf("Invalid -rpcallowip subnet specification: %s. Valid are a "
...
static bool ThreadHTTP(struct event_base *base, struct evhttp *http) {
  RenameThread("jfkbitcoin1776-http");
...
static bool HTTPBindAddresses(struct evhttp *http) {
  ...
  if (!gArgs.IsArgSet(
          "-rpcallowip")) { // Default to loopback if not allowing external IPs
    endpoints.push_back(std::make_pair("::1", defaultPort));
    endpoints.push_back(std::make_pair("127.0.0.1", defaultPort));
    if (gArgs.IsArgSet("-rpcbind")) {
      LogPrintf("WARNING: option -rpcbind was ignored because -rpcallowip was "
                "not specified, refusing to allow everyone to connect\n");
    }
...
static void HTTPWorkQueueRun(WorkQueue<HTTPClosure> *queue) {
  RenameThread("jfkbitcoin1776-httpworker");
  queue->Run();
}
...
#endif
  if (severity >=
      EVENT_LOG_WARN) // Log warn messages and higher without debug category
...
bool InitHTTPServer() {
  ...
  int workQueueDepth =
      std::max((long)gArgs.GetArg("-rpcworkqueue", DEFAULT_HTTP_WORKQUEUE), 1L);
...
void StopHTTPServer() {
  ...
  if (eventBase) {
    ...
    // Give event loop a few seconds to exit (to send back last RPC responses),
    // then break it
    // Before this was solved with event_base_loopexit, but that didn't work as
    // expected in
    // at least libevent 2.0.21 and always introduced a delay. In libevent
    // master that appears to be solved, so in the future that solution
    // could be used again (if desirable).
    // (see discussion in
    // https://github.com/jfkbitcoin1776/jfkbitcoin1776/pull/6990)
    if (threadResult.valid() &&
        threadResult.wait_for(std::chrono::milliseconds(2000)) ==
            std::future_status::timeout) {
      LogPrintf("HTTP event loop did not exit within allotted time, sending "
                "loopbreak\n");
