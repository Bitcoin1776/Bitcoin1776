 ...
 * S> <S> <hashtype>
 * Where R and S are not negative (their first byte has its highest bit not
 * set), and not
 * excessively padded (do not start with a 0 byte, unless an otherwise negative
 * number follows,
 * in which case a single 0 byte is necessary and even required).
 *
 * See https://jfkbitcoin1776talk.org/index.php?topic=8392.msg127623#msg127623
 *
...
bool static CheckMinimalPush(const valtype &data, opcodetype opcode) {
  if (data.size() == 0) {
    ...
    // Could have used a direct push (opcode indicating number of bytes pushed +
    // those bytes).
    ...
    while (pc < pend) {
      ...
      if (opcode == OP_CAT || opcode == OP_SUBSTR || opcode == OP_LEFT ||
        ...
        return set_error(serror,
                         SCRIPT_ERR_DISABLED_OPCODE); // Disabled opcodes.
      ...
      if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4) {
        ...
        case OP_CHECKMULTISIGVERIFY: {
          ...
          if (nOpCount > MAX_OPS_PER_SCRIPT)
            return set_error(serror, SCRIPT_ERR_OP_COUNT);
          int ikey = ++i;
          // ikey2 is the position of last non-signature item in the stack. Top
          // stack item = 1.
          // With SCRIPT_VERIFY_NULLFAIL, this is used for cleanup if operation
          // fails.
          ...
          while (fSuccess && nSigsCount > 0) {
            ...
            // Note how this makes the exact order of pubkey/signature
            // evaluation
            // distinguishable by CHECKMULTISIG NOT if the STRICTENC flag is
            // set.
          ...
          while (i-- > 1) {
            // If the operation failed, we require that all signatures must be
            // empty vector
...
private:
  const CTransaction &txTo;  //!< reference to the spending transaction (the one
                             //!being serialized)
  const CScript &scriptCode; //!< output script being consumed
  const unsigned int nIn;    //!< input index of txTo being signed
  const bool fAnyoneCanPay;  //!< whether the hashtype has the
...
static bool VerifyWitnessProgram(const CScriptWitness &witness, int witversion,
  ...
  if (witversion == 0) {
    if (program.size() == 32) {
      ...
      scriptPubKey =
          CScript(witness.stack.back().begin(), witness.stack.back().end());
      ...
      if (witness.stack.size() != 2) {
        return set_error(
            serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH); // 2 items in witness
      }
...
bool VerifyScript(const CScript &scriptSig, const CScript &scriptPubKey,
  ...
  if (flags & SCRIPT_VERIFY_WITNESS) {
    if (scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
      hadWitness = true;
      if (scriptSig.size() != 0) {
        // The scriptSig must be _exactly_ CScript(), otherwise we reintroduce
        // malleability.
        return set_error(serror, SCRIPT_ERR_WITNESS_MALLEATED);
      }
      if (!VerifyWitnessProgram(*witness, witnessversion, witnessprogram, flags,
                                checker, serror)) {
        return false;
      }
      // Bypass the cleanstack check at the end. The actual stack is obviously
      // not clean
  ...
  if (flags & SCRIPT_VERIFY_WITNESS) {
    // We can't check for correct unexpected witness data if P2SH was off, so
    // require
    // that WITNESS implies P2SH. Otherwise, going from WITNESS->P2SH+WITNESS
    // would be
...
size_t static WitnessSigOps(int witversion,
                            const std::vector<unsigned char> &witprogram,
                            const CScriptWitness &witness, int flags) {
  if (witversion == 0) {
    if (witprogram.size() == 20)
      return 1;

    if (witprogram.size() == 32 && witness.stack.size() > 0) {
      CScript subscript(witness.stack.back().begin(),
...
size_t CountWitnessSigOps(const CScript &scriptSig, const CScript &scriptPubKey,
                          const CScriptWitness *witness, unsigned int flags) {
  ...
  std::vector<unsigned char> witnessprogram;
  if (scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
    return WitnessSigOps(witnessversion, witnessprogram,
                         witness ? *witness : witnessEmpty, flags);
  }
