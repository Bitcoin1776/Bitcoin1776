...
#ifndef JFKBITCOIN1776_CHAIN_H
#define JFKBITCOIN1776_CHAIN_H
...
enum BlockStatus : uint32_t {
  ...
  //! previous, checkpoint. Implies all parents
  //! are also at least TREE.
  BLOCK_VALID_TREE = 2,

  /**
   * Only first tx is coinbase, 2 <= coinbase input script length <= 100,
   * transactions valid, no duplicate txids,
   * sigops, size, merkle root. Implies all parents are at least TREE but not
   * necessarily TRANSACTIONS. When all
  ...
  BLOCK_OPT_WITNESS = 128, //!< block data in blk*.data was received with a
                           //!witness-enforcing client
};

/** The block chain is a tree shaped structure starting with the
 * genesis block at the root, with each block potentially having multiple
 * candidates to be the next block. A blockindex may have multiple pprev
 * pointing
...
public:
  ...
  //! (memory only) Sequential id assigned to distinguish order in which blocks
  //! are received.
  int32_t nSequenceId;
...
public:
  /** Returns the index entry for the genesis block of this chain, or nullptr if
   * none. */
  ...
  /** Find the successor of a block in this chain, or nullptr if the given index
