 ...
 * their
 * approximate feerate and then track how long it took for those txs to be
 * included in a block
 *
...
private:
  ...
  const std::vector<double>
      &buckets; // The upper-bound of the range for the bucket (inclusive)
  const std::map<double, unsigned int> &bucketMap; // Map of bucket upper-bound
                                                   // to index into all vectors
                                                   // by bucket
  ...
  // Combine the conf counts with tx counts to calculate the confirmation % for
  // each Y,X
  // Combine the total value with the tx counts to calculate the avg feerate per
  // bucket
...
public:
   ...
   * buckets
   * to make sure we have enough data points) whose transactions still have
   * sufficient likelihood
   * of being confirmed within the target number of confirmations
   * @param confTarget target number of confirmations
   * @param sufficientTxVal required average number of transactions per block in
   * a bucket range
   * @param minSuccess the success probability we require
   * @param requireGreater return the lowest feerate such that all higher values
   * pass minSuccess OR
...
void TxConfirmStats::resizeInMemoryCounters(size_t newbuckets) {
  // newbuckets must be passed in because the buckets referred to during Read
...
double TxConfirmStats::EstimateMedianVal(int confTarget, double sufficientTxVal,
  ...
  double failNum = 0; // Number of tx's that were never confirmed but removed
                      // from the mempool after confTarget
  ...
  // higher
  // values pass, so we start at maxbucketindex (highest feerate) and look at
  // successively
  // smaller buckets until we reach failure.  Otherwise, we are looking for the
  // highest
  // feerate such that all lower values fail, and we go in the opposite
  // direction.
  ...
  // conditions
  // Find the bucket with the median transaction and then report the average
  // feerate from that bucket
  // This is a compromise between finding the median which we can't since we
  // don't save all tx's
  ...
  // If we were passing until we reached last few buckets with insufficient
  // data, then report those as failed
  ...
  LogPrint(
      BCLog::ESTIMATEFEE, "FeeEst: %d %s%.0f%% decay %.5f: feerate: %g from "
                          "(%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out) Fail: "
                          "(%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out)\n",
      ...
      passBucket.leftMempool, failBucket.start, failBucket.end,
...
void TxConfirmStats::Read(CAutoFile &filein, int nFileVersion,
  ...
  // The current version will store the decay with each individual
  // TxConfirmStats and also keep a scale factor
  if (nFileVersion >= 149900) {
    filein >> decay;
    if (decay <= 0 || decay >= 1) {
      throw std::runtime_error("Corrupt estimates file. Decay must be between "
...
void TxConfirmStats::removeTx(unsigned int entryHeight,
  ...
  if (blocksAgo < 0) {
    ...
    return; // This can't happen because we call this with our best seen height,
            // no entries can have higher
  }
  ...
  if (blocksAgo >= (int)unconfTxs.size()) {
    if (oldUnconfTxs[bucketindex] > 0) {
      ...
      LogPrint(BCLog::ESTIMATEFEE, "Blockpolicy error, mempool tx removed from "
                                   ">25 blocks,bucketIndex=%u already\n",
               bucketindex);
    }
  } else {
  ...
  if (!inBlock &&
      (unsigned int)blocksAgo >= scale) { // Only counts as a failure if not
                                          // confirmed for entire period
...
CBlockPolicyEstimator::CBlockPolicyEstimator()
  ...
  feeStats = new TxConfirmStats(buckets, bucketMap, MED_BLOCK_PERIODS,
                                MED_DECAY, MED_SCALE);
  shortStats = new TxConfirmStats(buckets, bucketMap, SHORT_BLOCK_PERIODS,
                                  SHORT_DECAY, SHORT_SCALE);
  longStats = new TxConfirmStats(buckets, bucketMap, LONG_BLOCK_PERIODS,
                                 LONG_DECAY, LONG_SCALE);
}
...
void CBlockPolicyEstimator::processTransaction(const CTxMemPoolEntry &entry,
  ...
  // Feerates are stored and reported as JFK-per-kb:
...
bool CBlockPolicyEstimator::processBlockTx(unsigned int nBlockHeight,
  ...
  if (blocksToConfirm <= 0) {
    // This can't happen because we don't process transactions from a block with
    // a height
...
void CBlockPolicyEstimator::processBlock(
  ...
  LogPrint(BCLog::ESTIMATEFEE, "Blockpolicy estimates updated by %u of %u "
                               "block txs, since last block %u of %u tracked, "
                               "mempool map size %u, max target %u from %s\n",
...
unsigned int CBlockPolicyEstimator::MaxUsableEstimate() const {
  // Block spans are divided by 2 to make sure there are enough potential
  // failing data points for the estimate
...
double CBlockPolicyEstimator::estimateCombinedFee(
  ...
  if (confTarget >= 1 && confTarget <= longStats->GetMaxConfirms()) {
    ...
    if (checkShorterHorizon) {
      ...
      // If a lower confTarget from a more recent horizon returns a lower answer
      // use it.
...
bool CBlockPolicyEstimator::Read(CAutoFile &filein) {
  try {
    ...
    if (nVersionThatWrote < 149900) {
      // Read the old fee estimates file for temporary use, but then discard.
      // Will start collecting data from scratch.
      // decay is stored before buckets in old versions, so pre-read decay and
      // pass into TxConfirmStats constructor
      double tempDecay;
      filein >> tempDecay;
      ...
      std::unique_ptr<TxConfirmStats> tempFeeStats(new TxConfirmStats(
          tempBuckets, tempMap, MED_BLOCK_PERIODS, tempDecay, 1));
      tempFeeStats->Read(filein, nVersionThatWrote, tempNum);
      // if nVersionThatWrote < 139900 then another TxConfirmStats (for
      // priority) follows but can be ignored.
      ...
      // Destroy old TxConfirmStats and point to new ones that already reference
      // buckets and bucketMap
      delete feeStats;
      delete shortStats;
      delete longStats;
