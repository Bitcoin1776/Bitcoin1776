...
#ifndef JFKBITCOIN1776_POLICY_POLICY_H
#define JFKBITCOIN1776_POLICY_POLICY_H
...
/** Default for -blockmaxweight, which controls the range of block weights the
 *mining code will create
 ** Note: we use a #define here, because MAX_BLOCK_WEIGHT changes after hard
 *forking. It can be replaced with
 ** a static const unsigned int set to the actual new block weight - 4000 once
 *the fork has occurred.
 **/
#define DEFAULT_BLOCK_MAX_WEIGHT (MAX_BLOCK_WEIGHT - 4000)
 ...
 * 3600bytes witnessScript size, 80bytes per witness stack element, 100 witness
 * stack elements
 * These limits are adequate for multi-signature up to n-of-100 using
 * OP_CHECKSIG, OP_ADD, and OP_EQUAL,
 */
...
#endif // JFKBITCOIN1776_POLICY_POLICY_H
