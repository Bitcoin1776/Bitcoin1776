...
class CJFKBitcoin1776LevelDBLogger : public leveldb::Logger {
public:
  ...
  virtual void Logv(const char *format, va_list ap) override {
    ...
    for (int iter = 0; iter < 2; iter++) {
      ...
      if (p < limit) {
        ...
        // Do not use vsnprintf elsewhere in jfkbitcoin1776 source code, see
        // above.
...
static leveldb::Options GetOptions(size_t nCacheSize) {
  ...
  options.info_log = new CJFKBitcoin1776LevelDBLogger();
  if (leveldb::kMajorVersion > 1 ||
      (leveldb::kMajorVersion == 1 && leveldb::kMinorVersion >= 16)) {
    // LevelDB versions before 1.16 consider short writes to be corruption. Only
    // trigger error
