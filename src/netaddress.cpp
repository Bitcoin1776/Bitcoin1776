...
// 0xFD + sha256("jfkbitcoin1776")[0:5]
...
bool CNetAddr::SetInternal(const std::string &name) {
  ...
  CSHA256()
      .Write((const unsigned char *)name.data(), name.size())
      .Finalize(hash);
...
bool CNetAddr::IsValid() const {
  ...
  // header20 vectorlen3 addr26 addr26 addr26 header20 vectorlen3 addr26 addr26
  // addr26...
...
std::vector<unsigned char> CNetAddr::GetGroup() const {
  ...
  else if (GetByte(15) == 0x20 && GetByte(14) == 0x01 && GetByte(13) == 0x04 &&
           GetByte(12) == 0x70)
    nBits = 36;
  ...
  case NET_IPV6:
    ...
    case NET_IPV6:
      return fTunnel ? REACH_IPV6_WEAK
                     : REACH_IPV6_STRONG; // only prefer giving our IPv6 address
                                          // if it's not tunnelled
    }
  case NET_TOR:
    switch (ourNet) {
    default:
...
CSubNet::CSubNet(const CNetAddr &addr, int32_t mask) {
  ...
  int32_t n = mask;
  if (n >= 0 &&
      n <= (128 - astartofs * 8)) // Only valid if in range of bits of address
  {
    n += astartofs * 8;
    // Clear bits [n..127]
    for (; n < 128; ++n)
