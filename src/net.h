...
#ifndef JFKBITCOIN1776_NET_H
#define JFKBITCOIN1776_NET_H
...
public:
  ...
  const std::vector<CNode *> &GetNodeVectorHaphazardly() const {
    return vNodes;
  }
...
public:
  ...
  // strSubVer is whatever byte array we read from the wire. However, this field
  // is intended
  // to be printed out, displayed to humans in various forms and so on. So we
  // sanitize it and
  // store the sanitized version in cleanSubVer. The original should be used
  // when dealing with
  ...
  // message
  // b) the peer may tell us in its version message that we should not relay tx
  // invs
