...
static const uint64_t RANDOMIZER_ID_ADDRESS_RELAY =
    0x3cac0035b5866b90ULL; // SHA256("main address relay")[0:8]
...
struct QueuedBlock {
  ...
  bool fValidatedHeaders; //!< Whether this block has validated headers at the
                          //!time of request.
  std::unique_ptr<PartiallyDownloadedBlock>
      partialBlock; //!< Optional, used for CMPCTBLOCK downloads
};
...
struct CNodeState {
   ...
   * fSupportsDesiredCmpctVersion,
    * but is used as a flag to "lock in" the version of compact blocks
   * (fWantsCmpctWitness) we send.
    */
   ...
   * witnesses in cmpctblocks/blocktxns,
...
void PushNodeVersion(CNode *pnode, CConnman *connman, int64_t nTime) {
  ...
  if (fLogIPs) {
    LogPrint(BCLog::NET, "send version message: version %d, blocks=%d, us=%s, "
                         "them=%s, peer=%d\n",
...
void UpdateBlockAvailability(NodeId nodeid, const uint256 &hash) {
  ...
  if (it != mapBlockIndex.end() && it->second->nChainWork > 0) {
    ...
    // An unknown block was announced; just assume that the latest one is the
    // best one.
...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count,
  ...
  // Never fetch further than the best block we know the peer has, or more than
  // BLOCK_DOWNLOAD_WINDOW + 1 beyond the last
  // linked block we have in common with this peer. The +1 is so we can detect
  // stalling, namely if we would be able to
  ...
  while (pindexWalk->nHeight < nMaxHeight) {
    // Read up to 128 (or more, if more blocks than that are needed) successors
    // of pindexWalk (towards
    // pindexBestKnownBlock) into vToFetch. We fetch 128, because
    // CBlockIndex::GetAncestor may be as expensive
    ...
    // Iterate over those blocks in vToFetch (in forward direction), adding the
    // ones that
    // are not yet downloaded and not in flight to vBlocks. In the mean time,
    // update
    // pindexLastCommonBlock as long as all ancestors are already downloaded, or
    // if it's
      ...
      if (pindex->nStatus & BLOCK_HAVE_DATA || chainActive.Contains(pindex)) {
        ...
        if (pindex->nHeight > nWindowEnd) {
          ...
          if (vBlocks.size() == 0 && waitingfor != nodeid) {
            // We aren't able to fetch anything, but we would be if the download
            // window was one larger.
...
bool AddOrphanTx(const CTransactionRef &tx, NodeId peer)
  ...
  // at most 10 megabytes of orphans and somewhat more byprev index (in the
  // worst case):
...
void EraseOrphansFor(NodeId peer) {
  ...
  while (iter != mapOrphanTransactions.end()) {
    std::map<uint256, COrphanTx>::iterator maybeErase =
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans)
  ...
  if (nNextSweep <= nNow) {
    ...
    // Sweep again 5 minutes after the next entry that expires in order to batch
    // the linear scan.
...
bool static AlreadyHave(const CInv &inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
  ...
  case MSG_WITNESS_TX: {
    ...
    return recentRejects->contains(inv.hash) || mempool.exists(inv.hash) ||
           ...
           pcoinsTip->HaveCoinInCache(COutPoint(
               inv.hash, 0)) || // Best effort: only try output 0 and 1
      ...
      if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK ||
        ...
        if (mi != mapBlockIndex.end()) {
          if (mi->second->nChainTx &&
            ...
            // validated it,
            // we might be in the middle of connecting it (ie in the unlock of
            // cs_main
            // before ActivateBestChain but after AcceptBlock).
            // In this case, we need to run ActivateBestChain prior to checking
            // the relay
            // conditions below.
          ...
          if (chainActive.Contains(mi->second)) {
            ...
            // To prevent fingerprinting attacks, only send blocks outside of
            // the active
            // chain if they are valid, and no more than a month older (both in
            // time, and in
            // best equivalent proof of work) than the best header chain we know
            // about.
            send =
                mi->second->IsValid(BLOCK_VALID_SCRIPTS) &&
                ...
                (pindexBestHeader->GetBlockTime() - mi->second->GetBlockTime() <
                 nOneMonth) &&
                (GetBlockProofEquivalentTime(*pindexBestHeader, *mi->second,
                                             *pindexBestHeader,
                                             consensusParams) < nOneMonth);
            if (!send) {
              LogPrintf("%s: ignoring request from peer=%i for old block that "
        ...
        if (send && (mi->second->nStatus & BLOCK_HAVE_DATA)) {
          ...
          if (inv.type == MSG_BLOCK)
            ...
            if (sendMerkleBlock) {
              ...
              // transactions in the block the client did not see
              // This avoids hurting performance by pointlessly requiring a
              // round-trip
              // Note that there is currently no way for a node to request any
              // single transactions we didn't send here -
              // they must either disconnect and retry or request the full
              // block.
              // Thus, the protocol spec specified allows for us to provide
              // duplicate txn here,
              // however we MUST always provide at least what the remote peer
              // needs
            ...
            // they won't have a useful mempool to match against a compact
            // block,
            // and we don't feel like constructing the object for them, so
            // instead we respond with the full, non-compact block.
            bool fPeerWantsWitness = State(pfrom->GetId())->fWantsCmpctWitness;
            int nSendFlags =
                fPeerWantsWitness ? 0 : SERIALIZE_TRANSACTION_NO_WITNESS;
            if (CanDirectFetch(consensusParams) &&
                mi->second->nHeight >=
                    chainActive.Height() - MAX_CMPCTBLOCK_DEPTH) {
              if ((fPeerWantsWitness ||
                   !fWitnessesPresentInARecentCompactBlock) &&
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman,
  ...
  const CBlockIndex *pindexLast = nullptr;
  {
    ...
    if (mapBlockIndex.find(headers[0].hashPrevBlock) == mapBlockIndex.end() &&
      ...
      LogPrint(BCLog::NET, "received header %s: missing prev block %s, sending "
                           "getheaders (%d) to end (peer=%d, "
...

  {
    ...
    if (nCount == MAX_HEADERS_RESULTS) {
      ...
      // TODO: optimize: if pindexLast is an ancestor of chainActive.Tip or
      // pindexBestHeader, continue
      // from there instead.
      LogPrint(BCLog::NET,
    ...
    if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) &&
      ...
      // Calculate all the blocks we'd need to switch to pindexLast, up to a
      // limit.
      ...
      if (!chainActive.Contains(pindexWalk)) {
        ...
        if (vGetData.size() > 0) {
          if (nodestate->fSupportsDesiredCmpctVersion && vGetData.size() == 1 &&
            ...
            // In any case, we want to download using a compact block, not a
            // regular one
...
bool static ProcessMessage(CNode *pfrom, const std::string &strCommand,
  ...
  if (strCommand == NetMsgType::REJECT) {
    if (LogAcceptCategory(BCLog::NET)) {
      try {
        ...
        // Avoid feedback loops by preventing reject messages from triggering a
        // new reject message.
    ...
    if (pfrom->nServicesExpected & ~nServices) {
      LogPrint(BCLog::NET, "peer=%d does not offer the expected services (%08x "
                           "offered, %08x expected); disconnecting\n",
    ...
    if (nServices & ((1 << 7) | (1 << 5))) {
      if (GetTime() < 1533096000) {
        // Immediately disconnect peers that use service bits 6 or 8 until
        // August 1st, 2018
        // These bits have been used as a flag to indicate that a node is
        // running incompatible
        // consensus rules instead of changing the network magic, so we're stuck
        // disconnecting
    ...
    // If the peer is old enough to have the old alert system, send it the final
    // alert.
    if (pfrom->nVersion <= 70012) {
      CDataStream finalAlert(
          ParseHex("60010000000000000000000000ffffff7f00000000ffffff7ffeffff7f0"
                   "1ffffff7f00000000ffffff7f00ffffff7f002f555247454e543a20416c"
                   "657274206b657920636f6d70726f6d697365642c2075706772616465207"
                   "265717569726564004630440220653febd6410f470f6bae11cad19c4841"
                   "3becb1ac2c17f908fd0fd53bdc3abd5202206d0e9c96fe88d4a0f01ed9d"
                   "edae2b6f9e00da94cad0fecaae66ecf689bf71b50"),
  ...
  if (strCommand == NetMsgType::VERACK) {
    ...
    if (nCMPCTBLOCKVersion == 1 ||
      ...
      LOCK(cs_main);
      // fProvidesHeaderAndIDs is used to "lock in" version of compact blocks we
      // send (fWantsCmpctWitness)
    ...
    for (CInv &inv : vInv) {
      ...
      if (inv.type == MSG_BLOCK) {
        ...
        if (!fAlreadyHave && !fImporting && !fReindex &&
          ...
          // headers-announcements are now the
          // primary method of announcement on the network, and since, in the
          // case that a node
          // fell back to inv we probably have a reorg which we should get the
          // headers for first,
          // we now only provide a getheaders response here. When we receive the
          // headers, we will
    ...
    for (; pindex; pindex = chainActive.Next(pindex)) {
      ...
      // If pruning, don't inv blocks unless we have on disk and are likely to
      // still have
      // for some reasonable time window (1 hour) that block relay might
      // require.
    ...
    if (IsInitialBlockDownload() && !pfrom->fWhitelisted) {
      LogPrint(BCLog::NET, "Ignoring getheaders from peer=%d because node is "
    ...
    // we must use CBlocks, as CBlockHeaders won't include the 0x00 nTx count at
    // the end
    ...
    if (!AlreadyHave(inv) &&
      ...
      while (!vWorkQueue.empty()) {
        ...
        for (auto mi = itByPrev->second.begin(); mi != itByPrev->second.end();
          ...
          // counter-DoS based on orphan
          // resolution (that is, feeding people an invalid transaction based on
          // LegitTxX in order to get
          ...
          if (AcceptToMemoryPool(mempool, stateDummy, porphanTx, true,
            ...
            if (!orphanTx.HasWitness() && !stateDummy.CorruptionPossible()) {
              ...
              // See
              // https://github.com/jfkbitcoin1776/jfkbitcoin1776/issues/8279
              // for details.
              assert(recentRejects);
      ...
      bool fRejectedParents = false; // It may be the case that the orphans
                                     // parents have all been rejected
      for (const CTxIn &txin : tx.vin) {
      ...
      if (!tx.HasWitness() && !state.CorruptionPossible()) {
        // Do not use rejection cache for witness transactions or
        // witness-stripped transactions, as they can have been malleated.
        // See https://github.com/jfkbitcoin1776/jfkbitcoin1776/issues/8279 for
        // details.
        assert(recentRejects);
        recentRejects->insert(tx.GetHash());
    ...
    if (state.IsInvalid(nDoS)) {
      ...
      if (state.GetRejectCode() > 0 &&
          state.GetRejectCode() < REJECT_INTERNAL) // Never send
                                                   // AcceptToMemoryPool's
                                                   // internal codes over P2P
        connman->PushMessage(pfrom,
                             msgMaker.Make(NetMsgType::REJECT, strCommand,
                                           ...
                                           inv.hash));
      if (nDoS > 0) {
        // to avoid banning same-fork peers who haven't upgraded to the new
        // consensus rules yet (due to them
        // activating just now), we let them stay as long as we are currently on
        // the fork height
        if (FORK_BLOCK != chainActive.Tip()->nHeight ||
            pfrom->fhash != FORK_HASH_UINT256) {
          Misbehaving(pfrom->GetId(), nDoS);
        }
      }
    }
           ...
           !fReindex) // Ignore blocks received while importing
  {
...

    {
      ...
      // If we're not close to tip yet, give up and let parallel block fetch
      // work its magic
      ...
      if (pindex->nHeight <= chainActive.Height() + 2) {
        if ((!fAlreadyInFlight &&
          ...
          if (!MarkBlockAsInFlight(pfrom->GetId(), pindex->GetBlockHash(),
            ...
            if (!(*queuedBlockIt)->partialBlock)
              ...
              // The block was already in flight using compact blocks from the
              // same peer
              LogPrint(BCLog::NET,
          ...
          if (status == READ_STATUS_INVALID) {
            MarkBlockAsReceived(pindex->GetBlockHash()); // Reset in-flight
                                                         // state in case of
                                                         // whitelist
            Misbehaving(pfrom->GetId(), 100);
            ...
            // Duplicate txindexes, the block is now in-flight, so just request
            // it
            std::vector<CInv> vInv(1);
            vInv[0] = CInv(MSG_BLOCK | GetFetchFlags(pfrom),
                           cmpctblock.header.GetHash());
          ...
          if (req.indexes.empty()) {
            // Dirty hack to jump to BLOCKTXN code (TODO: move message handling
        ...
        if (fAlreadyInFlight) {
          ...
          // If this was an announce-cmpctblock, we want the same treatment as a
          // header message
           ...
           !fReindex) // Ignore blocks received while importing
  {
    ...
    bool fBlockRead = false;
    {
      ...
      if (it == mapBlocksInFlight.end() || !it->second.second->partialBlock ||
        ...
        LogPrint(BCLog::NET, "Peer %d sent us block transactions for block we "
                             "weren't expecting\n",
                 pfrom->GetId());
        return true;
      }
      ...
      if (status == READ_STATUS_INVALID) {
        MarkBlockAsReceived(
            resp.blockhash); // Reset in-flight state in case of whitelist
        Misbehaving(pfrom->GetId(), 100);
    ...
    if (fBlockRead) {
      bool fNewBlock = false;
      // Since we requested this block (it was in mapBlocksInFlight), force it
      // to be processed,
      // even if it would not be a candidate for new tip (missing previous
      // block, chain not long enough, etc)
           ...
           !fReindex) // Ignore blocks received while importing
  {
    ...
    // Making nodes which are behind NAT and can only make outgoing connections
    // ignore
    ...
    if (pfrom->nVersion > BIP0031_VERSION) {
      ...
      // 2) Remote nodes can measure the latency of the network thread. If this
      // node
      //    is overloaded it won't respond to pings quickly and the remote node
      //    can
      //    avoid sending us more work, like chain download requests.
      //
      // The nonce stops the remote getting confused between different pings:
      // without
      // it, if the remote node sends a ping once per second and this node takes
      // 5
      // seconds to respond to each, the 5th ping the remote sends would appear
      // to
      // return very quickly.
    ...
    if (nAvail >= sizeof(nonce)) {
      ...
      // Only process pong message if there is an outstanding ping (old ping
      // without nonce should never pong)
      ...
      // This is most likely a bug in another implementation somewhere; cancel
      // this ping
      bPingFinished = true;
    ...
    // Nodes must NEVER send a data item > 520 bytes (the max size for a script
    // data object,
    // and thus, the maximum size any matched object can have) in a filteradd
    // message
    bool bad = false;
...
void PeerLogicValidation::ConsiderEviction(CNode *pto,
  ...
  if (!state.m_chain_sync.m_protect && IsOutboundDisconnectionCandidate(pto) &&
    ...
    if (state.pindexBestKnownBlock != nullptr &&
      ...
      // noticing
      // that for the first time, OR this peer was able to catch up to some
      // earlier point
      ...
      // No evidence yet that our peer has synced to a chain with work equal to
      // that
      // of our tip, when we first detected it was behind. Send a single
      // getheaders
      ...
      if (state.m_chain_sync.m_sent_getheaders) {
        ...
        LogPrint(BCLog::NET, "sending getheaders to outbound peer=%d to verify "
                             "chain work (current best known block:%s, "
                             "benchmark blockhash: %s)\n",
                 pto->GetId(),
                 state.pindexBestKnownBlock != nullptr
                     ? state.pindexBestKnownBlock->GetBlockHash().ToString()
                     : "<none>",
  ...
  const Consensus::Params &consensusParams = Params().GetConsensus();
  {
    ...
    if (pingSend) {
      ...
      if (pto->nVersion > BIP0031_VERSION) {
        ...
        // Peer is too old to support ping command with nonce, pong will never
        // arrive.
    ...
    bool fFetch = state.fPreferredDownload ||
                  (nPreferredDownload == 0 && !pto->fClient &&
                   !pto->fOneShot); // Download if this is a nice peer, or we
                                    // have no nice peers and this one might do.
    if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
      // Only actively request headers from a single peer, unless we're close to
      // today.
    ...
    //
    {
      ...
      const CBlockIndex *pBestIndex =
          nullptr; // last header queued for delivery
      ProcessBlockAvailability(
          pto->GetId()); // ensure pindexBestKnownBlock is up-to-date
    ...
    std::vector<CInv> vInv;
    {
      ...
      if (fSendTrickle) {
        ...
        // priority reasons.
        // A heap is used so that not all items need sorting if only a few are
        // being sent.
        CompareInvMempoolOrder compareInvMempoolOrder(&mempool);
        std::make_heap(vInvTx.begin(), vInvTx.end(), compareInvMempoolOrder);
        // No reason to drain out at many times the network's capacity,
        // especially since we have many peers and some will draw much shorter
        // delays.
    ...
    if (state.nStallingSince &&
      ...
      // the download window should be much larger than the to-be-downloaded set
      // of blocks, so disconnection
    ...
    // 0.5 * N times the block interval
    // (with N the number of peers from which we're downloading validated
    // blocks), disconnect due to timeout.
    // We compensate for other peers to prevent killing off peers due to our own
    // downstream link
    // being saturated. We only count validated in-flight blocks so peers can't
    // advertise non-existing block hashes
    ...
    // GetTime() is used by this anti-DoS logic so we can test this using
    // mocktime
    ...
    if (pto->nVersion >= FEEFILTER_VERSION &&
      ...
      // until scheduled broadcast, then move the broadcast to within
      // MAX_FEEFILTER_CHANGE_DELAY.
