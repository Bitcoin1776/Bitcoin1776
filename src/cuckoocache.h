...
#ifndef _JFKBITCOIN1776_CUCKOOCACHE_H_
#define _JFKBITCOIN1776_CUCKOOCACHE_H_
...
private:
   ...
   * size) in a
   *  manner which preserves as much of the hash's uniformity as possible.
   * Ideally
   *  this would be done by bitmasking but the size is usually not a power of
   * two.
   *
   * The naive approach would be to use a mod -- which isn't perfectly uniform
   * but so
   *  long as the hash is much larger than size it is not that bad.
   * Unfortunately,
   *  mod/division is fairly slow on ordinary microprocessors (e.g. 90-ish
   * cycles on
   *  haswell, ARM doesn't even have an instruction for it.); when the divisor
   * is a
   *  constant the compiler will do clever tricks to turn it into a
   * multiply+add+shift,
   ...
   * compute the
   *  constants for exact division based on the size, as described in "{N}-bit
   * Unsigned
   *  Division via {N}-bit Multiply-Add" by Arch D. Robison in 2005. But that
   * code is
   ...
   * the range
   *  [0,1) and simply multiply it by the size.  Then we just shift the result
   * down by
   *  32-bits to get our bucket number.  The results has non-uniformity the same
   * as a
   *  mod, but it is much faster to compute. More about this technique can be
   * found at
  ...
  inline std::array<uint32_t, 8> compute_hashes(const Element &e) const {
    return {
        {(uint32_t)(
             (hash_function.template operator()<0>(e) * (uint64_t)size) >> 32),
         (uint32_t)(
             (hash_function.template operator()<1>(e) * (uint64_t)size) >> 32),
         (uint32_t)(
             (hash_function.template operator()<2>(e) * (uint64_t)size) >> 32),
         (uint32_t)(
             (hash_function.template operator()<3>(e) * (uint64_t)size) >> 32),
         (uint32_t)(
             (hash_function.template operator()<4>(e) * (uint64_t)size) >> 32),
         (uint32_t)(
             (hash_function.template operator()<5>(e) * (uint64_t)size) >> 32),
         (uint32_t)(
             (hash_function.template operator()<6>(e) * (uint64_t)size) >> 32),
         (uint32_t)(
             (hash_function.template operator()<7>(e) * (uint64_t)size) >>
             32)}};
  }
