 ...
 * the
 * chainstate, while keeping user interface out of the common library, which is
 * shared
 * between jfkbitcoin1776d, and jfkbitcoin1776-qt and non-server tools.
*/
...
public:
  ...
  bool GetCoin(const COutPoint &outpoint, Coin &coin) const override {
    try {
      ...
      // Starting the shutdown sequence and returning false to the caller would
      // be
      // interpreted as 'entry not found' (as opposed to unable to read data),
      // and
      // could lead to invalid interpretation. Just exit immediately, as we
      // can't
...
void Shutdown() {
  ...
  /// failed part of the way,
  /// for example if the data directory was found to be locked.
  /// Be sure that anything that writes files or flushes caches only does this
  /// if the respective
  /// module was initialized.
  RenameThread("jfkbitcoin1776-shutoff");
  ...
  // would too. The only reason to do the above flushes is to let the wallet
  // catch
 ...
 */
static void HandleSIGTERM(int) { fRequestShutdown = true; }

static void HandleSIGHUP(int) { fReopenDebugLog = true; }
...
std::string HelpMessage(HelpMessageMode mode) {
  ...
  // Do not translate _(...) -help-debug options, Many technical terms, and only
  // a very small audience, so is unnecessary stress to translators.
  std::string strUsage = HelpMessageGroup(_("Options:"));
  strUsage += HelpMessageOpt("-?", _("Print this help message and exit"));
  strUsage += HelpMessageOpt("-version", _("Print version and exit"));
  strUsage += HelpMessageOpt(
      "-alertnotify=<cmd>",
      _("Execute command when a relevant alert is received or we see a really "
  ...
  if (showDebug)
    strUsage += HelpMessageOpt(
        "-blocksonly",
        strprintf(_("Whether to operate in a blocks only mode (default: %u)"),
                  DEFAULT_BLOCKSONLY));
  strUsage += HelpMessageOpt(
      "-assumevalid=<hex>",
      strprintf(
  ...
  strUsage += HelpMessageOpt(
      "-conf=<file>", strprintf(_("Specify configuration file (default: %s)"),
                                JFKBITCOIN1776_CONF_FILENAME));
  if (mode == HMM_JFKBITCOIN1776D) {
#if HAVE_DECL_DAEMON
    strUsage += HelpMessageOpt(
  ...
  if (showDebug)
    strUsage += HelpMessageOpt(
        "-feefilter", strprintf("Tell other nodes to filter invs to us by our "
                                "mempool min fee (default: %u)",
                                DEFAULT_FEEFILTER));
  strUsage += HelpMessageOpt(
      "-loadblock=<file>",
  ...
  strUsage += HelpMessageOpt(
      "-persistmempool", strprintf(_("Whether to save the mempool on shutdown "
                                     "and load on restart (default: %u)"),
                                   DEFAULT_PERSIST_MEMPOOL));
  strUsage +=
      HelpMessageOpt("-blockreconstructionextratxn=<n>",
                     strprintf(_("Extra transactions to keep in memory for "
                                 "compact block reconstructions (default: %u)"),
...
#ifndef WIN32
  strUsage += HelpMessageOpt("-pid=<file>",
                             strprintf(_("Specify pid file (default: %s)"),
                                       JFKBITCOIN1776_PID_FILENAME));
#endif
  strUsage += HelpMessageOpt(
      "-prune=<n>",
      strprintf(_("Reduce storage requirements by enabling pruning (deleting) "
                  ...
                  "entire blockchain. "
                  "(default: 0 = disable pruning blocks, 1 = allow manual "
                  "pruning via RPC, >%u = automatically prune block files to "
  ...
  strUsage += HelpMessageOpt(
      "-reindex",
      _("Rebuild chain state and block index from the blk*.dat files on disk"));
#ifndef WIN32
  strUsage += HelpMessageOpt(
      "-sysperms",
...
#endif
  strUsage += HelpMessageOpt(
      "-txindex", strprintf(_("Maintain a full transaction index, used by the "
                              "getrawtransaction rpc call (default: %u)"),
                            DEFAULT_TXINDEX));
  ...
  strUsage += HelpMessageOpt("-connect=<ip>",
                             _("Connect only to the specified node(s); "
                               "-connect=0 disables automatic connections"));
  ...
  strUsage +=
      HelpMessageOpt("-listen", _("Accept connections from outside (default: 1 "
  ...
  strUsage += HelpMessageOpt(
      "-maxsendbuffer=<n>",
      strprintf(
          _("Maximum per-connection send buffer, <n>*1000 bytes (default: %u)"),
  ...
  strUsage +=
      HelpMessageOpt("-onion=<ip:port>",
                     strprintf(_("Use separate SOCKS5 proxy to reach peers via "
                                 "Tor hidden services (default: %s)"),
                               "-proxy"));
  strUsage += HelpMessageOpt(
      "-onlynet=<net>",
  ...
  strUsage +=
      HelpMessageOpt("-peerbloomfilters",
                     strprintf(_("Support filtering of blocks and transaction "
                                 "with bloom filters (default: %u)"),
  ...
  strUsage += HelpMessageOpt(
      "-timeout=<n>", strprintf(_("Specify connection timeout in milliseconds "
                                  "(minimum: 1, default: %d)"),
                                DEFAULT_CONNECT_TIMEOUT));
  strUsage += HelpMessageOpt(
      "-torcontrol=<ip>:<port>",
      strprintf(
          _("Tor control port to use if onion listening enabled (default: %s)"),
          DEFAULT_TOR_CONTROL));
...
#if USE_UPNP
  strUsage +=
      HelpMessageOpt("-upnp", _("Use UPnP to map the listening port (default: "
  ...
  strUsage += HelpMessageOpt(
      ...
      _("Whitelist peers connecting from the given IP address (e.g. 1.2.3.4) "
        "or CIDR notated network (e.g. 1.2.3.0/24). Can be specified multiple "
        "times.") +
  ...
  strUsage += HelpMessageOpt("-zmqpubhashtx=<address>",
  ...
  strUsage += HelpMessageOpt("-zmqpubrawtx=<address>",
  ...
  if (showDebug) {
    strUsage += HelpMessageOpt(
        ...
        strprintf(
            ...
            DEFAULT_CHECKBLOCKS));
    strUsage += HelpMessageOpt(
        "-checklevel=<n>", strprintf(_("How thorough the block verification of "
    ...
    strUsage += HelpMessageOpt("-checkpoints",
                               strprintf("Disable expensive verification for "
    ...
    strUsage += HelpMessageOpt("-vbparams=deployment:start:end",
                               ...
                               "version bits deployment (regtest-only)");
  }
  strUsage += HelpMessageOpt(
      "-debug=<category>",
      strprintf(_("Output debugging information (default: %u, supplying "
                ...
                0) +
          ". " + _("If <category> is not supplied or if <category> = 1, output "
  ...
  if (showDebug) {
    ...
    strUsage += HelpMessageOpt(
        "-maxsigcachesize=<n>",
        strprintf("Limit sum of signature cache and script execution cache "
                  "sizes to <n> MiB (default: %u)",
                  DEFAULT_MAX_SIG_CACHE_SIZE));
    strUsage +=
        HelpMessageOpt("-maxtipage=<n>",
                       strprintf("Maximum tip age in seconds to consider node "
                                 "in initial block download (default: %u)",
                                 DEFAULT_MAX_TIP_AGE));
  }
  strUsage += HelpMessageOpt(
      "-maxtxfee=<amt>",
  ...
  if (showDebug) {
    strUsage += HelpMessageOpt(
        "-printpriority",
        strprintf("Log transaction fee per kB when mining blocks (default: %u)",
  ...
  if (showDebug) {
    ...
    strUsage += HelpMessageOpt(
        ...
        strprintf("Fee rate (in %s/kB) used to define cost of relay, used for "
    ...
    strUsage += HelpMessageOpt(
        ...
        strprintf("Fee rate (in %s/kB) used to defined dust, the value of an "
                  "output such that it will cost more than its value in fees "
                  "at this fee rate to spend it. (default: %s)",
  ...
  strUsage += HelpMessageOpt(
      "-datacarriersize",
      strprintf(_("Maximum size of data in data carrier transactions we relay "
                  "and mine (default: %u)"),
                MAX_OP_RETURN_RELAY));
  strUsage += HelpMessageOpt(
      "-mempoolreplacement",
      strprintf(
          _("Enable transaction replacement in the memory pool (default: %u)"),
  ...
  strUsage += HelpMessageOpt(
      "-whitelistrelay",
      strprintf(_("Accept relayed transactions received from whitelisted peers "
                  "even when not relaying transactions (default: %d)"),
                DEFAULT_WHITELISTRELAY));
  strUsage += HelpMessageOpt(
      "-whitelistforcerelay",
      strprintf(_("Force relay of transactions from whitelisted peers even if "
  ...
  strUsage += HelpMessageOpt(
      "-blockmaxweight=<n>",
      strprintf(_("Set maximum BIP141 block weight (default: %d)"),
                DEFAULT_BLOCK_MAX_WEIGHT));
  strUsage += HelpMessageOpt("-blockmaxsize=<n>",
                             _("Set maximum BIP141 block weight to this * 4. "
                               "Deprecated, use blockmaxweight"));
  strUsage += HelpMessageOpt(
      "-blockmintxfee=<amt>",
  ...
  strUsage += HelpMessageOpt(
      "-rpcbind=<addr>[:port]",
      _("Bind to given address to listen for JSON-RPC connections. This option "
  ...
  strUsage += HelpMessageOpt("-rpcport=<port>",
                             strprintf(_("Listen for JSON-RPC connections on "
                                         "<port> (default: %u or testnet: %u)"),
                                       defaultBaseParams->RPCPort(),
                                       testnetBaseParams->RPCPort()));
  strUsage += HelpMessageOpt(
      "-rpcallowip=<ip>",
      _("Allow JSON-RPC connections from specified source. Valid for <ip> are "
        "a single IP (e.g. 1.2.3.4), a network/netmask (e.g. "
        "1.2.3.4/255.255.255.0) or a network/CIDR (e.g. 1.2.3.4/24). This "
        "option can be specified multiple times"));
  strUsage +=
      HelpMessageOpt("-rpcserialversion",
                     strprintf(_("Sets the serialization of raw transaction or "
                                 "block hex returned in non-verbose mode, "
                                 "non-segwit(0) or segwit(1) (default: %d)"),
  ...
  if (showDebug) {
    strUsage += HelpMessageOpt("-rpcworkqueue=<n>",
                               strprintf("Set the depth of the work queue to "
                                         "service RPC calls (default: %d)",
                                         DEFAULT_HTTP_WORKQUEUE));
    strUsage +=
        HelpMessageOpt("-rpcservertimeout=<n>",
                       strprintf("Timeout during HTTP requests (default: %d)",
                                 DEFAULT_HTTP_SERVER_TIMEOUT));
  }
  strUsage += HelpMessageOpt(
      "-gen", strprintf(_("Generate coins (default: %u)"), DEFAULT_GENERATE));
  strUsage += HelpMessageOpt(
      "-genproclimit=<n>",
      strprintf(_("Set the number of threads for coin generation if enabled "
                  "(-1 = all cores, default: %d)"),
...
std::string LicenseInfo() {
  ...
  const std::string URL_WEBSITE = "<https://jfkbitcoin1776core.org>";
  ...
  return CopyrightHolders(
         ...
         "\n" + "\n" +
         strprintf(_("This product includes software developed by the OpenSSL "
                     "Project for use in the OpenSSL Toolkit %s and "
...
// ignored by the
// reindex.  Since reindexing works by starting at block file 0 and looping
// until a blockfile
// is missing, do the same here to delete any later block files after a gap.
// Also delete all
// rev files since they'll be rewritten by the reindex anyway.  This ensures
// that vinfoBlockFile
// is in sync with what's actually on disk by the time we start downloading, so
// that pruning
// works correctly.
...
void ThreadImport(std::vector<fs::path> vImportFiles) {
  ...
  RenameThread("jfkbitcoin1776-loadblk");
...
/** Sanity checks
 *  Ensure that JFKBitcoin1776 is running in a usable environment with all
...
void InitParameterInteraction() {
  ...
  if (gArgs.IsArgSet("-connect")) {
    // when only connecting to trusted nodes, do not seed via DNS, or listen by
    // default
  ...
  if (gArgs.IsArgSet("-proxy")) {
    // to protect privacy, do not listen by default if a default proxy server is
    // specified
    if (gArgs.SoftSetBoolArg("-listen", false))
      LogPrintf("%s: parameter interaction: -proxy set -> setting -listen=0\n",
                __func__);
    // to protect privacy, do not use UPNP when a proxy is set. The user may
    // still specify -listen=1
  ...
  if (gArgs.GetBoolArg("-whitelistforcerelay", DEFAULT_WHITELISTFORCERELAY)) {
    if (gArgs.SoftSetBoolArg("-whitelistrelay", true))
      LogPrintf("%s: parameter interaction: -whitelistforcerelay=1 -> setting "
                "-whitelistrelay=1\n",
                __func__);
  }
  ...
  if (gArgs.IsArgSet("-blockmaxsize")) {
    ...
    if (gArgs.SoftSetArg("blockmaxweight",
                         strprintf("%d", max_size * WITNESS_SCALE_FACTOR))) {
      LogPrintf("%s: parameter interaction: -blockmaxsize=%d -> setting "
                "-blockmaxweight=%d (-blockmaxsize is deprecated!)\n",
                __func__, max_size, max_size * WITNESS_SCALE_FACTOR);
    } else {
      LogPrintf("%s: Ignoring blockmaxsize setting which is overridden by "
                "blockmaxweight",
                __func__);
    }
  }
...
void InitLogging() {
  ...
  LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
  LogPrintf("JFKBitcoin1776 version %s\n", FormatFullVersion());
}
...
bool AppInitParameterInteraction() {
  ...
  if (gArgs.GetBoolArg("-disablefork", false)) {
    printf("NOTE: disabling fork; this node will run on the jfkbitcoin1776 "
           "network\n");
    FORK_ALLOWED = false;
  ...
  if (nMaxConnections < nUserMaxConnections)
    InitWarning(strprintf(_("Reducing -maxconnections from %d to %d, because "
  ...
  hashAssumeValid = uint256S(gArgs.GetArg(
      "-assumevalid", chainparams.GetConsensus().defaultAssumeValid.GetHex()));
  ...
  if (gArgs.IsArgSet("-minimumchainwork")) {
    ...
    if (!IsHexNumber(minChainWorkStr)) {
      return InitError(
          strprintf("Invalid non-hex (%s) minimum chain work value specified",
                    minChainWorkStr));
    }
  ...
  // incremental relay fee sets the minimum feerate increase necessary for BIP
  // 125 replacement in the mempool
  // and the amount the mempool min fee increases above the feerate of txs
  // evicted due to mempool limiting.
  ...
  if (nPruneArg == 1) { // manual pruning: -prune=1
    LogPrintf("Block pruning enabled.  Use RPC call pruneblockchain(height) to "
              "manually prune block and undo files.\n");
    nPruneTarget = std::numeric_limits<uint64_t>::max();
    fPruneMode = true;
  } else if (nPruneTarget) {
    if (nPruneTarget < MIN_DISK_SPACE_FOR_BLOCK_FILES) {
      return InitError(strprintf(_("Prune configured below the minimum of %d "
  ...
  if (gArgs.IsArgSet("-minrelaytxfee")) {
    ...
    if (!ParseMoney(gArgs.GetArg("-minrelaytxfee", ""), n)) {
      return InitError(
          AmountErrMsg("minrelaytxfee", gArgs.GetArg("-minrelaytxfee", "")));
    }
  ...
  if (gArgs.IsArgSet("-blockmintxfee")) {
    ...
    if (!ParseMoney(gArgs.GetArg("-blockmintxfee", ""), n))
      return InitError(
          AmountErrMsg("blockmintxfee", gArgs.GetArg("-blockmintxfee", "")));
  }
  ...
  fEnableReplacement =
      gArgs.GetBoolArg("-mempoolreplacement", DEFAULT_ENABLE_REPLACEMENT);
  if ((!fEnableReplacement) && gArgs.IsArgSet("-mempoolreplacement")) {
    // Minimal effort at forwards compatibility
    std::string strReplacementModeList =
        gArgs.GetArg("-mempoolreplacement", ""); // default is impossible
    std::vector<std::string> vstrReplacementModes;
    boost::split(vstrReplacementModes, strReplacementModeList,
                 boost::is_any_of(","));
    fEnableReplacement =
        (std::find(vstrReplacementModes.begin(), vstrReplacementModes.end(),
  ...
  if (gArgs.IsArgSet("-vbparams")) {
    ...
    for (const std::string &strDeployment : gArgs.GetArgs("-vbparams")) {
      ...
      if (vDeploymentParams.size() != 3) {
        return InitError("Version bits parameters malformed, expecting "
      ...
      for (int j = 0; j < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++j) {
        if (vDeploymentParams[0].compare(VersionBitsDeploymentInfo[j].name) ==
            0) {
          UpdateVersionBitsParameters(Consensus::DeploymentPos(j), nStartTime,
                                      nTimeout);
          found = true;
...
static bool LockDataDirectory(bool probeOnly) {
  ...
  // Make sure only a single JFKBitcoin1776 process is using the data directory.
...
bool AppInitSanityChecks() {
  ...
  if (!InitSanityCheck())
    return InitError(
        strprintf(_("Initialization sanity check failed. %s is shutting down."),
                  _(PACKAGE_NAME)));
...
bool AppInitLockDataDirectory() {
  ...
  // until exit
  // This creates a slight window for a race condition to happen, however this
  // condition is harmless: it
...
// application initialization
  ...
  LogPrintf("Using config file %s\n",
            GetConfigFile(gArgs.GetArg("-conf", JFKBITCOIN1776_CONF_FILENAME))
                .string());
...
// wallet database integrity
  ...
  if (strSubVersion.size() > MAX_SUBVERSION_LENGTH) {
    return InitError(strprintf(
        _("Total length of network version string (%i) exceeds maximum length "
          "(%i). Reduce the number or size of uacomments."),
  ...
  // -noproxy (or -proxy=0) as well as the empty string can be used to not set a
  ...
  if (proxyArg != "" && proxyArg != "0") {
    ...
    if (!Lookup(proxyArg.c_str(), proxyAddr, 9050, fNameLookup)) {
      return InitError(
          strprintf(_("Invalid -proxy address or hostname: '%s'"), proxyArg));
    }
    ...
    SetLimited(NET_TOR, false); // by default, -proxy sets onion as reachable,
                                // unless -noonion later
  }

  // -onion can be used to set only a proxy for .onion, or override normal proxy
  // for .onion addresses
  // -noonion (or -onion=0) disables connecting to .onion entirely
  // An empty string is used to not override the onion proxy (in which case it
  // defaults to -proxy set above, or none)
  ...
  if (onionArg != "") {
    if (onionArg == "0") { // Handle -noonion/-onion=0
      ...
      if (!Lookup(onionArg.c_str(), onionProxy, 9050, fNameLookup)) {
        return InitError(
            strprintf(_("Invalid -onion address or hostname: '%s'"), onionArg));
      }
  ...
  nTotalCache = std::max(
      nTotalCache,
      nMinDbCache << 20); // total cache cannot be less than nMinDbCache
  nTotalCache = std::min(
      nTotalCache,
      nMaxDbCache << 20); // total cache cannot be greater than nMaxDbcache
  ...
  int64_t nCoinDBCache =
      std::min(nTotalCache / 2,
               (nTotalCache / 4) +
                   (1 << 23)); // use 25%-50% of the remainder for disk cache
  nCoinDBCache = std::min(nCoinDBCache,
                          nMaxCoinsDBCache << 20); // cap total coins db cache
    ...
    do {
      try {
        ...
        if (fReset) {
          ...
          // If we're reindexing in prune mode, wipe away unusable block files
          // and all undo data files
          if (fPruneMode)
        ...
        // If we're not mid-reindex (based on disk + args), add a genesis block
        // on disk
        ...
        pcoinsdbview =
            new CCoinsViewDB(nCoinDBCache, false, fReset || fReindexChainState);
        pcoinscatcher = new CCoinsViewErrorCatcher(pcoinsdbview);

        // If necessary, upgrade from older database format.
        // This is a no-op if we cleared the coinsviewdb with -reindex or
        // -reindex-chainstate
        ...
        // ReplayBlocks is a no-op if we cleared the coinsviewdb with -reindex
        // or -reindex-chainstate
        if (!ReplayBlocks(chainparams, pcoinsdbview)) {
          strLoadError = _("Unable to replay blocks. You will need to rebuild "
        ...
        bool is_coinsview_empty =
            fReset || fReindexChainState || pcoinsTip->GetBestBlock().IsNull();
        ...
        conforksus_init(chainActive.Tip() ? chainActive.Tip()->nHeight : 0,
                        Params().NetworkIDString() == "regtest");
        if (fork_conforksus.active) {
          // change port from default
          chainparams.ActivateConsensusRules();
        }

        if (!fReset) {
          // Note that RewindBlockIndex MUST run even if we're about to
          // -reindex-chainstate.
          // It both disconnects blocks based on chainActive, and drops block
          // data in
          // mapBlockIndex based on lack of available witness data.
          uiInterface.InitMessage(_("Rewinding blocks..."));
          if (!RewindBlockIndex(chainparams)) {
            strLoadError =
                _("Unable to rewind the database to a pre-fork state. You will "
                  "need to redownload the blockchain");
            break;
          }
        }
        ...
        if (!is_coinsview_empty) {
          ...
          if (fHavePruned &&
              gArgs.GetArg("-checkblocks", DEFAULT_CHECKBLOCKS) >
                  MIN_BLOCKS_TO_KEEP) {
            LogPrintf("Prune: pruned datadir may not have more than %d blocks; "
                      "only checking available blocks",
                      MIN_BLOCKS_TO_KEEP);
          }

          {
            ...
            if (tip && tip->nTime > GetAdjustedTime() + 2 * 60 * 60) {
              strLoadError =
                  _("The block database contains a block which appears to be "
                    "from the future. "
                    "This may be due to your computer's date and time being "
                    "set incorrectly. "
                    "Only rebuild the block database if you are sure that your "
...
// ********************************************************* Step 8: load wallet
  ...
  if (chainparams.GetConsensus()
          .vDeployments[Consensus::DEPLOYMENT_SEGWIT]
          .nTimeout != 0) {
    // Only advertise witness capabilities if they have a reasonable start time.
    // This allows us to have the code merged without a defined softfork, by
    // setting its
    // end time to 0.
    // Note that setting NODE_WITNESS is never required: the only downside from
    // not
    // doing so is that after activation, no upgraded nodes will fetch from you.
    nLocalServices = ServiceFlags(nLocalServices | NODE_WITNESS);
    // Only care about others providing witness capabilities if there is a
    // softfork
    // defined.
  ...
  GenerateJFKBitcoin1776s(
      gArgs.GetBoolArg("-gen", DEFAULT_GENERATE),
      gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
